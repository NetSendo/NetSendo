...
export function registerCampaignTools(server, api) {
    // List Campaigns
    server.tool('list_campaigns', 'List all campaigns (email/SMS messages) with optional filtering. Returns campaign details including status, type, and recipient counts.', {
    ...
    server.tool('get_campaign', 'Get detailed information about a specific campaign including contact lists, exclusions, and configuration.', {
    ...
    // Create Campaign
    server.tool('create_campaign', 'Create a new email or SMS campaign. Returns the created campaign with its ID.', {
    ...
    // Update Campaign
    server.tool('update_campaign', 'Update an existing campaign. Only draft or scheduled campaigns can be updated.', {
    ...
    // Send Campaign
    server.tool('send_campaign', 'Send a campaign immediately (broadcast) or activate it (autoresponder).', {
        campaign_id: z.number().describe('Campaign ID'),
    }, async ({ campaign_id }) => {
        try {
