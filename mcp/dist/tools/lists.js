...
Use default_mailbox info when creating campaigns to automatically select the right sender.`, {
    ...
    // Get List Stats
    server.tool('get_list_stats', `Operational snapshot of a list: members broken down by status, engagement share (how many have ever opened or clicked), growth over the last 30 days, and the configuration that affects sending — double opt-in, resubscription behaviour, signup limits, default mailbox and webhook.
...
Use this to answer "how is this list doing?" in one call. For deeper analysis: get_list_engagement for performance over time, analyze_list_health for data-quality problems.`, {
    ...
    // Create Contact List
    server.tool('create_contact_list', `Create a contact list.
...
Set a default mailbox so campaigns for this list pick the right sender automatically (see list_mailboxes).`, {
        ...
        try {
            return ok(await api.createContactList(compact(input)));
        }
        catch (error) {
            return fail(error);
        }
    ...
    // Update Contact List
    server.tool('update_contact_list', `Update a contact list's settings. Only the fields you pass are changed; the rest of the list configuration is preserved.
...
Note: changing resubscription_behavior or double_opt_in affects future signups only, never existing members.`, {
        ...
        try {
            return ok(await api.updateContactList(list_id, compact(input)));
        }
        catch (error) {
            return fail(error);
        }
    ...
    // Delete Contact List
    server.tool('delete_contact_list', `Delete a contact list. The contacts themselves are NOT deleted — they keep their other memberships and stay in the account.
