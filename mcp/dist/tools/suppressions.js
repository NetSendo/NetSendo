...
export function registerSuppressionTools(server, api) {
    server.tool('list_suppressions', 'Show the account-wide suppression list — addresses that must never be mailed again, with the reason and the date they were suppressed.', {
    ...
    server.tool('suppress_emails', `Add addresses to the account-wide do-not-mail list and, by default, unsubscribe them from every list they are on.
...
Only suppress addresses when the user has asked for it or the person clearly requested it — it is a deliberate, account-wide block, not a clean-up shortcut.`, {
    ...
    server.tool('unsuppress_emails', `Lift suppression for addresses. This does NOT resubscribe anyone — it only removes the block, so consent has to be collected again through a normal signup.
