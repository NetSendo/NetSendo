...
export function registerListExportTools(server, api) {
    server.tool('export_list', `Export a list's members. Returns the rows inline, so the data can be analysed, transformed or handed to the user directly.
...
Use get_export_fields first if unsure which columns, including custom fields, are available.`, {
        ...
        try {
            return ok(await api.exportList(list_id, compact(options)));
        }
        catch (error) {
            return fail(error);
        }
    });
    server.tool('get_export_fields', 'List the columns export_list can return for a list: standard subscriber fields plus the account\'s custom fields with their names and types.', {
        list_id: z.number().describe('Contact list ID'),
    }, async ({ list_id }) => {
        try {
    ...
    server.tool('queue_list_export', `Queue the full CSV export of a list. The file is generated in the background and NetSendo sends the account owner a download link.
...
//# sourceMappingURL=list-export.js.map