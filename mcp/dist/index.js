...
async function main() {
    ...
    server.resource('netsendo://info', 'NetSendo instance information', async () => {
        try {
            ...
            return {
                contents: [{
                        uri: 'netsendo://info',
                        mimeType: 'text/plain',
                        text: `Error fetching NetSendo info: ${error.message}`,
                    }],
            };
        }
    });
    // Quick stats resource
    server.resource('netsendo://stats', 'Quick statistics overview', async () => {
        try {
            ...
            const totalSubscribers = lists.data.reduce((sum, list) => sum + list.subscribers_count, 0);
            return {
                contents: [{
                        uri: 'netsendo://stats',
                        mimeType: 'application/json',
...
- Suggested segmentation strategies`,
    ...
    server.prompt('send_newsletter', 'Help compose and send a newsletter to subscribers', {
