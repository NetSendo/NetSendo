...
import type { HygieneAction, HygieneCategory } from '../types.js';
...
export function registerListHygieneTools(server: McpServer, api: NetSendoApiClient) {

  server.tool(
    'analyze_list_health',
    `Full hygiene report for a list. Read-only — this never changes anything.
...
${CATEGORY_GUIDE}`,
  ...
  server.tool(
    'clean_list',
    `Apply an action to every member matching the chosen problem categories.
...
${CATEGORY_GUIDE}`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.cleanList(list_id, compact(payload) as {
  ...
  server.tool(
    'dedupe_list',
    `Merge members whose addresses point at the same real mailbox — case differences, and Gmail dot/+tag aliases (j.an+news@gmail.com = jan@gmail.com).
...
SAFETY: dry_run defaults to true and shows the exact merge groups. Writing requires dry_run=false AND confirm=true.`,
  ...
  server.tool(
    'verify_list_emails',
    `Check whether a list's addresses can actually receive mail: syntax, disposable and role detection, typo domains with suggested corrections, and an MX/DNS lookup per domain.
...
Returns deliverable / risky / undeliverable counts, the overall deliverable rate, domains lacking MX records, and the individual problem addresses.`,
  ...
  server.tool(
    'get_hygiene_options',
    'Return the exact category and action names clean_list accepts, and which actions are irreversible. Use this if a clean_list call was rejected for an unknown category.',
    {},
    async () => {
      try {
