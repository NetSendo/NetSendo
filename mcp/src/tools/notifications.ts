...
export function registerNotificationTools(server: McpServer, api: NetSendoApiClient) {

  server.tool(
    'send_notification',
    `Post a notification to the NetSendo account owner's notification centre.
...
This reaches only the account owner. It cannot message subscribers or third parties — use campaigns or send_email for that.`,
    ...
    async (payload) => {
      try {
        const result = await api.createNotification(compact(payload) as { title: string });
  ...
  server.tool(
    'list_notifications',
    'Read the account owner\'s recent notifications, including how many are unread. Useful to check whether an earlier report was already delivered before sending another.',
    {
