...
export function registerSubscriberTools(server: McpServer, api: NetSendoApiClient) {
  
  // List Subscribers
  server.tool(
    'list_subscribers',
    'List subscribers with optional filtering. Returns paginated results with subscriber details including email, name, status, and tags.',
    {
  ...
  server.tool(
    ...
    async ({ id, email }) => {
      try {
        if (!id && !email) {
          return {
            content: [{ type: 'text' as const, text: 'Error: Either id or email is required' }],
            isError: true,
          };
        }
  ...
  server.tool(
    'create_subscriber',
    'Create a new subscriber with email, name, and optional list/tag assignments.',
    {
    ...
    async (params) => {
      try {
        const subscriber = await api.createSubscriber({
          ...
          phone: params.phone,
          lists: params.lists,
          tags: params.tags as (number[] | string[] | undefined),
  ...
  server.tool(
    'delete_subscriber',
    'Permanently delete a subscriber from the system.',
    {
    ...
    async ({ id, confirm }) => {
      try {
        if (!confirm) {
          return {
            content: [{ type: 'text' as const, text: 'Error: Must set confirm=true to delete subscriber' }],
            isError: true,
          };
        }
  ...
  server.tool(
    'sync_subscriber_tags',
    'Update the tags assigned to a subscriber. This replaces all existing tags.',
    {
