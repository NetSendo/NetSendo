 ...
 */
function describeStep(step: FunnelStep) {
  const byType: Record<string, Array<keyof FunnelStep>> = {
    email: ['message_id'],
    sms: ['sms_content'],
...
export function registerFunnelTools(server: McpServer, api: NetSendoApiClient) {
  ...
  server.tool(
    'update_funnel',
    'Rename a funnel or change its trigger. A list_signup funnel starts when someone signs up to trigger_list_id, tag_added when trigger_tag is added, form_submit on trigger_form_id; manual only through enroll_subscriber_in_funnel.',
    {
      funnel_id: z.number().describe('Funnel ID'),
    ...
    async ({ funnel_id, ...changes }) => {
      try {
        const funnel = await api.updateFunnel(funnel_id, compact(changes) as Parameters<NetSendoApiClient['updateFunnel']>[1]);
  ...
  server.tool(
    'add_funnel_step',
    'Add a step to a funnel and connect it: after after_step_id, or else after the last step, continuing where that step led. After a condition step the new step goes on its "yes" path unless branch is "no". Set the settings of the step type (see each parameter). Use get_funnel to see step IDs and connections.',
    {
      funnel_id: z.number().describe('Funnel ID'),
    ...
    async ({ funnel_id, ...input }) => {
      try {
        const step = await api.addFunnelStep(funnel_id, compact(input) as Parameters<NetSendoApiClient['addFunnelStep']>[1]);
  ...
  server.tool(
    'update_funnel_step',
    'Change a funnel step: any of its settings, and its connections. next_step_id is where the step leads; a condition uses next_step_yes_id and next_step_no_id. Pass null to disconnect. Connected steps must belong to the same funnel.',
    {
      funnel_id: z.number().describe('Funnel ID'),
    ...
    async ({ funnel_id, step_id, ...changes }) => {
      try {
        const step = await api.updateFunnelStep(funnel_id, step_id, compact(changes) as Parameters<NetSendoApiClient['updateFunnelStep']>[2]);
  ...
  server.tool(
    'delete_funnel_step',
    'Delete a funnel step. Steps that led to it lead to its next step instead, and subscribers on it move on to that step. The start step cannot be deleted.',
    {
      funnel_id: z.number().describe('Funnel ID'),
  ...
  // Enroll Subscriber
  server.tool(
    'enroll_subscriber_in_funnel',
    'Put a subscriber into an active funnel now (the way to start a funnel with the manual trigger). The funnel runs its steps up to the first wait immediately. A subscriber goes through a funnel once.',
    {
      funnel_id: z.number().describe('Funnel ID'),
