...
export function registerFunnelTools(server: McpServer, api: NetSendoApiClient) {
  ...
  server.tool(
    'add_funnel_step',
    'Add a step to a funnel and connect it: after after_step_id, or else after the last step, continuing where that step led. After a condition step the new step goes on its "yes" path unless branch is "no"; after a split (A/B test) step on the path of variant, else on the split\'s default path. Set the settings of the step type (see each parameter). Use get_funnel to see step IDs, variant keys and connections.',
    {
      funnel_id: z.number().describe('Funnel ID'),
  ...
  server.tool(
    'delete_funnel_step',
    'Delete a funnel step. Steps (and split variants) that led to it lead to its next step instead, and subscribers on it move on to that step. The start step cannot be deleted.',
    {
      funnel_id: z.number().describe('Funnel ID'),
