...
- Not specifying mailbox_id for email campaigns`,
    {
      ...
      channel: z
        .enum(["email", "sms"])
        .describe(
          '⚠️ REQUIRED: Campaign channel - must be exactly "email" or "sms"',
        ),
      type: z
        .enum(["broadcast", "autoresponder"])
        .describe(
          'REQUIRED: Campaign type - "broadcast" (one-time) or "autoresponder" (triggered on subscription)',
        ),
      content: z
        .string()
        .optional()
      ...
      mailbox_id: z
        ...
        .optional()
        .describe(
          "Mailbox ID for sending (use list_mailboxes to get available IDs)",
        ),
      contact_list_ids: z
        .array(z.number())
        .optional()
        .describe(
          "Array of contact list IDs to send to (use list_contact_lists to get IDs)",
        ),
      excluded_list_ids: z
        .array(z.number())
        .optional()
      ...
      scheduled_at: z
        ...
        .optional()
        .describe(
          'ISO 8601 datetime to schedule sending (e.g., 2024-12-25T10:00:00Z). If provided, campaign status will be "scheduled"',
        ),
      day: z
        .number()
        .optional()
        .describe(
          "For autoresponders only: day offset after subscription (0 = same day)",
        ),
      time_of_day: z
        .string()
        .optional()
        .describe(
          'For autoresponders only: time to send (format: HH:MM, e.g., "09:00")',
        ),
      timezone: z
        .string()
        .optional()
    ...
    async ({
      ...
      try {
        ...
        if (channel === "email" && !mailbox_id) {
          console.error(
            "[NetSendo MCP] Warning: Creating email campaign without mailbox_id. Campaign will need mailbox configured before sending.",
          );
        }
...
NOTE: Sent campaigns cannot be modified.`,
  ...
  server.tool(
    ...
    async ({ campaign_id, excluded_list_ids }) => {
      try {
        const result = await api.setMessageExclusions(
          campaign_id,
          excluded_list_ids,
        );
...
- UTC`,
    {
      ...
      timezone: z
        ...
        .optional()
        .describe(
          "Timezone for interpreting scheduled_at (e.g., Europe/Warsaw). If not provided, uses user profile timezone.",
        ),
    },
    async ({ campaign_id, scheduled_at, timezone }) => {
      try {
        const campaign = await api.scheduleMessage(
          campaign_id,
          scheduled_at,
          timezone,
        );
              ...
              text: JSON.stringify(
                {
                  success: true,
                  message:
                    result.message.type === "autoresponder"
