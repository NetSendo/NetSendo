...
import type { ListImportPayload } from '../types.js';
...
export function registerListImportTools(server: McpServer, api: NetSendoApiClient) {

  server.tool(
    'preview_list_import',
    `Dry-run an import: parse the data, show the detected column mapping and report exactly what would happen — without writing anything.
...
Returns: detected mapping, per-action counts (create/update/reactivate/skip/invalid), a row sample and the problem rows with reasons.`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        return ok(await api.previewListImport(list_id, compact(payload) as ListImportPayload));
      } catch (error) {
        return fail(error);
      }
    }
  ...
  server.tool(
    'import_subscribers',
    `Import contacts onto a list. Creates new contacts, reactivates unsubscribed ones and fills gaps on existing ones.
...
Returns: created / updated / reactivated / skipped / invalid / failed counts plus the first errors with row numbers.`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.importToList(list_id, compact(payload) as ListImportPayload);
