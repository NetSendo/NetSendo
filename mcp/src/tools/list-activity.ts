...
export function registerListActivityTools(server: McpServer, api: NetSendoApiClient) {

  server.tool(
    'get_list_activity',
    `Chronological event feed for a list — who joined, confirmed, unsubscribed or bounced, and which messages were sent, opened, clicked or failed.
...
Narrow with "types" when looking for something specific, e.g. types=["unsubscribed","bounced"].`,
  ...
  server.tool(
    'get_list_engagement',
    `Performance snapshot for a list: audience by status, growth (added / lost / net / churn with a daily series), delivery metrics (sent, unique openers and clickers, open rate, click rate, click-to-open rate), the best performing messages and links, the most engaged contacts, and how much of the audience has gone quiet.
...
Use this before planning a campaign, when reporting on list health, or to decide whether a re-engagement sequence is warranted.`,
  ...
  server.tool(
    ...
    `Full history of one contact: profile and engagement counters, every list they belong to with status and dates, their tags, how many messages are still queued for them, and a timeline of signups, confirmations, unsubscribes, sends, opens and clicks.
