...
export function registerAbTestTools(server: McpServer, api: NetSendoApiClient) {

  // List A/B Tests
  server.tool(
    'list_ab_tests',
    'List all A/B tests with optional filtering by status or campaign.',
    {
  ...
  server.tool(
    'create_ab_test',
    'Create a new A/B test for a campaign. Add variants after creation.',
    {
  ...
  server.tool(
    'add_ab_variant',
    'Add a variant to an A/B test. Each variant represents one version to test.',
    {
  ...
  server.tool(
    'start_ab_test',
    'Start running an A/B test. Requires at least 2 variants.',
    {
      test_id: z.number().describe('A/B test ID'),
    },
    async ({ test_id }) => {
  ...
  server.tool(
    'end_ab_test',
    'End a running A/B test and optionally select a winner manually.',
    {
      test_id: z.number().describe('A/B test ID'),
  ...
  server.tool(
    'get_ab_test_results',
    'Get detailed results and statistics for an A/B test.',
    {
      test_id: z.number().describe('A/B test ID'),
    },
    async ({ test_id }) => {
  ...
  server.tool(
    'delete_ab_test',
    'Delete an A/B test. Cannot delete running tests.',
    {
