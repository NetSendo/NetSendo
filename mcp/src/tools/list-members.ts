...
import type { MemberSelection } from '../types.js';
...
export function registerListMemberTools(server: McpServer, api: NetSendoApiClient) {

  server.tool(
    'add_list_members',
    `Attach existing contacts to a list. This does not create contacts — use import_subscribers for new people.
...
By default the signup event fires, so the target list's welcome sequence and automations start. Pass trigger_automations=false to attach quietly.`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.addListMembers(list_id, compact(payload) as MemberSelection);
  ...
  server.tool(
    'remove_list_members',
    `Detach contacts from a list. The contacts themselves and their other list memberships are kept; their queued messages for this list are cancelled.
...
A filter-based removal requires confirm=true, since it can match a large segment.`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.removeListMembers(list_id, compact(payload) as MemberSelection);
        return ok({ ...result.data, message: result.message });
      } catch (error) {
        return fail(error);
      }
  ...
  server.tool(
    'set_member_status',
    `Change the membership status of contacts on a list.
...
${SELECTION_GUIDE}`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.setListMemberStatus(list_id, compact(payload) as MemberSelection & { status: 'active' });
        return ok({ ...result.data, message: result.message });
      } catch (error) {
        return fail(error);
      }
  ...
  server.tool(
    'copy_list_members',
    `Copy contacts to another list, keeping them on the source list.
...
${SELECTION_GUIDE}`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.transferListMembers(list_id, 'copy', compact(payload) as MemberSelection & { target_list_id: number });
        return ok({ ...result.data, message: result.message });
      } catch (error) {
        return fail(error);
      }
  ...
  server.tool(
    'move_list_members',
    `Move contacts to another list — added to the target, then detached from the source.
...
${SELECTION_GUIDE}`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.transferListMembers(list_id, 'move', compact(payload) as MemberSelection & { target_list_id: number });
        return ok({ ...result.data, message: result.message });
      } catch (error) {
        return fail(error);
      }
  ...
  server.tool(
    'tag_list_members',
    `Add or remove tags across a segment of a list. Tags named in "add" are created if they do not exist.
...
${SELECTION_GUIDE}`,
    ...
    async ({ list_id, ...payload }) => {
      try {
        const result = await api.tagListMembers(list_id, compact(payload) as MemberSelection);
        return ok({ ...result.data, message: result.message });
      } catch (error) {
        return fail(error);
      }
