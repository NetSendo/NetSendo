...
- **Subscribers on more than one of a message's lists could be misreported as "missed" and never scheduled (older lists):** the schedule projection took the signup date from an arbitrary `first()` list among the message's target lists — and ignored that membership's status — while the scheduler anchors on the most recent signup. For a message targeting several lists (or a subscriber who had unsubscribed from an older list and re-signed up on another) the two disagreed: the stale/earliest date put the send in the past, so the subscriber was counted as "missed" instead of being scheduled. Both paths now resolve the same anchor — the latest **active** membership among the message's lists — so the counts match what is actually scheduled. Legacy rows without a usable signup date fall back to the pivot's `created_at` instead of being silently dropped from every bucket.
