...
- **Vimeo support in the Webinars module:** live webinars can now stream from Vimeo, not just YouTube. The create/edit forms gained a video-provider selector (YouTube / Vimeo) that toggles the matching ID field; only the selected provider's id is persisted so the player renders unambiguously. The public watch page and the host Studio preview both embed the Vimeo player (controls hidden, autoplay), and the new `Webinar::vimeoEmbedUrl()` helper handles the unlisted-video `id/hash` form. New nullable `webinars.vimeo_id` column (migration `2026_07_10_150000_add_vimeo_to_webinars.php`, which also backfills `video_provider = 'youtube'` for existing YouTube webinars); translated in EN/PL/DE/ES.
