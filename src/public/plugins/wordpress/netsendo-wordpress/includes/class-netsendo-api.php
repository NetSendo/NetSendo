...
class NetSendo_WP_API {
    ...
    /**
     * Get custom fields from NetSendo
     *
     * @return array|false
     */
    public function get_custom_fields() {
        ...
        $endpoint = $this->api_url . '/api/v1/custom-fields';
        ...
        if (is_wp_error($response)) {
            $this->log('API Error getting custom fields: ' . $response->get_error_message());
            return false;
        }
        ...
        $this->log('API Error getting custom fields: ' . ($body['message'] ?? 'Unknown error'));
        return false;
    }
    ...
    public function is_subscribed($email, $list_id = null) {
        ...
        $endpoint = $this->api_url . '/api/v1/subscribers/by-email/' . urlencode($email);
        ...
        if ($code === 200 && !empty($body['data'])) {
            if ($list_id) {
                // Check if subscribed to specific list
                foreach ($body['data'] as $subscriber) {
                    if (isset($subscriber['contact_list_id']) && $subscriber['contact_list_id'] == $list_id) {
    ...
    public function test_connection() {
        if (!$this->is_configured()) {
            return [
                'success' => false,
                'message' => __('API is not configured. Please enter API URL and API Key.', 'netsendo-wordpress'),
            ];
        }
        ...
        if ($code === 200) {
            ...
            return [
                'success' => true,
                'message' => __('Connection successful! NetSendo API is working.', 'netsendo-wordpress'),
                'user_id' => $user_id,
            ];
        } elseif ($code === 401) {
            return [
                'success' => false,
                'message' => __('Authentication failed. Please check your API Key.', 'netsendo-wordpress'),
            ];
        } else {
            return [
                'success' => false,
                'message' => sprintf(__('API returned error code: %d', 'netsendo-wordpress'), $code),
            ];
        }
    }
    ...
    private function save_user_id($user_id) {
        $settings = get_option(NetSendo_WP_Admin_Settings::OPTION_NAME, []);
        $settings['user_id'] = (int) $user_id;
        update_option(NetSendo_WP_Admin_Settings::OPTION_NAME, $settings);
    }
    ...
    private function log($message) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[NetSendo WordPress] ' . $message);
        }
    }
}
