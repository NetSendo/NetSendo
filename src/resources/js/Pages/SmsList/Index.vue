...
watch(form, debounce(() => {
    router.get(route('sms-lists.index'), form.value, {
        preserveState: true,
        replace: true,
    });
}, 300), { deep: true });
...
// View Mode Logic
const viewMode = ref(localStorage.getItem('smsList_viewMode') || 'grid');

watch(viewMode, (newMode) => {
    localStorage.setItem('smsList_viewMode', newMode);
});
</script>

<template>
    <Head :title="$t('sms_lists.title')" />
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        {{ $t('sms_lists.subtitle') }}
                    </p>
                </div>
                <Link
                    :href="route('sms-lists.create')"
                ...
                >
                    ...
                    {{ $t('sms_lists.new_list') }}
                </Link>
            </div>
        </template>

        <!-- Filters -->
        <div class="mb-6 flex flex-col gap-4 rounded-xl bg-white p-4 shadow-sm dark:bg-slate-900 md:flex-row md:items-center md:justify-between">
            <div class="flex flex-1 flex-col gap-4 md:flex-row">
                <div class="relative w-full md:max-w-xs">
                    ...
                    <input
                        v-model="form.search"
                        type="text"
                        :placeholder="$t('sms_lists.search_placeholder')"
                ...
                >
                    <option value="">{{ $t('sms_lists.all_groups') }}</option>
                ...
                >
                    <option value="">{{ $t('sms_lists.status') }}</option>
                    <option value="public">{{ $t('sms_lists.public') }}</option>
                    <option value="private">{{ $t('sms_lists.private') }}</option>
                </select>
                ...
                <select
                    ...
                    class="block w-full rounded-xl border-slate-200 bg-slate-50 text-sm text-slate-900 focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white md:w-48"
                >
                    <option value="">{{ $t('sms_lists.all_tags') }}</option>
            ...
            <div class="flex items-center gap-2">
                ...
                >
                    {{ $t('sms_lists.clear_filters') }}
                </button>
            </div>
        </div>

        <!-- Empty State -->
        <div v-if="lists.data.length === 0" class="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-white py-16 text-center dark:border-slate-700 dark:bg-slate-900">
            <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-indigo-50 dark:bg-indigo-900/20">
            ...
            <h3 class="text-lg font-medium text-slate-900 dark:text-white">{{ $t('sms_lists.empty_title') }}</h3>
            <p class="mt-1 text-slate-500 dark:text-slate-400">{{ $t('sms_lists.empty_description') }}</p>
            <Link
                :href="route('sms-lists.create')"
            ...
            >
                <div>
                    ...
                    <h3 class="mb-1 text-lg font-bold text-slate-900 dark:text-white">
                        <Link :href="route('sms-lists.show', list.id)" class="hover:text-indigo-600 dark:hover:text-indigo-400 focus:outline-none">
                            <span class="absolute inset-0" aria-hidden="true"></span>
                            {{ list.name }}
                        </Link>
                    </h3>
                    <p class="mb-4 line-clamp-2 text-sm text-slate-500 dark:text-slate-400">
                        {{ list.description || $t('sms_lists.no_description') }}
                    </p>

                    <div class="mb-4">
                        <div class="mb-2 flex flex-wrap gap-2">
                             <!-- Visibility Badge -->
                            <div class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium" :class="list.is_public ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'">
                                <span class="h-1.5 w-1.5 rounded-full" :class="list.is_public ? 'bg-green-500' : 'bg-slate-400'"></span>
                                {{ list.is_public ? $t('sms_lists.public') : $t('sms_lists.private') }}
                            </div>

                            <!-- Group -->
                             <div v-if="list.group" class="inline-flex items-center gap-1 rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
                ...
                <div class="flex items-center justify-between border-t border-slate-100 pt-4 dark:border-slate-800">
                    <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                        ...
                        <span class="font-semibold text-slate-900 dark:text-white">{{ list.subscribers_count }}</span> {{ $t('sms_lists.subscribers') }}
                    </div>
        ...
        <!-- Lists Table -->
        <div v-else class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-500 dark:text-slate-400">
                    <thead class="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                        <tr>
                            <th scope="col" class="px-6 py-3">{{ $t('sms_lists.table.name') }}</th>
                            <th scope="col" class="px-6 py-3">{{ $t('sms_lists.table.status') }}</th>
                            <th scope="col" class="px-6 py-3">{{ $t('sms_lists.table.groups_tags') }}</th>
                            <th scope="col" class="px-6 py-3 text-center">{{ $t('sms_lists.table.subscribers') }}</th>
                            <th scope="col" class="px-6 py-3">{{ $t('sms_lists.table.created_at') }}</th>
                            <th scope="col" class="px-6 py-3 text-right">{{ $t('sms_lists.table.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        <tr v-for="list in lists.data" :key="list.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                            <td class="px-6 py-4 font-medium text-slate-900 dark:text-white">
                                <Link :href="route('sms-lists.show', list.id)" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                                    {{ list.name }}
                                </Link>
                                <div class="text-xs text-slate-500 dark:text-slate-400">{{ list.description ? list.description.substring(0, 50) + (list.description.length > 50 ? '...' : '') : '' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium" :class="list.is_public ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'">
                                    <span class="h-1.5 w-1.5 rounded-full" :class="list.is_public ? 'bg-green-500' : 'bg-slate-400'"></span>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1">
                                    <div v-if="list.group" class="inline-flex w-fit items-center gap-1 rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
                                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/></svg>
                                        {{ list.group.name }}
                                    </div>
                                    <div class="flex flex-wrap gap-1">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="font-semibold text-slate-900 dark:text-white">{{ list.subscribers_count }}</span>
                            </td>
