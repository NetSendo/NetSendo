                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        <tr v-for="contact in contacts?.data" :key="contact.id"
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <Link :href="`/crm/contacts/${contact.id}`" class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-medium text-slate-900 dark:text-white">
                                            ...
                                            <span v-if="!contact.subscriber?.first_name && !contact.subscriber?.last_name" class="text-slate-500">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <span v-if="contact.company" class="text-sm text-slate-900 dark:text-white">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm text-slate-600 dark:text-slate-400">{{ contact.score }}</span>
                                </div>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <span v-if="contact.owner" class="text-sm text-slate-900 dark:text-white">
                                    {{ contact.owner.name }}
                                </span>
                                <span v-else class="text-sm text-slate-400">—</span>
                            </td>
                ...
                <div class="flex items-center gap-2">
                    <Link v-for="link in contacts.links" :key="link.label" :href="link.url || '#'"
                        :class="[
                            ...
                            link.active ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700',
