...
const getActivityIcon = (type) => {
    const icons = {
        ...
        deal_won: "M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z",
    ...
    <AuthenticatedLayout>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm dark:bg-slate-800">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            {{ formatCurrency(stats?.deals?.won_value_this_month || 0) }}
                        </p>
                    </div>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2 space-y-6">
                <!-- Overdue Tasks -->
                <div v-if="overdueTasks?.length > 0" class="rounded-2xl bg-white p-6 shadow-sm dark:bg-slate-800">
                    <div class="mb-4 flex items-center justify-between">
                        <h2 class="flex items-center gap-2 text-lg font-semibold text-red-600 dark:text-red-400">
                        ...
                        <div v-for="task in overdueTasks" :key="task.id"
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p v-if="task.contact" class="text-sm text-slate-500 dark:text-slate-400">
                ...
                <!-- Hot Leads -->
                <div v-if="hotLeads?.length > 0" class="rounded-2xl bg-white p-6 shadow-sm dark:bg-slate-800">
                    <div class="mb-4 flex items-center justify-between">
                        <h2 class="flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-white">
                    ...
                    <div class="space-y-3">
                        <Link v-for="lead in hotLeads" :key="lead.id" :href="`/crm/contacts/${lead.id}`"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-orange-400 to-red-500 text-white font-semibold">
                                    {{ (lead.subscriber?.first_name?.[0] || lead.subscriber?.email?.[0] || '?').toUpperCase() }}
                                </div>
                                <div>
                                    <p class="font-medium text-slate-900 dark:text-white">
                                        {{ lead.subscriber?.first_name }} {{ lead.subscriber?.last_name || lead.subscriber?.email }}
                                    </p>
                                    <p v-if="lead.company" class="text-sm text-slate-500 dark:text-slate-400">
                                        {{ lead.company.name }}
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="rounded-full bg-orange-100 px-2 py-1 text-xs font-bold text-orange-700 dark:bg-orange-900/30 dark:text-orange-300">
                                    Score: {{ lead.score }}
                                </span>
                            </div>
                        </Link>
