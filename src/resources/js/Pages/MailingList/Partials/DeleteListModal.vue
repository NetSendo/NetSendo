...
const deleteList = () => {
    ...
    form.delete(route('mailing-lists.destroy', props.list.id), {
        preserveScroll: true,
    ...
    <Modal :show="show" @close="closeModal">
        <div class="p-6">
            <h2 class="text-lg font-medium text-slate-900 dark:text-white">
                {{ $t('mailing_lists.delete_modal.title', { name: list?.name }) }}
            </h2>

            <div v-if="list?.subscribers_count > 0" class="mt-4">
                <p class="text-sm text-slate-500 dark:text-slate-400" v-html="$t('mailing_lists.delete_modal.has_subscribers', { count: list.subscribers_count })"></p>
                ...
                <div class="mt-4 space-y-4">
                    <label class="flex items-center space-x-3">
                        ...
                        <span class="block text-sm font-medium text-slate-700 dark:text-slate-300">
                            {{ $t('mailing_lists.delete_modal.transfer_label') }}
                        </span>
                    </label>
                    ...
                    <div v-if="form.action === 'transfer'" class="pl-7">
                        ...
                        >
                            <option value="" disabled>{{ $t('mailing_lists.delete_modal.transfer_placeholder') }}</option>
                            <option 
                        ...
                        <p v-if="availableLists.length === 0" class="mt-1 text-xs text-red-500">
                            {{ $t('mailing_lists.delete_modal.transfer_empty') }}
                        </p>
                    </div>
                    ...
                    <label class="flex items-center space-x-3">
                        ...
                        <span class="block text-sm font-medium text-slate-700 dark:text-slate-300 text-red-600 dark:text-red-400">
                            {{ $t('mailing_lists.delete_modal.delete_all_label') }}
                        </span>
                    </label>
                </div>
            </div>
            ...
            <p v-else class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {{ $t('mailing_lists.delete_modal.confirm_empty') }}
            </p>
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                >
                    {{ $t('mailing_lists.delete_modal.submit') }}
                </DangerButton>
            </div>
        </div>
    </Modal>
