...
const form = useForm({
    ...
    settings: {
        subscription: {
            ...
            security_options: getSetting('subscription', 'security_options', []),
        },
        sending: {
            mailbox_id: getSetting('sending', 'mailbox_id', props.list.default_mailbox_id),
            ...
            headers: {
                list_unsubscribe: props.list.settings?.sending?.headers?.list_unsubscribe || '',
                list_unsubscribe_post: props.list.settings?.sending?.headers?.list_unsubscribe_post || '',
            },
        },
        pages: (() => {
            ...
            const getPage = (key) => {
                ...
                return listPage || defaultPage || { type: 'system', url: '', external_page_id: null };
            };
            return {
        ...
        cron: {
            ...
            volume_per_minute: props.list.cron_settings?.volume_per_minute ?? null,
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        {{ $t('mailing_lists.edit_subtitle', { name: list.name }) }}
                    </p>
                </div>
            </div>
        </template>
                        ...
                        <div v-show="currentTab === 'settings'" class="flex flex-col gap-6 lg:flex-row">
                            ...
                            <div class="flex-1 lg:pl-6">
                                        ...
                                        <div class="grid gap-6">
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    type="text"
                                                    class="block w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 placeholder-slate-400 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 dark:focus:border-indigo-400 dark:focus:bg-slate-800"
                                                    :placeholder="defaultSettings?.sending?.headers?.list_unsubscribe || '<mailto:...>, <https://...>'"
                                                >
                                                <p class="mt-1 text-xs text-slate-500">{{ $t('mailing_lists.settings.headers_inherit_help') }}</p>
                                            </div>
                                            <div>
                                                <div class="flex items-center justify-between mb-2">
                                                    <label class="block text-sm font-medium text-slate-900 dark:text-white">
                                                ...
                                                <input
                                                    ...
                                                    type="text"
                                                    class="block w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 placeholder-slate-400 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 dark:focus:border-indigo-400 dark:focus:bg-slate-800"
                                                    :placeholder="defaultSettings?.sending?.headers?.list_unsubscribe_post || 'List-Unsubscribe=One-Click'"
                                                >
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div v-show="currentSettingsTab === 'cron'" class="space-y-6">
                                    <h3 class="text-lg font-medium text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-2">
                                        {{ $t('mailing_lists.settings.cron_section') }}
                                    </h3>
                                    ...
                                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800">
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <span class="block text-sm font-medium text-slate-900 dark:text-white">{{ $t('mailing_lists.settings.use_custom_cron') }}</span>
                                                <span class="block text-xs text-slate-500 dark:text-slate-400">
                                                    {{ $t('mailing_lists.settings.use_custom_cron_desc') }}
                                                </span>
                                            </div>
                                            <div class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out" 
                                                    :class="form.settings.cron.use_custom ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'"
                                                    @click="form.settings.cron.use_custom = !form.settings.cron.use_custom">
                                                <span class="pointer-events-none inline-block h-5 w-5 translate-x-0 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out" 
                                                        :class="{ 'translate-x-5': form.settings.cron.use_custom }"></span>
                                            </div>
                                        </div>
                                    </div>

                                    <!-- Global settings info -->
                                    <div v-if="!form.settings.cron.use_custom" class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
                                        <div class="flex items-start gap-3">
                                            <svg class="h-5 w-5 text-blue-500 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                            <div>
                                                <p class="text-sm font-medium text-blue-800 dark:text-blue-200">{{ $t('mailing_lists.settings.using_global_cron') }}</p>
                                                <p class="text-xs text-blue-600 dark:text-blue-300 mt-1" v-html="$t('mailing_lists.settings.using_global_cron_desc', { link: `<a href='${route('settings.cron.index')}' class='underline hover:no-underline'>${$t('navigation.settings')} CRON</a>` })">
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Custom settings -->
                                    <template v-if="form.settings.cron.use_custom">
                                        <!-- Volume per minute -->
                                        <div>
                                            <label class="mb-2 block text-sm font-medium text-slate-900 dark:text-white">
                                                {{ $t('mailing_lists.settings.cron_limit_label') }}
                                            </label>
                                            <input
                                                ...
                                                :placeholder="$t('mailing_lists.settings.cron_limit_placeholder')"
                                            >
                                            <p class="mt-1 text-xs text-slate-500">{{ $t('mailing_lists.settings.cron_limit_help') }}</p>
                                        </div>

                                        <!-- Weekly schedule -->
                                        <div>
                                            <label class="mb-3 block text-sm font-medium text-slate-900 dark:text-white">
                                                {{ $t('mailing_lists.settings.weekly_schedule') }}
                                            </label>
                                            <div class="space-y-3">
                                                <div 
                                                    v-for="day in days" 
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <div class="flex items-center gap-2">
                                                            <label class="text-xs text-slate-500">{{ $t('mailing_lists.settings.cron_from') }}</label>
                                                            <input 
                                                                type="time" 
                                                        ...
                                                        <div class="flex items-center gap-2">
                                                            <label class="text-xs text-slate-500">{{ $t('mailing_lists.settings.cron_to') }}</label>
                                                            <input 
                                                                type="time" 
                                                    ...
                                                    >
                                                        {{ $t('mailing_lists.settings.cron_disabled') }}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
