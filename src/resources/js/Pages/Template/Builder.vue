...
const templateName = ref(
    props.template?.name || props.starterTemplate?.name || ""
);
...
// Builder state
const blocks = ref(
    props.template?.json_structure?.blocks ||
        props.starterTemplate?.json_structure?.blocks ||
        []
);
const settings = ref(
    props.template?.settings ||
        props.starterTemplate?.settings || { ...props.defaultSettings }
);
    ...
    for (const parentBlock of blocks.value) {
        if (
            ...
            for (const columnBlocks of parentBlock.content.columnBlocks) {
                const nestedBlock = columnBlocks?.find(
...
const updateBlock = (blockId, updates) => {
    ...
    for (const parentBlock of blocks.value) {
        if (
            ...
            for (
                let colIndex = 0;
                colIndex < parentBlock.content.columnBlocks.length;
                colIndex++
            ) {
                const columnBlocks = parentBlock.content.columnBlocks[colIndex];
                if (!columnBlocks) continue;

                const nestedIndex = columnBlocks.findIndex(
                    (b) => b.id === blockId
                );
                if (nestedIndex !== -1) {
                    columnBlocks[nestedIndex] = {
                        ...columnBlocks[nestedIndex],
                        ...updates,
                    };
                    scheduleAutoSave();
...
const removeNestedBlock = (parentBlockId, columnIndex, nestedBlockId) => {
    ...
    if (parentBlock && parentBlock.content?.columnBlocks?.[columnIndex]) {
        const index = parentBlock.content.columnBlocks[columnIndex].findIndex(
            (b) => b.id === nestedBlockId
        );
        if (index > -1) {
...
<template>
    ...
    <div class="flex h-screen flex-col bg-slate-100 dark:bg-slate-950">
        <!-- Header Bar -->
        <header
            class="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 dark:border-slate-800 dark:bg-slate-900"
        >
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <div
                class="hidden w-80 shrink-0 overflow-y-auto border-l border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 md:block"
            >
                <!-- Tabs -->
                <div
                ...
                <div class="p-4">
                    ...
                    <BlockEditor
                        ...
                        @update="
                            (updates) => updateBlock(selectedBlockId, updates)
                        "
                    ...
                    <div v-else class="py-12 text-center text-slate-400">
                        ...
                        <p class="mt-3 text-sm">
                            {{ $t("template_builder.select_block") }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <!-- Mobile Bottom Nav -->
        <div
            class="fixed bottom-0 left-0 right-0 z-40 flex h-16 items-center justify-around border-t border-slate-200 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] dark:border-slate-800 dark:bg-slate-900 md:hidden"
        >
            <button
                @click="
                    activeMobileTab =
                        activeMobileTab === 'blocks' ? null : 'blocks'
                "
                :class="
            ...
            >
                ...
                <span class="text-[10px] font-medium">{{
                    $t("template_builder.mobile_nav_blocks")
                }}</span>
            </button>

            <button
                @click="
                    activeMobileTab = activeMobileTab === 'edit' ? null : 'edit'
                "
                :class="
            ...
            >
                ...
                <span class="text-[10px] font-medium">{{
                    $t("template_builder.mobile_nav_edit")
                }}</span>
            </button>
            ...
            <button
                @click="
                    activeMobileTab =
                        activeMobileTab === 'styles' ? null : 'styles'
                "
                :class="
            ...
            >
                ...
                <span class="text-[10px] font-medium">{{
                    $t("template_builder.mobile_nav_styles")
                }}</span>
            </button>
        ...
        <!-- Mobile Drawer -->
        <div
            v-if="activeMobileTab"
            class="fixed inset-0 z-50 flex flex-col bg-slate-100 dark:bg-slate-950 md:hidden pb-16"
        >
            <!-- Header -->
            <div
            ...
            >
                <h3 class="font-bold text-slate-900 dark:text-white">
                    <span v-if="activeMobileTab === 'blocks'">{{
                        $t("template_builder.mobile_nav_blocks")
                    }}</span>
                    <span v-else-if="activeMobileTab === 'edit'">{{
                        $t("template_builder.mobile_nav_edit")
                    }}</span>
                    <span v-else-if="activeMobileTab === 'styles'">{{
                        $t("template_builder.mobile_nav_styles")
                    }}</span>
                </h3>
