...
const getStatusText = (providerKey) => {
    const integration = getIntegration(providerKey);
    if (!integration) return page.props.auth.user ? t('ai.status.not_configured') : 'Nie skonfigurowano';
    
    if (!isConfigured(providerKey)) return t('ai.status.no_api_key');
    
    if (!integration.is_active) return t('ai.status.inactive');
    
    if (integration.last_test_status === 'success') return t('ai.status.active');
    if (integration.last_test_status === 'error') return t('ai.status.connection_error');
...
const submitForm = () => {
    if (modalMode.value === 'edit') {
        form.put(route('settings.ai-integrations.update', editingIntegration.value.id), {
            preserveScroll: true,
...
const testConnection = async (integration) => {
    ...
    try {
        ...
        const response = await fetch(route('settings.ai-integrations.test', integration.id), {
            method: 'POST',
            headers: {
...
const fetchModels = async (integration) => {
    ...
    try {
        const response = await fetch(route('settings.ai-integrations.models', integration.id), {
            headers: {
                'Content-Type': 'application/json',
...
const deleteIntegration = () => {
    ...
    router.delete(route('settings.ai-integrations.destroy', deletingIntegration.value.id), {
        preserveScroll: true,
...
const toggleActive = (integration) => {
    router.put(route('settings.ai-integrations.update', integration.id), {
...
const localizeModelName = (name) => {
    if (!name) return name;
    return name
        .replace('(Najnowszy - Grudzień 2025)', `(${t('ai.models.latest')} - ${t('ai.models.december_2025')})`)
        .replace('(Najnowszy - Listopad 2025)', `(${t('ai.models.latest')} - ${t('ai.models.november_2025')})`)
        .replace('(Najnowszy - Październik 2025)', `(${t('ai.models.latest')} - ${t('ai.models.october_2025')})`)
        .replace('(Najnowszy - Wrzesień 2025)', `(${t('ai.models.latest')} - ${t('ai.models.september_2025')})`)
        .replace('(Wrzesień 2025)', `(${t('ai.models.september_2025')})`)
        .replace('(Październik 2025)', `(${t('ai.models.october_2025')})`)
...
const defaultModels = {
    ...
    anthropic: [
        ...
        { model_id: 'claude-opus-4.1', display_name: 'Claude Opus 4.1' },
    ...
    openrouter: [
        ...
        { model_id: 'openai/gpt-5.2-instant', display_name: 'OpenAI GPT-5.2 Instant' },
        { model_id: 'openai/gpt-5', display_name: 'OpenAI GPT-5' },
        { model_id: 'openai/gpt-4o', display_name: 'OpenAI GPT-4o' },
        ...
        { model_id: 'anthropic/claude-haiku-4.5', display_name: 'Claude Haiku 4.5' },
        ...
        { model_id: 'mistralai/mistral-large-2411', display_name: 'Mistral Large' },
        { model_id: 'qwen/qwen-3-72b-instruct', display_name: 'Qwen 3 72B' },
    ...
    ollama: [
        ...
        { model_id: 'llama3.2-vision', display_name: 'Llama 3.2 Vision' },
        { model_id: 'qwen3', display_name: 'Qwen 3' },
        { model_id: 'qwen2.5-coder', display_name: 'Qwen 2.5 Coder' },
        { model_id: 'mistral-large', display_name: 'Mistral Large' },
        { model_id: 'mixtral', display_name: 'Mixtral 8x7B' },
        { model_id: 'codellama', display_name: 'Code Llama' },
    ...
    gemini: [
        ...
        { model_id: 'gemini-2.5-flash-native-audio-preview-12-2025', display_name: 'Gemini 2.5 Flash Native Audio (Grudzień 2025)' },
        ...
        <div class="space-y-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div
                    ...
                    :class="isConfigured(key) && getIntegration(key)?.is_active ? 'border-emerald-200 dark:border-emerald-800' : 'border-slate-200'"
                >
                    <!-- Provider Icon & Name -->
                    <div class="flex items-start justify-between">
                        ...
                        <button
                            ...
                            :class="getIntegration(key)?.is_active ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-slate-600'"
                            :title="getIntegration(key)?.is_active ? $t('ai.actions.disable') : $t('ai.actions.enable')"
                        >
                            <span 
                                class="absolute left-0.5 top-0.5 h-5 w-5 transform rounded-full bg-white shadow transition-transform"
                                :class="getIntegration(key)?.is_active ? 'translate-x-5' : 'translate-x-0'"
                            ></span>
                        </button>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        >
                            <span 
                                ...
                                :class="{
                                    'bg-emerald-500': isConfigured(key) && getIntegration(key)?.is_active && getIntegration(key)?.last_test_status === 'success',
                                    'bg-rose-500': getIntegration(key)?.last_test_status === 'error',
                                    'bg-amber-500': isConfigured(key) && getIntegration(key)?.is_active && !getIntegration(key)?.last_test_status,
                                    'bg-slate-400': !isConfigured(key) || !getIntegration(key)?.is_active,
                                }"
                            ></span>
                            {{ getStatusText(key) }}
                        </span>
                    ...
                    <div v-if="getIntegration(key)?.default_model" class="mt-3">
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            Model: <span class="font-medium text-gray-700 dark:text-gray-300">{{ getIntegration(key).default_model }}</span>
                        </p>
                    </div>
                    ...
                    <div class="mt-4 flex items-center gap-2">
                        <button
                            ...
                            :class="isConfigured(key) 
                                ? 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-slate-700 dark:text-gray-300 dark:hover:bg-slate-600' 
                                : 'bg-indigo-600 text-white hover:bg-indigo-700'"
                        >
                            {{ isConfigured(key) ? $t('ai.actions.edit') : $t('ai.actions.configure') }}
                        </button>

                        <template v-if="isConfigured(key)">
                            <button
                                @click="testConnection(getIntegration(key))"
                                :disabled="testingIntegration === getIntegration(key)?.id"
                            ...
                            >
                                <svg v-if="testingIntegration === getIntegration(key)?.id" class="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24">
                    ...
                    <div v-if="getIntegration(key)?.last_test_status === 'error'" class="mt-3">
                ...
                <div class="flex gap-3">
                    ...
                    <div class="text-sm text-blue-800 dark:text-blue-200">
                        ...
                        <ul class="mt-2 list-inside list-disc space-y-1 text-blue-700 dark:text-blue-300">
                            ...
                            <li><strong>Anthropic:</strong> <a href="https://console.anthropic.com/settings/keys" target="_blank" class="underline hover:no-underline">console.anthropic.com/settings/keys</a></li>
                            <li><strong>Grok:</strong> <a href="https://console.x.ai/" target="_blank" class="underline hover:no-underline">console.x.ai</a></li>
                            <li><strong>OpenRouter:</strong> <a href="https://openrouter.ai/keys" target="_blank" class="underline hover:no-underline">openrouter.ai/keys</a></li>
                            <li><strong>Gemini:</strong> <a href="https://aistudio.google.com/apikey" target="_blank" class="underline hover:no-underline">aistudio.google.com/apikey</a></li>
                            <li><strong>Ollama:</strong> {{ $t('ai.info.ollama', { url: 'ollama.ai' }) }}</li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
                    ...
                    <div class="mb-6 flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            {{ modalMode === 'edit' ? $t('ai.actions.edit') : $t('ai.actions.configure') }} {{ providers[form.provider]?.name }}
                        </h3>
                        <button
                            @click="closeModal"
                    ...
                    <form @submit.prevent="submitForm" class="space-y-4">
                        ...
                        <div v-if="providers[form.provider]?.requires_api_key">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                ...
                                <span v-if="modalMode === 'edit'" class="font-normal text-gray-500">{{ $t('ai.modal.api_key_help') }}</span>
                            </label>
                            <div class="relative mt-1">
                                <input
                                    ...
                                    :class="{ 'border-rose-500': form.errors.api_key }"
                                />
                                <button
                                    type="button"
                            ...
                            <p v-if="form.errors.api_key" class="mt-1 text-sm text-rose-600">{{ form.errors.api_key }}</p>
                        </div>
                        ...
                        <div v-if="providers[form.provider]?.supports_base_url">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                ...
                                <span class="font-normal text-gray-500">{{ $t('ai.modal.base_url_help') }}</span>
                            </label>
                            <input
                                v-model="form.base_url"
                                type="url"
                                :placeholder="providers[form.provider]?.default_base_url"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                            />
                            <p class="mt-1 text-xs text-gray-500">
                                {{ $t('ai.modal.base_url_default', { url: providers[form.provider]?.default_base_url }) }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative mt-1">
                                ...
                                <input
                                    v-model="modelSearch"
                                    type="text"
                                    :placeholder="$t('ai.modal.model_search_placeholder')"
                            ...
                            <select
                                ...
                                size="6"
                            >
                                <option value="">{{ $t('ai.modal.select_model_placeholder') }}</option>
                                <option 
                            ...
                            <p v-if="form.default_model" class="mt-1 text-xs text-gray-500">
                                {{ $t('ai.modal.selected_model', { model: form.default_model }) }}
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                />
                                <p class="mt-1 text-xs text-gray-500">
                                    {{ $t('ai.modal.tokens_small_help', { default: 2000 }) }}
                                </p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                ...
                                />
                                <p class="mt-1 text-xs text-gray-500">
                                    {{ $t('ai.modal.tokens_large_help', { default: 50000 }) }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4">
                            ...
                            >
                                {{ form.processing ? $t('ai.modal.saving') : $t('ai.modal.save') }}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {{ $t('ai.delete.confirm', { name: deletingIntegration?.name }) }} {{ $t('ai.delete.irreversible') }}
                        </p>
                    </div>
                </div>
