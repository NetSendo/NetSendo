...
const permissionLabels = computed(() => ({
    'subscribers:read': { label: t('api_keys.permissions.subscribers_read'), icon: '📧', color: 'blue' },
    'subscribers:write': { label: t('api_keys.permissions.subscribers_write'), icon: '✏️', color: 'green' },
    'lists:read': { label: t('api_keys.permissions.lists_read'), icon: '📋', color: 'purple' },
    'tags:read': { label: t('api_keys.permissions.tags_read'), icon: '🏷️', color: 'orange' },
}));
...
function deleteKey(keyId) {
    if (confirm(t('api_keys.delete_confirm'))) {
        router.delete(route('settings.api-keys.destroy', keyId));
    }
}
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm font-medium text-blue-800 dark:text-blue-200">{{ t('api_keys.docs.title') }}</p>
                            <p class="text-sm text-blue-600 dark:text-blue-300 mt-1">
                                {{ t('api_keys.docs.text') }} 
                                <a href="/docs/api" target="_blank" class="underline font-medium hover:text-blue-800 dark:hover:text-blue-100">
                                    /docs/api
                                </a>
                            </p>
                        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                    ...
                    <div v-if="apiKeys.length === 0" class="px-6 py-12 text-center">
                    ...
                    <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="key in apiKeys" :key="key.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <code class="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded text-sm font-mono text-gray-600 dark:text-gray-300">
                                        {{ key.key_prefix }}...
                                    </code>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        >
                                            {{ permissionLabels[perm]?.icon }} {{ permissionLabels[perm]?.label || perm }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    {{ key.last_used_at ? formatDateTime(key.last_used_at) : t('api_keys.status.never_used') }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                    ...
                    <div class="bg-gray-900 rounded-lg p-4 font-mono text-sm text-green-400 overflow-x-auto">
                        <pre># {{ t('api_keys.examples.get_subscribers') }}
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input 
                                    v-model="form.name"
                                    type="text"
                                    :placeholder="t('api_keys.modal.name_placeholder')"
                    ...
                    <div v-else>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <code class="flex-1 px-3 py-2 bg-gray-900 text-green-400 rounded-lg text-sm font-mono overflow-x-auto">
                                    {{ keyVisible ? newKeyResult.key : '••••••••••••••••••••••••••••••••••••' }}
                                </code>
                                <button 
                                    @click="keyVisible = !keyVisible"
                                    class="px-3 py-2 bg-gray-200 dark:bg-gray-600 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500"
                                    :title="keyVisible ? t('api_keys.modal.hide') : t('api_keys.modal.show')"
                                >
                                    {{ keyVisible ? '👁️' : '👁️‍🗨️' }}
                                </button>
                                <button 
                                ...
                                >
                                    {{ copySuccess ? '✓ ' + t('api_keys.modal.copied') : '📋 ' + t('api_keys.modal.copy') }}
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex justify-end">
                            ...
                            >
                                {{ keyCopied ? '✓ ' + t('api_keys.modal.close') : t('api_keys.modal.close') }}
                            </button>
                        </div>
                    </div>
                </div>
