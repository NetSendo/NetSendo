...
watch(() => form.provider, (newProvider) => {
    if (newProvider === 'gmail') {
        ...
        form.allowed_types = form.allowed_types.filter(type => type !== 'broadcast');
    } else {
    ...
    if (editingMailbox.value && newProvider === editingMailbox.value.provider) {
        return;
    }
...
const openModal = (mailbox = null) => {
    if (mailbox) {
        ...
        form.allowed_types = mailbox.allowed_types || ['broadcast', 'autoresponder', 'system'];
...
const testConnection = async (mailbox) => {
    ...
    try {
        const response = await fetch(route('settings.mailboxes.test', mailbox.id), {
            method: 'POST',
            headers: {
...
const setDefault = (mailbox) => {
    router.post(route('settings.mailboxes.default', mailbox.id), {}, {
        preserveScroll: true,
        onSuccess: () => {
...
const deleteMailbox = () => {
    ...
    router.delete(route('settings.mailboxes.destroy', deletingMailbox.value.id), {
        preserveScroll: true,
        onSuccess: () => {
            closeDeleteModal();
...
const toggleActive = (mailbox) => {
    router.put(route('settings.mailboxes.update', mailbox.id), {
        name: mailbox.name,
...
const disconnectGmail = () => {
    ...
    router.post(route('settings.mailboxes.gmail.disconnect', editingMailbox.value.id), {}, {
        preserveScroll: true,
        onSuccess: (page) => {
            // Update the editingMailbox with the fresh instance from the updated props
            const updatedMailbox = props.mailboxes.find(m => m.id === editingMailbox.value.id);
            if (updatedMailbox) {
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6">
            ...
            <!-- Empty State -->
            <div v-if="mailboxes.length === 0" class="rounded-xl border-2 border-dashed border-gray-300 p-12 text-center dark:border-slate-700">
                <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">{{ $t('mailboxes.empty.title') }}</h3>
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ $t('mailboxes.empty.description') }}</p>
                <button
                    @click="openModal()"
            ...
            <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div
                    ...
                    :class="mailbox.is_default ? 'border-indigo-300 dark:border-indigo-700' : 'border-slate-200'"
                >
                    <!-- Default Badge -->
                    <div v-if="mailbox.is_default" class="absolute right-3 top-3">
                        <span class="rounded-full bg-indigo-100 px-2 py-1 text-xs font-medium text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">
                            {{ $t('mailboxes.default') }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        <div 
                            ...
                            :style="{ backgroundColor: providerColors[mailbox.provider] + '20' }"
                        >
                            <div 
                                class="h-6 w-6"
                                :style="{ color: providerColors[mailbox.provider] }"
                        ...
                        <button
                            ...
                            :class="mailbox.is_active ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-slate-600'"
                        >
                            <span 
                                class="absolute left-0.5 top-0.5 h-5 w-5 transform rounded-full bg-white shadow transition-transform"
                                :class="mailbox.is_active ? 'translate-x-5' : 'translate-x-0'"
                            ></span>
                        </button>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        >
                            <span 
                                ...
                                :class="{
                                    'bg-emerald-500': mailbox.is_active && mailbox.last_test_success === true,
                                    'bg-rose-500': mailbox.last_test_success === false,
                                    'bg-amber-500': mailbox.is_active && mailbox.last_test_success === null,
                    ...
                    <div v-if="mailbox.daily_limit" class="mt-3">
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <span class="font-medium">{{ mailbox.sent_today || 0 }} / {{ mailbox.daily_limit }}</span>
                        </p>
                    </div>
                    ...
                    <div class="mt-4 flex items-center gap-2">
                        ...
                        >
                            <svg v-if="testingMailbox === mailbox.id" class="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24">
                    ...
                    <div v-if="mailbox.last_test_success === false && mailbox.last_test_message" class="mt-3">
                        <p class="text-xs text-rose-600 dark:text-rose-400 truncate" :title="mailbox.last_test_message">
            ...
            <div class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
                <div class="flex gap-3">
                    ...
                    <div class="text-sm text-blue-800 dark:text-blue-200">
                        <p class="font-medium">{{ $t('mailboxes.info.title') }}</p>
                        <ul class="mt-2 list-inside list-disc space-y-1 text-blue-700 dark:text-blue-300">
                            <li><strong>SMTP:</strong> {{ $t('mailboxes.info.smtp') }}</li>
                            <li><strong>SendGrid:</strong> {{ $t('mailboxes.info.sendgrid') }}</li>
                            <li><strong>Gmail:</strong> {{ $t('mailboxes.info.gmail') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <Teleport to="body">
            ...
            >
                <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800 max-h-[90vh] overflow-y-auto">
                    <!-- Modal Header -->
                    <div class="mb-6 flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            {{ modalMode === 'edit' ? $t('mailboxes.modal.edit_title') : $t('mailboxes.modal.add_title') }}
                        </h3>
                        <button
                            @click="closeModal"
                    ...
                    <form @submit.prevent="submitForm" class="space-y-4">
                        ...
                        <!-- Gmail OAuth Info -->
                        <div v-if="form.provider === 'gmail'" class="rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-900/20">
                            <div class="flex gap-2">
                                <svg class="h-5 w-5 flex-shrink-0 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                        ...
                        <div>
                            ...
                            <input
                                v-model="form.name"
                                type="text"
                                :placeholder="$t('mailboxes.modal.name_placeholder')"
                        ...
                        <div v-if="form.provider !== 'gmail'">
                            ...
                            <input
                                ...
                                type="email"
                                :placeholder="$t('mailboxes.modal.from_email_placeholder')"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                                :class="{ 'border-rose-500': form.errors.from_email }"
                            />
                            <p v-if="form.errors.from_email" class="mt-1 text-sm text-rose-600">{{ form.errors.from_email }}</p>
                        </div>
                        <div v-else class="text-sm text-gray-500 dark:text-gray-400">
                            <p class="mb-2">{{ $t('mailboxes.modal.gmail_auto_email') }}</p>
                        </div>
                        ...
                        <div v-if="form.provider !== 'gmail'">
                            ...
                            <input
                                ...
                                type="email"
                                :placeholder="$t('mailboxes.modal.reply_to_placeholder')"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                                :class="{ 'border-rose-500': form.errors.reply_to }"
                            />
                            <p v-if="form.errors.reply_to" class="mt-1 text-sm text-rose-600">{{ form.errors.reply_to }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="text"
                                :placeholder="$t('mailboxes.modal.from_name_placeholder')"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                                :class="{ 'border-rose-500': form.errors.from_name }"
                            />
                            <p v-if="form.errors.from_name" class="mt-1 text-sm text-rose-600">{{ form.errors.from_name }}</p>
                        </div>


                        <!-- Gmail OAuth Section -->
                        <div v-if="form.provider === 'gmail'" class="space-y-4">
                            <div v-if="!gmail_configured" class="rounded-lg bg-amber-50 p-4 text-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                                <p class="text-sm">
                                    {{ $t('mailboxes.modal.integration_not_configured_warning') }}
                                    <br>
                                    <Link :href="route('settings.integrations.index')" class="font-medium underline hover:text-amber-900 dark:hover:text-amber-100">
                                        {{ $t('mailboxes.modal.go_to_integrations') }}
                                    </Link>
                                </p>
                            </div>
                            ...
                            <div v-else class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                        {{ $t('mailboxes.modal.google_integration_label') }}
                                    </label>
                                    <select
                                        v-model="form.google_integration_id"
                                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                                    >
                                        <option :value="null" disabled>{{ $t('mailboxes.modal.google_integration_placeholder') }}</option>
                                        <option v-for="integration in google_integrations" :key="integration.id" :value="integration.id">
                                            {{ integration.name }} ({{ integration.client_id.substring(0, 15) }}...)
                                        </option>
                                    </select>
                                    <p v-if="!google_integrations.length" class="mt-1 text-xs text-rose-500">
                                        {{ $t('mailboxes.modal.no_integrations') }}
                                    </p>
                                </div>
                                ...
                                <div v-if="editingMailbox">
                                    ...
                                    <div v-if="form.google_integration_id !== editingMailbox.google_integration_id" class="rounded-lg bg-amber-50 p-4 text-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                                        <p class="text-sm">
                                            {{ $t('mailboxes.modal.integration_changed_warning') }}
                                        </p>
                                    </div>

                                    <div v-else-if="!form.google_integration_id" class="rounded-lg bg-blue-50 p-4 text-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
                                         <p class="text-sm">{{ $t('mailboxes.modal.select_integration_to_connect') }}</p>
                                    </div>

                                    <div v-else class="rounded-lg border p-4" :class="editingMailbox.gmail_connected ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-900/20' : 'border-gray-200 bg-gray-50 dark:border-slate-700 dark:bg-slate-800'">
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <h4 class="font-medium" :class="editingMailbox.gmail_connected ? 'text-emerald-800 dark:text-emerald-200' : 'text-gray-900 dark:text-white'">
                                                    {{ editingMailbox.gmail_connected ? $t('mailboxes.oauth.connected') : $t('mailboxes.oauth.not_connected') }}
                                                </h4>
                                                <p v-if="editingMailbox.gmail_email" class="text-sm text-emerald-600 dark:text-emerald-400">
                                                    {{ editingMailbox.gmail_email }}
                                                </p>
                                            </div>
                                            
                                            <div v-if="editingMailbox.gmail_connected">
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    {{ $t('mailboxes.oauth.disconnect') }}
                                                </button>
                                            </div>
                                            <div v-else>
                                                <a
                                                    :href="route('settings.mailboxes.gmail.connect', editingMailbox.id)"
                                                ...
                                                >
                                                    ...
                                                    {{ $t('mailboxes.oauth.connect_google') }}
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div v-else class="rounded-lg bg-blue-50 p-4 text-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
                                    <p class="text-sm">
                        ...
                        <div v-else v-for="field in currentProviderFields" :key="field.name">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                {{ field.label }}
                                <span v-if="modalMode === 'edit' && field.type === 'password'" class="font-normal text-gray-500">
                                    ({{ $t('mailboxes.modal.leave_empty') }})
                                </span>
                            </label>
                            
                            <!-- Minimum Interval (Time Restriction) -->
                            <div v-if="field.name === 'port'" class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                ...
                                <div class="relative mt-1">
                                    ...
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                        <span class="text-gray-500 sm:text-sm">{{ $t('mailboxes.modal.seconds') }}</span>
                                    </div>
                                </div>
                                <p class="mt-1 text-xs text-gray-500">{{ $t('mailboxes.modal.min_interval_desc') }}</p>
                            </div>
                            ...
                            <div v-else-if="field.type === 'password'" class="relative mt-1">
                                <input
                                    v-model="form.credentials[field.name]"
                                    :type="showPassword[field.name] ? 'text' : 'password'"
                                    :placeholder="modalMode === 'edit' ? $t('mailboxes.modal.leave_empty') : (field.placeholder || '')"
                                ...
                                />
                                <p v-if="modalMode === 'edit'" class="mt-1 text-xs text-gray-500">{{ $t('mailboxes.modal.leave_empty') }}</p>
                                <button
                                    type="button"
                                    @click="showPassword[field.name] = !showPassword[field.name]"
                                    class="absolute inset-y-0 right-0 flex items-center px-3 text-gray-400 hover:text-gray-600"
                                >
                                    <svg v-if="showPassword[field.name]" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <label 
                                    ...
                                    :class="{
                                        'border-indigo-300 bg-indigo-50 dark:border-indigo-700 dark:bg-indigo-900/20': form.allowed_types.includes(type),
                                        'border-gray-200 dark:border-slate-600 hover:border-gray-300': !form.allowed_types.includes(type),
                                        'opacity-50 cursor-not-allowed': type === 'broadcast' && isBroadcastDisabled
                                    }"
                                >
                                    <input
                                        type="checkbox"
                                        ...
                                        :disabled="type === 'broadcast' && isBroadcastDisabled"
                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                    />
                                    <div>
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ label }}</span>
                                        <p v-if="type === 'broadcast' && isBroadcastDisabled" class="text-xs text-gray-500">
                                            {{ $t('mailboxes.modal.broadcast_disabled') }}
                                        </p>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                ...
                                <span class="font-normal text-gray-500">({{ $t('mailboxes.modal.optional') }})</span>
                            </label>
                            <input
                        ...
                        <!-- Test Result -->
                        <div v-if="testResult" class="rounded-lg p-4" :class="testResult.success ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-rose-50 dark:bg-rose-900/20'">
                            <div class="flex items-center gap-2">
                                <svg v-if="testResult.success" class="h-5 w-5 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <svg v-else class="h-5 w-5 text-rose-600 dark:text-rose-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span :class="testResult.success ? 'text-emerald-800 dark:text-emerald-200' : 'text-rose-800 dark:text-rose-200'">
                                    {{ testResult.message }}
                                </span>
                            </div>
                        </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {{ $t('mailboxes.delete.confirm', { name: deletingMailbox?.name }) }}
                        </p>
                    </div>
                </div>
