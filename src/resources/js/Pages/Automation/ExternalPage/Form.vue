...
<template>
    <form @submit.prevent="submit" class="space-y-6">
        <div>
            <InputLabel for="name" value="Internal Name" />
            <TextInput
                id="name"
                v-model="form.name"
                type="text"
        ...
        <div>
            <InputLabel for="url" value="Page URL" />
            <TextInput
                id="url"
                v-model="form.url"
                type="url"
            ...
            <p class="text-sm text-gray-500 mt-1">NetSendo will fetch/redirect to this URL.</p>
        </div>

        <div class="block">
            <label class="flex items-center">
                <Checkbox v-model="form.is_redirect" name="is_redirect" :checked="form.is_redirect" />
                <span class="ms-2 text-sm text-gray-600 dark:text-gray-400">Redirect Mode (User is redirected to the URL)</span>
            </label>
            <p class="text-xs text-gray-500 mt-1 ml-6">
                If unchecked, NetSendo will fetch the content and display it (Proxy Mode), allowing for placeholder replacement.
            </p>
        </div>
        
        <div class="border-t pt-4">
             <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Udostępnione pola systemowe</h3>
         ...
         <div class="border-t pt-4">
             <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Udostępnione pola własne (Optional)</h3>
             <!-- For now simple text area to input keys, comma separated -->
             <div class="text-sm text-gray-500 mb-2">Since custom fields are dynamic, please enter the keys you wish to share/replace (comma separated).</div>
             <TextInput
                id="custom_fields"
