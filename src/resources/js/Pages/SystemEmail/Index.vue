...
const toggleActive = (email) => {
    ...
    router.post(
        route("settings.system-emails.toggle", { systemEmail: email.id }),
        {
    ...
    <AuthenticatedLayout>
        <template #header>
            ...
            >
                ...
                <!-- List Selector with Label -->
                <div class="flex flex-col gap-1">
                    <label
                        class="text-sm font-medium text-gray-600 dark:text-gray-400"
                    >
                        {{ t("system_emails.show_for") }}
                    </label>
                    <div class="relative w-full sm:w-80">
                        ...
                        >
                            <div class="flex flex-col">
                                ...
                                >
                                    {{ t("system_emails.select_list_hint") }}
                                </span>
                            </div>
                            <svg
                        ...
                        >
                            <div
                                v-if="isDropdownOpen"
                                class="absolute right-0 z-50 mt-2 w-full origin-top-right rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-800 dark:ring-gray-700"
                            >
                                <div class="p-2">
                                    <input
                                        v-model="searchQuery"
        ...
        <div class="py-8 sm:py-12">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <!-- Global templates info box (when global is selected) -->
                <div
                    v-if="!currentListId"
                    class="mb-6 rounded-lg bg-gray-50 border border-gray-200 p-4 shadow-sm dark:bg-gray-700/50 dark:border-gray-600"
                >
                    <div class="flex items-start gap-3">
                        <span class="text-xl">💡</span>
                        <p class="text-sm text-gray-600 dark:text-gray-300">
                            {{ t("system_emails.global_templates_info") }}
                        </p>
                    </div>
                </div>

                <!-- List-specific editing info box (when a list is selected) -->
                <div
                    v-else
                    class="mb-6 rounded-lg bg-blue-50 p-4 text-sm text-blue-700 shadow-sm dark:bg-blue-900/30 dark:text-blue-200"
                >
                    <div class="flex items-center">
                        ...
                        {{
                            t("system_emails.edit_for_list", {
                                name: lists.find((l) => l.id == currentListId)
                                    ?.name,
                            })
                        }}
                    </div>
                ...
                <!-- Desktop Table View -->
                <div
                    class="hidden overflow-hidden bg-white shadow-sm sm:block sm:rounded-lg dark:bg-gray-800"
                >
                            ...
                            >
                                <tr>
                                    <th class="px-6 py-4 font-semibold">
                                        {{ t("system_emails.table.name") }}
                                    </th>
                                    <th class="px-6 py-4 font-semibold">
                                        {{ t("system_emails.table.subject") }}
                                    </th>
                                    <th class="px-6 py-4 font-semibold">
                                        {{ t("system_emails.table.slug") }}
                                    </th>
                                    <th class="px-6 py-4 font-semibold">
                                        {{ t("system_emails.table.active") }}
                                    </th>
                                    <th class="px-6 py-4 font-semibold">
                                        {{ t("system_emails.table.status") }}
                                    </th>
                                    <th
                                        ...
                                        >
                                            <button
                                                ...
                                                :title="
                                                    email.is_active
                                                        ? t(
                                                              'system_emails.click_to_disable',
                                                          )
                                                        : t(
                                                              'system_emails.click_to_enable',
                                                          )
                                                "
                                            >
                                                <span
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        >
                                            {{
                                                email.is_custom
                                                    ? t(
                                                          "system_emails.customized",
                                                      )
                                                    : t("system_emails.default")
                                            }}
                                        </span>
                                    </td>
                    ...
                    >
                        <div class="p-5">
                            ...
                            >
                                ...
                                <Link
                                    :href="
                                        route('settings.system-emails.edit', {
                                            systemEmail: email.id,
                                            list_id: currentListId,
                                        })
                                    "
