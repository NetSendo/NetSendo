...
const ResizableImageView = defineComponent({
    ...
    setup(props) {
        ...
        const currentWidthPercent = ref(
            parseInt(props.node.attrs["data-width"]) || 100,
        );
        ...
        const onResizeStart = (e) => {
            ...
            if (container) {
                startWidth.value =
                    container.offsetWidth * (currentWidthPercent.value / 100);
            }
        ...
        const onResizeMove = (e) => {
            ...
            const newPercent = Math.round(
                Math.min(100, Math.max(10, (newWidth / containerWidth) * 100)),
            );
        ...
        return {
            ...
            "data-border-radius": {
                ...
                renderHTML: (attributes) => {
                    return {
                        "data-border-radius":
                            attributes["data-border-radius"] || "0",
                    };
                },
            },
            style: {
    ...
    addCommands() {
        return {
            ...
            toggleTextTransform:
                (textTransform) =>
                ({ chain, editor }) => {
                    const currentTransform = editor.getAttributes(
                        this.name,
                    )?.textTransform;
...
// Preview key for forcing iframe refresh (increment to reload)
...
const commonEmojis = Object.values(emojiCategories).flatMap((cat) =>
    cat.emojis.slice(0, 5),
);
        ...
        handleClick: (view, pos, event) => {
            ...
            const imgElement =
                target.tagName === "IMG"
                    ? target
                    : target
                          .closest(".resizable-image-container")
                          ?.querySelector("img");
...
const switchMode = (mode) => {
    ...
    // Force refresh preview when switching to preview mode
    if (mode === "preview") {
        refreshPreview();
    }
...
const setLink = () => {
    if (linkUrl.value) {
        ...
        if (linkText.value && linkText.value !== currentText) {
            const targetAttr = linkTarget.value
                ? ` target="${linkTarget.value}"`
                : "";
            const titleAttr = linkTitle.value
            ...
            editor.value
                ...
                .deleteSelection()
                .insertContent(
                    `<a href="${linkUrl.value}"${targetAttr}${titleAttr}>${linkText.value}</a>`,
                )
                .run();
        } else {
...
const openImageEditModal = (imgElement) => {
    ...
    if (editorInstance) {
        ...
        if (
            ...
            editorInstance.state.doc.descendants((n, pos) => {
                if (
                    n.type.name === "image" &&
                    n.attrs.src === imgElement.getAttribute("src")
                ) {
                    nodeAttrs = n.attrs;
    ...
    if (nodeAttrs?.["data-border-radius"]) {
        ...
        if (dataBorderRadius) {
            ...
            imageBorderRadius.value = borderRadiusMatch
                ? borderRadiusMatch[1]
                : "0";
        }
    }
...
const handleImageUpload = async (event) => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            imageUploadError.value =
                response.data.message || t("editor.image_upload_error");
        }
    } catch (error) {
        ...
        if (error.response?.status === 422) {
            ...
            imageUploadError.value =
                errors?.image?.[0] || t("editor.image_upload_error");
        } else {
            ...
            if (state) {
                ...
                state.doc.descendants((node, pos) => {
                    if (
                        node.type.name === "image" &&
                        node.attrs.src ===
                            editingImageElement.value.getAttribute("src")
                    ) {
                        foundPos = pos;
                        return false; // stop searching
                    }
...
const deleteImage = () => {
    ...
    if (state) {
        ...
        state.doc.descendants((node, pos) => {
            if (
                node.type.name === "image" &&
                node.attrs.src === editingImageElement.value.getAttribute("src")
            ) {
                foundPos = pos;
                return false; // stop searching
            }
    ...
    const handleMessage = (event) => {
        ...
        if (event.data?.type === "htmlUpdate" && event.data.html) {
            ...
            newHtml = newHtml.replace(
                /<style>\s*\[data-edit-id\][\s\S]*?<\/style>/gi,
                "",
            );
            newHtml = newHtml.replace(
                /<script>\s*document\.addEventListener\("DOMContentLoaded"[\s\S]*?<\/script>/gi,
                "",
            );

            // Remove hint element
            newHtml = newHtml.replace(
                /<div class="netsendo-edit-hint"[^>]*>[\s\S]*?<\/div>/gi,
                "",
            );
...
defineExpose({
    ...
    insertAtCursor: (content) => {
        if (
            ...
            if (textarea) {
                ...
                const text = sourceCode.value;
                sourceCode.value =
                    text.substring(0, start) + content + text.substring(end);
                emit("update:modelValue", sourceCode.value);
    ...
    getWrappedContent: () => {
        ...
        const alignStyle =
            contentAlign.value === "center" ? "margin: 0 auto;" : "margin: 0;";
...
<template>
    ...
    >
        <!-- Mode Tabs & Toolbar -->
        <div
            class="border-b border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800"
        >
            <!-- Mode Switcher -->
            <div
            ...
            <div
                v-if="editorMode === 'visual' && editor && !isFullHtmlDocument"
            ...
            >
                    ...
                    >
                        ...
                        >
                            <path
                                ...
                                stroke-linejoin="round"
                                stroke-width="2"
                                d="M3 7V5a2 2 0 012-2h4m6 0h4a2 2 0 012 2v2M3 17v2a2 2 0 002 2h4m6 0h4a2 2 0 002-2v-2"
                            />
                        </svg>
                        <span class="text-xs font-medium min-w-[28px]">{{
                ...
                <div class="relative">
                    ...
                    >
                        ...
                        <span
                            class="absolute bottom-0 left-1/2 -translate-x-1/2 w-3 h-1 rounded"
                            :style="{
                                backgroundColor:
                ...
                <div class="relative">
                    <button
                        type="button"
                        @click="
                            showTextTransformPicker = !showTextTransformPicker;
                            showFontPicker = false;
                            showSizePicker = false;
                            showColorPicker = false;
                            showHighlightPicker = false;
                    ...
                    >
                        <button
                            ...
                            @click="
                                editor
                                    ...
                                    .run();
                                showTextTransformPicker = false;
                            "
                            class="w-full text-left px-3 py-1.5 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors text-slate-700 dark:text-slate-300"
                            :class="{
                                'bg-indigo-100 dark:bg-indigo-900/30':
                                    editor.getAttributes('textTransform')
                                        ?.textTransform === 'uppercase',
                            }"
                        >
                            <span class="uppercase">{{
                                $t("editor.text_transform_uppercase") ||
                                "WIELKIE LITERY"
                            }}</span>
                        </button>
                        <button
                        ...
                        >
                            <span class="lowercase">{{
                                $t("editor.text_transform_lowercase") ||
                                "małe litery"
                            }}</span>
                        </button>
                        <button
                        ...
                        >
                            <span class="capitalize">{{
                                $t("editor.text_transform_capitalize") ||
                                "Pierwsza Wielka"
                            }}</span>
                        </button>
                        <hr
                ...
                >
                    ...
                    <svg
                        ...
                        viewBox="0 0 24 24"
                    >
                        <text x="2" y="8" font-size="6" font-weight="bold">
                            1.
                        </text>
                        <text x="2" y="14" font-size="6" font-weight="bold">
                            2.
                        </text>
                        <text x="2" y="20" font-size="6" font-weight="bold">
                            3.
                        </text>
                        <path
                            stroke="currentColor"
                ...
                <button
                    ...
                    :class="[
                        btnClass(),
                        {
                            'opacity-40 cursor-not-allowed': !editor
                                .can()
                ...
                >
                    ...
                    >
                        <path
                            ...
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M3 4h18M9 9h12M9 14h12M3 19h18M3 9l4 2.5L3 14"
                        />
                    </svg>
                </button>
                <button
                ...
                >
                    ...
                    >
                        <path
                            ...
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M3 4h18M9 9h12M9 14h12M3 19h18M7 9l-4 2.5L7 14"
                        />
                    </svg>
                </button>
                ...
                >
                    ...
                    <div
                        class="mx-0.5 h-4 w-px bg-slate-300 dark:bg-slate-600"
                    ></div>
                    <button
                        type="button"
                ...
                <Teleport to="body">
                    ...
                    >
                        <!-- Category tabs -->
                        <div
                            class="flex gap-1 mb-2 border-b border-slate-200 dark:border-slate-700 pb-2"
                        >
                            <button
                                ...
                                :title="
                                    $t('editor.emoji_categories.' + key) || key
                                "
                            >
                                {{ category.icon }}
                            </button>
                        ...
                        <!-- Emoji grid -->
                        <div
                            class="grid grid-cols-10 gap-0.5 max-h-[200px] overflow-y-auto"
                        >
                            <button
                ...
                <div
                    class="flex items-center gap-0.5 rounded border border-slate-300 dark:border-slate-600 p-0.5"
                >
                    <button
                        type="button"
        ...
        <div v-show="editorMode === 'visual'" :style="{ minHeight }">
            ...
            <div v-if="isFullHtmlDocument" class="bg-white dark:bg-slate-900">
                <div
                    class="p-2 border-b border-slate-200 dark:border-slate-700 bg-emerald-50 dark:bg-emerald-900/20"
                >
                    <p
                        class="text-xs text-emerald-700 dark:text-emerald-300 flex items-center gap-2"
                    >
                        <svg
                            class="h-4 w-4"
                            fill="none"
                ...
                <div
                    class="p-4 flex justify-center bg-slate-50 dark:bg-slate-900/50"
                >
                    <div
            ...
            >
                <div class="flex items-center justify-center gap-2">
                    ...
                    <button
                        ...
                        class="flex items-center gap-2 px-4 py-2 text-sm rounded-lg transition-colors bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 dark:bg-slate-700 dark:text-slate-300 dark:border-slate-600"
                        :title="
                            $t('editor.refresh_preview') || 'Odśwież podgląd'
                        "
                    >
                        <svg
                            class="h-5 w-5"
        ...
        <div
            v-if="showTextEditModal"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
            @click="cancelTextEdit"
        >
            <div
                class="bg-white dark:bg-slate-800 rounded-xl p-6 w-full max-w-xl shadow-2xl"
                @click.stop
            >
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                        class="text-xs text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-full font-mono"
                    >
                        {{ editingElementTag }}
                    </span>
                </div>
                ...
                <div class="mb-4">
                    ...
                    >
                        {{
                            $t("editor.edit_text_modal.content_label") ||
                            "Zawartość"
                        }}
                    </label>
                    <textarea
                        ...
                        :placeholder="
                            $t('editor.edit_text_modal.placeholder') ||
                            'Wprowadź treść...'
                        "
                    ></textarea>
                </div>
                ...
                <div class="mb-6">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        >
                            {{
                                $t("editor.edit_text_modal.insert_variable") ||
                                "Wstaw zmienną"
                            }}
                        </span>
                    </div>
        ...
        >
            <div
                class="bg-white dark:bg-slate-800 rounded-lg p-6 w-full max-w-md shadow-xl"
            >
                <h3
                ...
                <div class="mb-4">
                    ...
                    >
                        <option value="_blank">
                            {{ $t("editor.link_target_blank") }}
                        </option>
                        <option value="_self">
                            {{ $t("editor.link_target_self") }}
                        </option>
                    </select>
                </div>
        ...
        >
            <div
                class="bg-white dark:bg-slate-800 rounded-lg p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto"
                @click.stop
            >
                <h3
                    class="text-lg font-semibold text-slate-900 dark:text-white mb-4"
                ...
                <div
                    ...
                    class="mb-4 border rounded-lg p-3 bg-slate-50 dark:bg-slate-900 max-h-48 overflow-y-auto"
                >
                    <div
                        v-if="isLoadingMedia"
                    ...
                    <div
                        v-else-if="mediaLibrary.length === 0"
                        class="text-center py-4 text-slate-500 dark:text-slate-400 text-sm"
                    >
                ...
                <div
                    v-if="showLogoBrowser"
                    class="mb-4 border rounded-lg p-3 bg-purple-50 dark:bg-purple-900/20 max-h-48 overflow-y-auto"
                >
                    <div
                        v-if="isLoadingMedia"
                        class="flex items-center justify-center py-4"
                    ...
                    <div
                        v-else-if="mediaLibrary.length === 0"
                        class="text-center py-4 text-purple-600 dark:text-purple-400 text-sm"
                    >
                ...
                <div class="mb-4">
                    <label
                        class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1"
                        >{{ $t("editor.image_properties.url") }}</label
                    >
                    <input
                        v-model="imageUrl"
                        type="url"
                ...
                <div v-if="imageUrl" class="mb-4">
                    <label
                        class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"
                        >{{ $t("editor.image_properties.preview") }}</label
                    >
                    <div
                        class="relative bg-slate-100 dark:bg-slate-700 rounded-lg p-4 min-h-[120px] flex items-center justify-center"
                    >
                        <!-- Loading state -->
                        <div
                            v-if="!imagePreviewLoaded && !imagePreviewError"
                ...
                <div v-if="imagePreviewLoaded" class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"
                            >{{
                                $t("editor.image_properties.alignment")
                            }}</label
                        >
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"
                        >
                            {{ $t("editor.image_properties.width") }}:
                            {{ imageWidth }}%
                        </label>
                        <input
                            v-model="imageWidth"
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1"
                            >{{ $t("editor.image_properties.link") }}</label
                        >
                        <input
                            v-model="imageLink"
                            type="url"
