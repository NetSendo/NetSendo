...
const links = computed(() => [
    ...
    { label: t('help_menu.report_bug'), href: 'https://support.netsendo.com/bug', icon: 'bug' },
]);
...
const formatDate = (dateString) => {
    ...
    try {
        return new Date(dateString).toLocaleDateString(locale.value === 'pl' ? 'pl-PL' : locale.value, {
            year: 'numeric',
            month: 'short',
            day: 'numeric',
        });
...
<template>
    <div class="relative">
        ...
        >
            <span class="relative flex h-5 w-5 items-center justify-center text-slate-400 group-hover:text-indigo-400">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <!-- Update notification badge -->
                <span 
                    v-if="versionData?.updates_available && versionData?.update_count > 0"
        ...
        >
            ...
            >
                ...
                <div class="flex items-center justify-between border-b border-white/10 px-4 py-3">
                    <span class="text-sm font-semibold text-white">{{ $t('help_menu.title') }}</span>
                    <button 
                        @click="close"
                ...
                <div v-if="versionData?.updates_available && versionData?.new_versions?.length > 0" class="border-b border-white/10 bg-amber-500/10 p-3">
                    <div class="mb-2 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider text-amber-400">
                                {{ $t('help_menu.updates_available', { count: versionData.update_count }) }}
                            </span>
                        </div>
                        <button 
                    ...
                    <div class="space-y-2">
                        ...
                        >
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs text-slate-400">{{ formatDate(version.published_at) }}</span>
                            </div>
                            <p v-if="version.name && version.name !== version.tag" class="mt-1 text-xs text-slate-300">
                                {{ version.name }}
                            </p>
                        </div>
                        
                        <a 
                            v-if="versionData.new_versions.length > 0 && versionData.new_versions[0].url"
                ...
                <div v-else-if="isLoadingUpdates" class="border-b border-white/10 p-3">
                    <div class="flex items-center gap-2 text-slate-400">
                        <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <span class="text-xs">{{ $t('help_menu.checking_updates') }}</span>
                    </div>
                </div>

                <!-- Up to date message -->
                <div v-else-if="versionData && !versionData.updates_available && !versionData.error" class="border-b border-white/10 p-3">
                    <div class="flex items-center gap-2 text-green-400">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                        <span class="text-xs font-medium">{{ $t('help_menu.up_to_date') }}</span>
                    </div>
                </div>
                ...
                <div class="p-2">
                    ...
                    >
                        <!-- Rocket -->
                        <svg v-if="link.icon === 'rocket'" class="h-4 w-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                        <!-- Book -->
                        <svg v-else-if="link.icon === 'book'" class="h-4 w-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                        </svg>
                        <!-- Users -->
                        <svg v-else-if="link.icon === 'users'" class="h-4 w-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                        </svg>
                        <!-- Academic -->
                        <svg v-else-if="link.icon === 'academic'" class="h-4 w-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z" />
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
                        </svg>
                        <!-- Bug -->
                        <svg v-else-if="link.icon === 'bug'" class="h-4 w-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                        <!-- Code -->
                        <svg v-else-if="link.icon === 'code'" class="h-4 w-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                        </svg>
                        {{ link.label }}
                        <svg v-if="!link.internal" class="ml-auto h-3 w-3 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="border-t border-white/10 p-3">
                    <div class="mb-2 flex items-center gap-2">
                        <span class="text-xs font-semibold uppercase tracking-wider text-slate-400">{{ $t('help_menu.whats_new') }}</span>
                        <span class="rounded-full bg-indigo-500/20 px-1.5 py-0.5 text-[10px] font-medium text-indigo-400">{{ $t('help_menu.new_tag') }}</span>
                    </div>
                    <div class="space-y-1">
                        <div 
                        ...
                        >
                            <span class="h-1 w-1 flex-shrink-0 rounded-full bg-indigo-500"></span>
