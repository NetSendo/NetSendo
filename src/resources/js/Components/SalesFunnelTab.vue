...
const props = defineProps({
    productType: {
        type: String,
        required: true,
        validator: (value) => ['stripe', 'polar'].includes(value),
    },
    products: {
        type: Array,
        default: () => [],
...
function openEditModal(funnel) {
    ...
    funnelForm.value = {
        ...
        embed_settings: { ...funnelOptions.value.default_embed_settings, ...(funnel.embed_settings || {}) },
    };
    showFunnelModal.value = true;
}
...
async function showEmbedCodeModal(product) {
    ...
    try {
        const response = await fetch(route('settings.sales-funnels.embed-code'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
...
async function assignFunnel(product, funnelId) {
    const routeName = props.productType === 'stripe'
        ? 'settings.stripe-products.update'
        : 'settings.polar-products.update';
...
<template>
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            ...
            <div v-else-if="salesFunnels.length === 0" class="p-8 text-center text-gray-500 dark:text-gray-400">
            ...
            <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="funnel in salesFunnels" :key="funnel.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-6 py-4">
                            <div class="font-medium text-gray-900 dark:text-white">{{ funnel.name }}</div>
                            <div v-if="funnel.description" class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">
                                {{ funnel.description }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                            {{ (funnel.stripe_products_count || 0) + (funnel.polar_products_count || 0) }}
                        </td>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="product in products" :key="product.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-6 py-4">
                            ...
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ product.formatted_price }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <select
                                :value="product.sales_funnel_id"
                                @change="assignFunnel(product, $event.target.value)"
                            ...
                            >
                                <option value="">{{ t('sales_funnels.no_funnel') }}</option>
                                <option v-for="funnel in salesFunnels" :key="funnel.id" :value="funnel.id">
                                    {{ funnel.name }}
                                </option>
                            </select>
                        </td>
        ...
        <Modal :show="showFunnelModal" @close="closeFunnelModal" max-width="lg">
            <div class="p-6">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">
                    {{ editingFunnel ? t('sales_funnels.edit') : t('sales_funnels.create') }}
                </h3>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option v-for="list in funnelOptions.contact_lists" :key="list.id" :value="list.id">
                                {{ list.name }}
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                        >
                            <option value="">{{ t('sales_funnels.default_thank_you') }}</option>
                            <option v-for="page in funnelOptions.external_pages" :key="page.id" :value="page.id">
                                {{ page.name }}
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    v-model="funnelForm.embed_settings.button_text"
                                    type="text"
                            ...
                            <div>
                                ...
                                <select
                                    v-model="funnelForm.embed_settings.button_style"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm"
                                >
                                    <option value="rounded">{{ t('sales_funnels.style_rounded') }}</option>
                                    <option value="pill">{{ t('sales_funnels.style_pill') }}</option>
                                    <option value="square">{{ t('sales_funnels.style_square') }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm text-gray-600 dark:text-gray-400 mb-1">
                                ...
                                <input
                                    v-model="funnelForm.embed_settings.button_color"
                                    type="color"
                            ...
                            <div>
                                ...
                                <input
                                    v-model="funnelForm.embed_settings.button_text_color"
                                    type="color"
                                    class="w-full h-10 px-1 py-1 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700"
                                />
                            </div>
        ...
        <Modal :show="showEmbedModal" @close="closeEmbedModal" max-width="lg">
            <div class="p-6">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">
                    📋 {{ t('sales_funnels.embed_code_for') }} {{ selectedProduct?.name }}
                </h3>

                <div class="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 mb-4">
                    <pre class="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-all">{{ generatedEmbedCode }}</pre>
                </div>
