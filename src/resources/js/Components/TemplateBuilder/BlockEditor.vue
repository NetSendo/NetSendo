...
const localTextContent = ref(
    props.block?.content?.html?.replace(/<[^>]*>/g, "") || ""
);
        ...
        if (newBlock) {
            ...
            if (newBlock.id !== currentBlockId.value) {
                ...
                localTextContent.value =
                    newBlock.content?.html?.replace(/<[^>]*>/g, "") || "";
            }
        }
    },
    { deep: true }
    ...
    try {
        ...
        if (response.data.success) {
            ...
            uploadError.value =
                ...
                t("template_builder.image_upload_error");
        ...
        if (error.response?.status === 422) {
            ...
            uploadError.value =
                errors?.image?.[0] || t("template_builder.image_upload_error");
        } else {
...
const handleInsert = (data) => {
    // Extract content from payload (might be object {content, type} or string)
    const content =
        typeof data === "object" && data.content ? data.content : data;
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label
                        ...
                        >{{ $t("template_builder.logo") }}</label
                    >
                    <div
                        v-if="localContent.logo"
                        class="relative mb-2 rounded bg-slate-100 p-2 dark:bg-slate-800"
                    >
                        <img
                        ...
                        />
                        <button
                            @click="updateContent('logo', null)"
                            class="absolute right-1 top-1 rounded bg-red-500 p-1 text-white hover:bg-red-600"
                        >
                            <svg
                                class="h-3 w-3"
                                fill="none"
                    ...
                    <label
                        :class="[
                            ...
                            uploadError
                                ...
                                : 'border-slate-200 bg-slate-50 hover:border-indigo-300 hover:bg-indigo-50 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-indigo-600',
                        ]"
                    >
                        <svg
                            v-if="isUploading"
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.logo_width") }}</label
                    >
                    <input
                        type="range"
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.background_color") }}</label
                    >
        ...
        <template v-else-if="block.type === 'text'">
            <div class="space-y-4">
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            ...
                            >{{ $t("template_builder.content") }}</label
                        >
                        <button
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.alignment") }}</label
                    >
                    <div class="flex gap-1">
                        <button
                            ...
                            :class="
                                localContent.alignment === opt.value
                ...
                <div
                    v-if="aiAvailable"
                    class="rounded-lg border border-indigo-200 bg-indigo-50 p-3 dark:border-indigo-800 dark:bg-indigo-900/20"
                >
                    <label
                        class="mb-2 flex items-center gap-2 text-xs font-medium text-indigo-700 dark:text-indigo-300"
                    >
                        <svg
                            class="h-4 w-4"
                            fill="none"
                    ...
                    <textarea
                        ...
                        :placeholder="
                            $t('template_builder.ai_prompt_placeholder')
                        "
                    ></textarea>
                    <select
                        v-model="aiTone"
                    ...
                    >
                        <option value="casual">
                            {{ $t("template_builder.tone_casual") }}
                        </option>
                        <option value="formal">
                            {{ $t("template_builder.tone_formal") }}
                        </option>
                        <option value="persuasive">
                            {{ $t("template_builder.tone_persuasive") }}
                        </option>
                    </select>
                    <div class="flex gap-2">
                        <button
                        ...
                        >
                            {{
                                isGenerating
                                    ? $t("template_builder.generating")
                                    : $t("template_builder.generate")
                            }}
                        </button>
                        <button
        ...
        <template v-else-if="block.type === 'image'">
            <div class="space-y-4">
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.image") }}</label
                    >
                    <div
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.alt_text") }}</label
                    >
                    <input
                        type="text"
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.link_url") }}</label
                    >
                    <input
                        type="url"
            ...
            <div class="space-y-4">
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            class="text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.button_text") }}</label
                        >
                        <button
                            @click="openInsertPicker('text')"
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label
                            class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{
                                $t("template_builder.background_color")
                            }}</label
                        >
                        <input
                            type="color"
                    ...
                    <div>
                        <label
                            class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.text_color") }}</label
                        >
                        <input
                            type="color"
                            :value="localContent.textColor"
                            @input="
                                updateContent('textColor', $event.target.value)
                            "
                            class="h-10 w-full cursor-pointer rounded border border-slate-200 dark:border-slate-700"
                        />
                    </div>
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.border_radius") }}</label
                    >
                    <select
                        :value="localContent.borderRadius"
                        @change="
                            updateContent('borderRadius', $event.target.value)
                        "
                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                    >
                        <option value="0">
                            {{ $t("template_builder.none") }}
                        </option>
                        <option value="4px">
                            {{ $t("template_builder.small") }}
                        </option>
                        <option value="8px">
                            {{ $t("template_builder.medium") }}
                        </option>
                        <option value="16px">
                            {{ $t("template_builder.large") }}
                        </option>
                        <option value="9999px">
                            {{ $t("template_builder.pill") }}
                        </option>
                    </select>
                </div>
        ...
        <template v-else-if="block.type === 'divider'">
            <div class="space-y-4">
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.line_color") }}</label
                    >
                    <div class="flex gap-2">
                        <input
                            type="color"
        ...
        <template v-else-if="block.type === 'spacer'">
            <div class="space-y-4">
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.height") }}</label
                    >
                    <select
        ...
        <template v-else-if="block.type === 'columns'">
            <div class="space-y-4">
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.columns_count") }}</label
                    >
                    <div class="flex gap-2">
                        <button
                            v-for="n in [2, 3, 4]"
                ...
                >
                    <strong>{{ $t("template_builder.note") }}:</strong>
                    {{ $t("template_builder.columns_note") }}
                </div>
            </div>
        </template>
        ...
        <template v-else-if="block.type === 'product'">
            <div class="space-y-4">
                ...
                <div
                    class="rounded-xl border-2 border-dashed border-purple-200 bg-purple-50/50 p-4 dark:border-purple-800 dark:bg-purple-900/20"
                >
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-lg bg-purple-100 dark:bg-purple-800"
                            >
                                <svg
                                    viewBox="0 0 24 24"
                                    fill="#96588a"
                                ...
                                >
                                    <path
                                        d="M23.594 9.25h-.587c-.22 0-.421.124-.52.32l-.872 1.746a.586.586 0 0 1-.52.32h-5.76a.586.586 0 0 0-.52.32l-.873 1.747a.586.586 0 0 1-.52.32H7.52a.586.586 0 0 0-.52.32l-.873 1.746a.586.586 0 0 1-.52.32H.406A.407.407 0 0 0 0 16.816v1.932c0 .847.686 1.533 1.533 1.533h20.934c.847 0 1.533-.686 1.533-1.533V9.657a.407.407 0 0 0-.406-.407z"
                                    />
                                </svg>
                            </div>
                            <div>
                                ...
                                >
                                    {{
                                        $t(
                                            "template_builder.import_from_woocommerce"
                                        )
                                    }}
                                </p>
                                <p
                                ...
                                >
                                    {{
                                        $t(
                                            "template_builder.import_from_woocommerce_desc"
                                        )
                                    }}
                                </p>
                            </div>
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.product_image") }}</label
                    >
                    <label
                        class="flex cursor-pointer items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500 transition-colors hover:border-indigo-300 hover:bg-indigo-50 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-indigo-600"
                    >
                        <svg
                            class="h-5 w-5"
                            fill="none"
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            class="text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.product_title") }}</label
                        >
                        <button
                            @click="openInsertPicker('title')"
                            class="text-indigo-600 hover:text-indigo-500"
                        >
                            <svg
                                class="h-4 w-4"
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            class="text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.description") }}</label
                        >
                        <button
                            @click="openInsertPicker('description')"
                            class="text-indigo-600 hover:text-indigo-500"
                        >
                            <svg
                                class="h-4 w-4"
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label
                            class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.price") }}</label
                        >
                        <input
                            type="text"
                    ...
                    <div>
                        <label
                            class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.old_price") }}</label
                        >
                        <input
                            type="text"
                            :value="localContent.oldPrice"
                            @input="
                                updateContent('oldPrice', $event.target.value)
                            "
                            class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        ...
                        >
                        <button
                            @click="openInsertPicker('buttonText')"
                            class="text-indigo-600 hover:text-indigo-500"
                        >
                            <svg
                                class="h-4 w-4"
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.button_url") }}</label
                    >
                    <input
                        type="url"
            ...
            <div class="space-y-4">
                <div>
                    <label
                        ...
                        >{{ $t("template_builder.social_icons") }}</label
                    >
                    <div class="space-y-2">
                        <div
                        ...
                        >
                            <input
                                ...
                                :checked="
                                    (localContent.icons || []).some(
                                        (i) => i.type === option.type
                                    )
                                "
                                @change="
                                    toggleSocialIcon(
                ...
                <div
                    v-if="(localContent.icons || []).length > 0"
                    class="space-y-3"
                >
                    <div
                        v-for="icon in localContent.icons || []"
                        :key="icon.type"
                    ...
                    >
                        ...
                        <input
                            ...
                            :value="icon.url"
                            @input="
                                updateSocialUrl(icon.type, $event.target.value)
                            "
                            class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                            placeholder="https://"
                        />
        ...
        <template v-else-if="block.type === 'product_grid'">
            <div class="space-y-4">
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        >
                            ...
                            {{
                                $t("template_builder.select_products") ||
                                $t("template_builder.select_product")
                            }}
                        </button>
                    </div>
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-300"
                        >{{ $t("template_builder.products_count") }}</label
                    >
                    <select
                        :value="(localContent.products || []).length"
                        @change="
                            updateProductsCount(parseInt($event.target.value))
                        "
                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                    >
            ...
            <div class="space-y-4">
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            class="text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.company_name") }}</label
                        >
                        <button
                            @click="openInsertPicker('companyName')"
                            class="text-indigo-600 hover:text-indigo-500"
                        >
                            <svg
                                class="h-4 w-4"
                    ...
                    <input
                        ...
                        @input="
                            updateContent('companyName', $event.target.value)
                        "
                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                    />
                </div>
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            class="text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{ $t("template_builder.address") }}</label
                        >
                        <button
                            @click="openInsertPicker('address')"
                            class="text-indigo-600 hover:text-indigo-500"
                        >
                            <svg
                                class="h-4 w-4"
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label
                            class="text-xs font-medium text-slate-700 dark:text-slate-300"
                            >{{
                                $t("template_builder.unsubscribe_text")
                            }}</label
                        >
                        <button
                            @click="openInsertPicker('unsubscribeText')"
                            class="text-indigo-600 hover:text-indigo-500"
                        >
                            <svg
                                class="h-4 w-4"
