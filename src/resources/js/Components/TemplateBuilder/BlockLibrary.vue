...
const filteredBlocks = computed(() => {
    if (!searchQuery.value) {
        ...
        return { [activeCategory.value]: groupedBlocks.value[activeCategory.value] || [] };
    }
...
<template>
    <div class="flex h-full flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <!-- Header -->
        <div class="shrink-0 border-b border-slate-200 p-4 dark:border-slate-800">
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ $t('template_builder.blocks_library') }}</h3>
            ...
            <div class="mt-3 flex gap-1 overflow-x-auto">
                <button
                    ...
                    :class="activeCategory === 'all' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'"
                ...
                <button
                    ...
                    :class="activeCategory === key ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'"
                    class="shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors"
                >
                    {{ label }}
                </button>
        ...
        <div class="flex-1 overflow-y-auto p-4">
            <div v-for="(blocks, category) in filteredBlocks" :key="category" class="mb-6 last:mb-0">
                <h4 class="mb-2 text-xs font-medium uppercase tracking-wider text-slate-400">
                ...
                >
                    <template #item="{ element: block }">
                        ...
                        >
                            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition-colors group-hover:bg-indigo-100 group-hover:text-indigo-600 dark:bg-slate-700 dark:text-slate-400 dark:group-hover:bg-indigo-900/50 dark:group-hover:text-indigo-400">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="blockIcons[block.type] || 'M4 6h16M4 12h16m-7 6h7'" />
                                </svg>
                            </div>
                            <span class="text-xs font-medium text-slate-600 group-hover:text-indigo-700 dark:text-slate-300 dark:group-hover:text-indigo-300">
                                {{ block.label }}
                            </span>
                        </div>
                    </template>
            ...
            <!-- Empty state -->
            <div v-if="Object.keys(filteredBlocks).length === 0" class="py-8 text-center text-slate-400">
                <p class="text-sm">{{ $t('template_builder.no_blocks_found') }}</p>
            </div>
        </div>

        <!-- AI Generator button -->
        <div v-if="aiAvailable" class="shrink-0 border-t border-slate-200 p-4 dark:border-slate-800">
            <button
                @click="$emit('show-ai')"
        ...
        <div v-if="savedBlocks?.length > 0" class="shrink-0 border-t border-slate-200 p-4 dark:border-slate-800">
            <button
            ...
            >
                <span class="font-medium">{{ $t('template_builder.saved_blocks') }}</span>
                <span class="text-xs text-slate-400">{{ savedBlocks.length }}</span>
            </button>
        </div>
    </div>
</template>
